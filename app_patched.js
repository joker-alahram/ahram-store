...
async function getLiveCheckoutSnapshot() {
  ...
  const [freshProducts, freshCartonPrices, freshPackPrices, freshTier] = await Promise.all([
    ...
    tierKey && ids.length ? apiGet('prices_carton', {
      select: 'product_id,tier_name,price,visible',
      tier_name: `eq.${tierKey}`,
      product_id: `in.(${ids.join(',')})`,
      visible: 'eq.true',
    }).catch(() => []) : Promise.resolve([]),
    tierKey && ids.length ? apiGet('prices_pack', {
      select: 'product_id,tier_name,price,visible',
      tier_name: `eq.${tierKey}`,
      product_id: `in.(${ids.join(',')})`,
      visible: 'eq.true',
    }).catch(() => []) : Promise.resolve([]),
    tierKey ? apiGet('tiers', { select: 'tier_name,min_order,visible_label', tier_name: `eq.${tierKey}`, limit: '1' }).catch(() => []) : Promise.resolve([]),
  ]);

  const productsMap = new Map((freshProducts || []).map((row) => [String(row.product_id), row]));
  const cartonMap = new Map((freshCartonPrices || []).map((row) => [String(row.product_id), Number(row.price)]));
  const packMap = new Map((freshPackPrices || []).map((row) => [String(row.product_id), Number(row.price)]));
  const tierRow = Array.isArray(freshTier) ? freshTier[0] : freshTier?.[0];

  return { productsMap, cartonMap, packMap, tierRow };
}
...
async function validateCartAgainstDatabase() {
  ...
  for (const item of state.cart) {
    ...
    const liveTier = state.selectedTier ? (
      item.unit === 'pack'
        ? snapshot.packMap.get(String(item.id))
        : snapshot.cartonMap.get(String(item.id))
    ) : null;
    const livePrice = Number.isFinite(Number(liveTier)) && Number(liveTier) > 0 ? Number(liveTier) : liveBase;
