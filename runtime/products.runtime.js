...
function compactProductCardHtml(product, options = {}) {
  ...
  const snapshot = product.pricingPreview || getPricingSnapshotForProduct(product, unit);
  ...
  return `
    <article class="product-tile" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body compact">
        ...
        ${typeof pricingBoxHtml === 'function' ? pricingBoxHtml(snapshot, { compact: true, className: 'pricing-box--product', title: 'تفاصيل السعر', subtitle: unit === 'carton' ? 'كرتونة' : 'دستة' }) : `<div class="price-wrap"><span class="price-main">—</span></div>`}
  ...
  return `
    <article class="product-card" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body">
        ...
        <div class="badge-row product-badge-row"><span class="badge badge-tier">يحتسب ضمن الشريحة</span></div>
        ${typeof pricingBoxHtml === 'function' ? pricingBoxHtml(product.pricingPreview || getPricingSnapshotForProduct(product, unit), { compact: true, className: 'pricing-box--product', title: 'تفاصيل السعر', subtitle: unit === 'carton' ? 'كرتونة' : 'دستة' }) : displayPriceBlock(product, unit)}
...
function dealCardHtml(item, type, activeFlash = null) {
  ...
  const price = num(item.pricing?.unit || 0);
  ...
  return `
    <article class="deal-card">
      ...
      <div class="deal-body">
        ...
        ${typeof pricingBoxHtml === 'function' ? pricingBoxHtml(item.pricing, { compact: true, className: 'pricing-box--deal', title: type === 'flash' ? 'عرض الساعة' : 'صفقة اليوم', subtitle: status }) : `<div class="price-wrap"><span class="price-main">${price} ج.م</span></div>`}
        <div class="card-actions">
...
function toggleProductFromCard(productId) {
  ...
  addToCart(CART_ENGINE, state.cart, {
    ...
    quantity: qty,
    product,
    sourceItem: { ...product, pricing: snapshot },
  }, buildPricingContext());
  persistCart();
  resetCheckoutStage();
  renderCart();
