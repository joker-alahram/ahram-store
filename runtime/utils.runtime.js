...
function buildNotifyNode(payload = {}) {
  ...
  if (action && action.label) {
    ...
    actionBtn.dataset.action = 'notify-action';
    actionBtn.__notifyAction = action.onClick;
...
async function loadData() {
  try {
    ...
    const cachedData = loadJSON(STORAGE.dataCache, null);
    const [
      companies,
      viewProducts,
      dailyDealsView,
    ...
    const companyRows = (companies || []).length ? companies : (cachedData?.companies || state.companies);
    const productRows = normalizeProductsFromTables(
      (viewProducts || []).length ? viewProducts : ((cachedData?.products || rawProducts) || []),
      cartonRows || [],
      packRows || []
    );
    const dailyDealRows = (dailyDealsView || []).length ? dailyDealsView : (cachedData?.dailyDeals || dailyDealsRaw);
    const flashOfferRows = (flashOffersView || []).length ? flashOffersView : (cachedData?.flashOffers || flashOffersRaw);
    ...
    state.tiers = (tiers || []).length ? tiers : (cachedData?.tiers || state.tiers);
    state.settings = (settings || []).length ? settings : (cachedData?.settings || state.settings);
