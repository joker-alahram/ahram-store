...
function htmlToFragment(html) {
  ...
  return range.createContextualFragment(String(html || '').trim());
}

function syncAttributes(target, source) {
  if (!target || !source || target.nodeType !== 1 || source.nodeType !== 1) return;

  const targetNames = new Set(target.getAttributeNames ? target.getAttributeNames() : []);
  const sourceNames = new Set(source.getAttributeNames ? source.getAttributeNames() : []);
...
function mountSurface(surface, html) {
  ...
  nextRoot.appendChild(fragment);

  if (!surface.firstChild) {
    while (nextRoot.firstChild) surface.appendChild(nextRoot.firstChild);
    return;
  }

  const currentChildren = Array.from(surface.childNodes);
  const nextChildren = Array.from(nextRoot.childNodes);
  ...
  for (let i = 0; i < max; i += 1) {
    ...
    if (!current && next) {
      surface.appendChild(next.cloneNode(true));
      continue;
    }
    if (current && !next) {
      current.remove();
...
function setPersistentHtml(surface, html) {
  mountSurface(surface, html);
}
