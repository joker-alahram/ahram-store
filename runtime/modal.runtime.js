...
function openProductModal(productId) {
  ...
  setPersistentHtml(els.productModalBody, `
    <div class="modal-hero">
      ...
      <div class="modal-meta">
        ...
        ${typeof pricingBoxHtml === 'function' ? pricingBoxHtml(snapshot, { compact: false, className: 'pricing-box--modal', title: 'تفاصيل السعر', subtitle: product.allow_discount === false ? 'غير قابل للخصم' : 'يتغير وفق الشريحة' }) : `<div class="price-main">${snapshot ? `${num(snapshot.unit || snapshot.breakdown?.final || 0)} ج.م` : '—'}</div>`}
  ...
  setPersistentHtml(els.productModalBody, `
    <div class="modal-hero">
      ...
      <div class="modal-meta">
        ...
        ${typeof pricingBoxHtml === 'function' ? pricingBoxHtml(item.pricing, { compact: false, className: 'pricing-box--offer', title: type === 'flash' ? 'عرض الساعة' : 'صفقة اليوم', subtitle: item.description || '' }) : `<div class="price-main">${num(item.pricing?.unit || 0)} ج.م</div>`}
...
function renderMyDataContent() {
  ...
  setPersistentHtml(els.myDataContent, `
...
async function fulfillPendingTierAction() {
  ...
  if (pending.kind === 'deal' || pending.kind === 'flash') {
    ...
    if (findCartItem(key)) return;
    addToCart(CART_ENGINE, state.cart, {
      key,
      type: pending.kind,
      id: item.id,
          ...
          <div class="register-grid">
            ...
            <div class="field">
              ...
              <div class="location-row">
                ...
                <button class="ghost-btn" id="registerLocateBtn" type="button" data-action="register-locate">تحديد الموقع</button>
              </div>
            </div>
          </div>

          <div class="register-actions">
            <button class="primary-btn" id="registerSubmitBtn" type="submit" data-action="register-submit">تحقق وسجل</button>
            <button class="ghost-btn" id="backToLoginBtn" type="button" data-action="back-login">رجوع للدخول</button>
          </div>
        </form>
      </div>
    </section>
