...
export async function loadCatalog(api, selectedTierName = null) {
  ...
  const requests = await Promise.allSettled([
    ...
    api.get('products', { select: 'product_id,product_name,company_id,product_image,status,visible,has_carton,has_pack,has_half_pack,has_piece,type,category,allow_discount', order: 'product_name.asc' }),
  ]);
  ...
  const rawProducts = requests[8].status === 'fulfilled' && requests[8].value.length ? requests[8].value : fallback.products;
  ...
  const productBase = rawProducts.map(normalizeProduct).filter((row) => row.product_id);
