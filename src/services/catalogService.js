...
export async function loadCatalog(api, selectedTierName = null) {
  ...
  const requests = await Promise.allSettled([
    ...
    api.get('prices', { select: 'product_id,unit_code,tier_name,price,valid_from,valid_to,product_unit_id', order: 'product_id.asc' }),
    ...
    api.get('products', { select: 'product_id,product_name,company_id,product_image,status,visible,has_carton,has_pack,type,category,has_unit,discount_carton,discount_pack,discount_unit', order: 'product_name.asc' }),
  ]);

  const companies = requests[0].status === 'fulfilled' && requests[0].value.length ? requests[0].value : fallback.companies;
  const priceRows = requests[1].status === 'fulfilled' && requests[1].value.length ? requests[1].value : [];
  ...
  const priceRowsForTier = priceRows.filter((row) => String(row.tier_name) === String(activeTierName));
  const productPriceRows = priceRowsForTier.length ? priceRowsForTier : priceRows;

  for (const product of Object.values(productIndex)) {
    const rowsForProduct = productPriceRows.filter((row) => String(row.product_id) === String(product.product_id));
    const sourceRows = rowsForProduct.length ? rowsForProduct : [];
    for (const row of sourceRows) {
      const unit = String(row.unit_code || '').trim();
      if (!unit) continue;
      const value = Number(row.price ?? 0) || null;
      product.prices[unit] = value;
      if (product.sellable_units.indexOf(unit) < 0 && value !== null) product.sellable_units.push(unit);
    }
