...
export async function loadHomeCatalog(api, selectedTierName = null) {
  const requests = await Promise.allSettled([
    ...
    api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc' }),
    api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc' }),
  ]);
  ...
  const catalogRows = requests[5].status === 'fulfilled' && requests[5].value.length ? requests[5].value : [];
  ...
  const catalogProducts = Object.values(catalogIndex).sort((a, b) => {
