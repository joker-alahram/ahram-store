...
const PLACEHOLDER_IMAGE_MAP = {
  company: '/assets/placeholder-company.svg',
  product: '/assets/placeholder-product.svg',
  offer: '/assets/placeholder-product.svg',
  generic: '/assets/placeholder-product.svg',
};

function isExternalPlaceholderHost(hostname = '') {
  const host = String(hostname || '').toLowerCase();
  return host === 'via.placeholder.com' || host === 'placeholder.com' || host.endsWith('.placeholder.com') || host === 'placehold.co' || host === 'dummyimage.com';
}

function sanitizeImageSrc(value, kind = 'generic') {
  const src = String(value ?? '').trim();
  if (!src) return '';
  try {
    const url = new URL(src, typeof window !== 'undefined' ? window.location.href : 'https://example.com');
    if (url.protocol === 'http:' || url.protocol === 'https:') {
      if (isExternalPlaceholderHost(url.hostname)) return PLACEHOLDER_IMAGE_MAP[kind] || PLACEHOLDER_IMAGE_MAP.generic;
    }
  } catch {
    if (/via\.placeholder\.com|placeholder\.com|placehold\.co|dummyimage\.com/i.test(src)) {
      return PLACEHOLDER_IMAGE_MAP[kind] || PLACEHOLDER_IMAGE_MAP.generic;
    }
  }
  return src;
}
...
function normalizeCompany(row) {
  const result = {
    ...
    company_logo: sanitizeImageSrc(row.company_logo || '', 'company'),
...
function normalizeOffer(row, kind) {
  const result = {
    ...
    image: sanitizeImageSrc(row.image || '', kind === 'flash' ? 'offer' : 'product'),
