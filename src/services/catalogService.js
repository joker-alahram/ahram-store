...
const CATALOG_SELECT_VARIANTS = [
  'product_id,product_name,company_id,company_name,company_logo,category,product_image,status,visible,unit_code,tier_name,final_price,can_buy,unit_active',
  'product_id,product_name,company_id,company_name,company_logo,category,product_image,status,visible,unit_code,tier_name,final_price',
...
export async function loadHomeCatalog(api, selectedTierName = null) {
  ...
  const topProductsRequest = api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc' }).catch(() => []);
  const topCompaniesRequest = api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc' }).catch(() => []);
...
export async function loadCompanyCatalog(api, companyId, selectedTierName = null) {
  ...
  const rows = await loadRowsWithProgressiveSelects(api, ['v_catalog_products', 'v_runtime_products_safe', 'v_runtime_products_mobile', 'v_runtime_products_full'], {
    ...
    order: 'display_order.asc,unit_code.asc',
  }, 72, [
    'product_id,product_name,company_id,company_name,company_logo,category,product_image,status,visible,unit_code,tier_name,final_price,can_buy,unit_active',
    'product_id,product_name,company_id,company_name,company_logo,category,product_image,status,visible,unit_code,tier_name,final_price',
    'product_id,product_name,company_id,company_name,company_logo,product_image,status,visible,unit_code,tier_name,final_price',
