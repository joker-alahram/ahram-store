...
export function buildPriceBook(products, tiers, selectedTierName) {
  const normalizedTier = normalizeTierName(selectedTierName)
    || normalizeTierName(tiers.find((tier) => tier.is_default)?.tier_name)
    || 'base';
  const book = {};
  ...
  for (const product of Array.isArray(products) ? products : []) {
    ...
    book[product.product_id] = {
      ...
      prices: Object.fromEntries(unitEntries.map(([unit, value]) => [unit, Number(Number(value.final_price).toFixed(2))])),
      allowDiscount: true,
    };
  }
