...
export async function loadGovernanceProjection(api, session) {
  ...
  const userRows = await api.get('system_users', {
    ...
    if (Array.isArray(byPhone) && byPhone.length) return byPhone;
    return await api.get('system_users', {
      select: 'id,full_name,phone,username,user_type,manager_user_id,is_active,is_blocked,blocked_reason,created_at,updated_at',
