...
export async function submitOrder(api, state, tier, totals) {
  ...
  const linkedRepId = userType === 'rep' ? (session.sales_rep_id || session.id) : session?.sales_rep_id || session?.rep_id || session?.created_by_rep_id || customer?.sales_rep_id || customer?.rep_id || null;
