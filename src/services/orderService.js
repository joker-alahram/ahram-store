...
import { buildOwnershipScope, normalizeOwnershipRow } from '../runtime/ownershipResolver.js';
...
export function validateCheckout(state, tier, totals) {
  ...
  if (userType === 'rep' && !state.auth.selectedCustomer) return { ok: false, code: 'NO_CUSTOMER', message: 'اختر العميل أولاً' };
...
export async function submitOrder(api, state, tier, totals) {
  ...
  const userType = session?.user_type || session?.userType || 'customer';
  const ownership = buildOwnershipScope(session);
  const customer = state.auth.selectedCustomer || (userType === 'customer' ? session : null);
  const linkedRepId = ownership.hasScope ? ownership.sales_rep_id : customer?.sales_rep_id || customer?.rep_id || null;

  if (!customer?.id && userType !== 'rep') throw new Error('INVALID_CUSTOMER');
  ...
  const orderPayload = {
    ...
    rep_id: linkedRepId,
    customer_type: customer?.customer_type || (userType === 'rep' ? 'rep' : 'direct'),
    tier_name: tier?.tier_name || 'base',
  };

  const orderRows = await withTimeout(api.post('orders', orderPayload), 'order create');
  const order = normalizeOwnershipRow(Array.isArray(orderRows) ? orderRows[0] : orderRows);
  ...
  if (orderItemsPayload.length) await withTimeout(api.post('order_items', orderItemsPayload), 'order items create');
