...
export function validateCheckout(state, tier, totals) {
  ...
  if (userType === 'rep' && !state.auth.selectedCustomer?.id) return { ok: false, code: 'NO_CUSTOMER', message: 'اختر العميل أولاً' };
...
export async function submitOrder(api, state, tier, totals) {
  ...
  const linkedRepId = resolveCanonicalRepId(session, customer?.sales_rep_id || null) || customer?.sales_rep_id || null;

  if (userType === 'rep' && !customer?.id) throw new Error('NO_SELECTED_CUSTOMER');
