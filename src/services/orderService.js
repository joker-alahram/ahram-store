...
function normalizeOrderItem(item, orderId) {
  const type = ['product', 'deal', 'flash'].includes(String(item?.type || '').trim()) ? String(item.type).trim() : 'product';
  const qty = Math.max(1, Math.trunc(Number(item?.qty || 1)));
  const price = Number.isFinite(Number(item?.price)) ? Number(Number(item.price).toFixed(2)) : 0;
  const rawUnit = String(item?.unit || 'piece').trim();
  const unit = ['carton', 'pack', 'half_pack', 'piece'].includes(rawUnit) ? rawUnit : 'piece';
