...
export function validateCheckout(state, tier, totals) {
  ...
  if (!Array.isArray(state.commerce.cart) || !state.commerce.cart.length) return { ok: false, code: 'EMPTY_CART', message: 'السلة فارغة' };
...
export async function submitOrder(api, state, tier, totals) {
  ...
  if (isRep && !customer?.id) throw new Error('NO_CUSTOMER');
  ...
  const items = (Array.isArray(state.commerce.cart) ? state.commerce.cart : []).map((item) => ({
    order_id: order.id,
  ...
  try {
    ...
    await api.del('orders', { id: `eq.${order.id}` }).catch(() => null);
    throw error;
  }
