...
export async function loadOffers(api) {
  ...
  return {
    daily: daily.status === 'fulfilled' && Array.isArray(daily.value) ? daily.value.map((row) => normalizeOffer(row, 'daily')) : [],
    flash: flash.status === 'fulfilled' && Array.isArray(flash.value) ? flash.value.map((row) => normalizeOffer(row, 'flash')) : [],
  };
}
