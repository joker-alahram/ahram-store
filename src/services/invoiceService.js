import { storageKeys, saveJSON, loadJSON } from '../core/storage.js';
...
export function persistInvoices(invoices) {
  ...
  for (const invoice of list) {
    const key = String(invoice.id || invoice.order_number || invoice.invoice_number || '');
    if (!key || seen.has(key)) continue;
    seen.add(key);
    deduped.push(invoice);
  }
  saveJSON(storageKeys.invoices, deduped);
}

export function loadCachedInvoices() {
  return loadJSON(storageKeys.invoices, []);
}
