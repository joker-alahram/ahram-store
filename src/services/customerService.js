...
export async function loadRepCustomers(api, repId) {
  if (!repId) return [];
  const query = {
    select: 'id,name,phone,address,location,username,created_at,sales_rep_id,rep_id,created_by,customer_type',
    sales_rep_id: `eq.${repId}`,
    order: 'created_at.desc',
  };
  let rows = await api.get('v_rep_customers', query).catch(() => []);
  if (!Array.isArray(rows) || !rows.length) {
    rows = await api.get('v_rep_customers', {
      ...query,
