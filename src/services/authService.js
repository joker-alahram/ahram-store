...
import { buildOwnershipScope } from '../runtime/ownershipResolver.js';
...
export function normalizeSessionRecord(session) {
  ...
  const normalizedExplicit = normalizeUserType(explicit, null);
  const ownership = buildOwnershipScope(session);
  const inferredRep = normalizedExplicit === 'rep' || Boolean(session.rep_id) || Boolean(session.sales_rep_id && !normalizedExplicit && !session.admin_id && !session.customer_id);
  const canonicalSalesRepId = session.sales_rep_id ?? (inferredRep ? ownership.sessionId : null) ?? null;
  const userType = normalizeUserType(
    explicit
    || (inferredRep && canonicalSalesRepId ? 'rep' : null)
