...
export function normalizeUserType(value, fallback = 'customer') {
  const raw = String(value || '').trim().toLowerCase();
  if (raw === 'rep' || raw === 'sales_rep' || raw === 'sales rep') return 'rep';
...
export function normalizeSessionRecord(session) {
  ...
  const userType = normalizeUserType(session.userType || session.user_type || (session.sales_rep_id ? 'rep' : null));
  return {
    ...session,
    userType,
    user_type: userType,
...
export async function login(api, identifier, password) {
  ...
  const session = {
    ...authenticated,
    userType: normalizeUserType(authenticated?.userType || authenticated?.user_type || (authenticated?.sales_rep_id ? 'rep' : null)),
  };
  session.user_type = session.userType;

  const table = session.userType === 'rep' ? 'sales_reps' : session.userType === 'admin' ? 'admins' : 'customers';
  const profile = await fetchUserProfile(api, table, trimmedIdentifier).catch(() => null);
  const merged = normalizeSessionRecord({ ...(profile || {}), ...session });
