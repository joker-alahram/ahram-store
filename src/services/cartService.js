...
export function addProductToCart(cart, product, tier, unitPreference, quantity = 1) {
  ...
  const next = [...cart, {
    ...
    snapshot: { tierName: tier?.tier_name || null, unit, authoritativePrice: Number(price || 0), basePrice: Number(product.prices?.[unit] || 0), snapshotAt: new Date().toISOString() },
  }];
...
export function recalcCart(cart, productsById, tier) {
  return cart.map((item) => {
    ...
    return {
      ...
      snapshot: { tierName: tier?.tier_name || null, unit: item.unit, authoritativePrice: Number(price || item.price || 0), basePrice: Number(product.prices?.[item.unit] || 0), snapshotAt: new Date().toISOString() },
    };
  });
}
