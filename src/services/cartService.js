...
export function addProductToCart(cart, product, tier, unitPreference, quantity = 1) {
  ...
  const next = [...cart, {
    ...
    snapshot: {
      unit,
      finalPrice: Number(unitRecord?.final_price ?? price ?? 0),
      availableQty: Number(unitRecord?.available_qty ?? 0),
      allowBackorder: unitRecord?.allow_backorder === true,
      runtimeHealthy: unitRecord?.runtime_healthy !== false,
      tierName: normalizeTierName(unitRecord?.tier_name || tier?.tier_name || null),
    },
  }];
...
export function recalcCart(cart, productsById) {
  return (Array.isArray(cart) ? cart : []).map((item) => {
    ...
    return {
      ...
      snapshot: {
        unit: item.unit,
        finalPrice: Number(unitRecord?.final_price ?? price ?? item.price ?? 0),
        availableQty: Number(unitRecord?.available_qty ?? 0),
        allowBackorder: unitRecord?.allow_backorder === true,
        runtimeHealthy: unitRecord?.runtime_healthy !== false,
        tierName: normalizeTierName(unitRecord?.tier_name || item.tierName || null),
