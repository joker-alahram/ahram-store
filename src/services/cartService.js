...
function sanitizeImageSrc(value) {
  ...
  try {
    ...
    if (url.protocol === 'http:' || url.protocol === 'https:') {
      if (host === 'via.placeholder.com' || host === 'placeholder.com' || host.endsWith('.placeholder.com') || host === 'placehold.co' || host === 'dummyimage.com') {
        return '/assets/placeholder-product.svg';
      }
    }
  } catch {
    if (/via\.placeholder\.com|placeholder\.com|placehold\.co|dummyimage\.com/i.test(src)) return '/assets/placeholder-product.svg';
  }
  return src;
}
...
function safeParseCart(raw) {
  ...
  return cart.map((item) => item && typeof item === 'object' ? { ...item, image: sanitizeImageSrc(item.image) } : item);
  } catch {
    return [];
  }
}
...
export function addProductToCart(cart, product, tier, unitPreference, quantity = 1) {
  ...
  const next = [...safeCart, {
    ...
    image: sanitizeImageSrc(product.product_image || ''),
...
export function toggleOfferInCart(cart, offer, kind) {
  ...
  return {
    cart: [...safeCart, {
      ...
      title: offer.title,
      image: sanitizeImageSrc(offer.image || ''),
...
export function recalcCart(cart, productsById) {
  return (Array.isArray(cart) ? cart : []).map((item) => {
    ...
    return {
      ...
      companyId: product.company_id,
      companyName: product.company_name,
      image: sanitizeImageSrc(item.image || product.product_image || ''),
