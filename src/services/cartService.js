...
export function addProductToCart(cart, product, tier, unitPreference, quantity = 1) {
  ...
  const next = [...cart, {
    ...
    snapshot: { tierName: tier?.tier_name || null, unit, finalPrice: Number(product.prices?.[unit] || 0) },
  }];
...
export function recalcCart(cart, productsById, tier) {
  return cart.map((item) => {
    ...
    return {
      ...
      snapshot: { tierName: tier?.tier_name || null, unit: item.unit, finalPrice: Number(product.prices?.[item.unit] || 0) },
    };
  });
}
