...
export function renderHero(container, state, { mode = 'home' } = {}) {
  ...
  const isActive = mode === 'home' && isOfferActive(offer);
  if (!isActive) {
    container.innerHTML = '';
    return;
  }
    ...
    <section class="hero-shell hero-shell--flash">
      ...
      <div class="hero-shell__details hero-shell__details--compact">
        <div class="hero-shell__thumb hero-shell__thumb--secondary">
          ${heroImage ? `<img src="${dom.escape(heroImage)}" alt="${dom.escape(offer.title)}" loading="eager" decoding="async" fetchpriority="high" />` : `<div class="hero-shell__thumb-fallback">${dom.escape(fallback)}</div>`}
        </div>
        <div class="hero-shell__text">
          ...
          <div class="hero-price-row">
            ...
            <span class="badge">${dom.escape(offer.runtime_status === 'active' ? 'متاح' : offer.runtime_status === 'scheduled' ? 'قريبًا' : 'منتهي')}</span>
          </div>
