    ...
    <section class="modal-overlay modal-overlay--center ${state.ui.activeModal === 'customer' ? 'is-open' : 'is-hidden'}" data-modal="customer" role="dialog" aria-modal="true" tabindex="-1">
      ${modalFrame('إضافة عميل', `
        <form class="auth-form auth-form--modal" data-form="customer-create">
          <label><span>اسم العميل</span><input name="name" type="text" required autocomplete="name" /></label>
          <label><span>رقم التليفون</span><input name="phone" type="text" required inputmode="tel" autocomplete="username" /></label>
          <label><span>كلمة المرور</span><input name="password" type="password" required autocomplete="new-password" /></label>
          <label><span>العنوان</span><input name="address" type="text" required autocomplete="street-address" /></label>
          <input name="location" type="hidden" value="" />
          <input name="location_lat" type="hidden" value="" />
          <input name="location_lng" type="hidden" value="" />
          <div class="customer-location-row">
            <button class="btn btn--ghost" type="button" data-action="capture-customer-location">تحديد الموقع</button>
            <span class="badge customer-location-row__status" data-role="customer-location-status">لم يتم تحديد الموقع</span>
          </div>
          <p class="form-hint">رقم الهاتف سيستخدم كاسم المستخدم تلقائيًا.</p>
