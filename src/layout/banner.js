...
import { safeImageUrl } from '../services/pricingService.js';

function resolveBannerImage(state) {
  const settings = state.commerce?.catalog?.settingsMap || {};
  return safeImageUrl(settings.company_banner_image || settings.company_banner || settings.banner_image || settings.home_banner_image || settings.hero_banner || settings.main_banner || '');
}
