  ...
  return `
    <div class="page-stack">
      ...
      <section class="page-section">
        ...
        <div class="offer-grid">${flash.map((offer) => offerCard(offer, 'flash', state.commerce.cart.some((item) => item.key === `flash:${offer.id}:single`), state.runtime.flashState)).join('') || '<div class="empty-state">لا توجد عروض ساعة</div>'}</div>
      </section>
    </div>
  `;
}
