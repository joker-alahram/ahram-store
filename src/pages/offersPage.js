  ...
  return `
    <div class="page-stack">
      <section class="page-section">
        ...
        <div class="offer-grid">${daily.map((offer) => offerCard(offer, 'deal', state.commerce.cart.some((item) => cartItemKey(item) === `deal:${offer.id}::single::`))).join('') || '<div class="empty-state">لا توجد صفقات</div>'}</div>
      </section>
      <section class="page-section">
        <div class="page-section__head"><div><h2>عرض الساعة</h2></div></div>
        <div class="offer-grid">${flash.map((offer) => offerCard(offer, 'flash', state.commerce.cart.some((item) => cartItemKey(item) === `flash:${offer.id}::single::`))).join('') || '<div class="empty-state">لا توجد عروض ساعة</div>'}</div>
      </section>
    </div>
  `;
}
