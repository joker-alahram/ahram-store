...
export function renderCustomersPage(state) {
  if (String(state.auth.session?.userType || state.auth.session?.user_type || '').trim().toLowerCase() !== 'rep') {
...
export function renderAccountPage(state) {
  ...
  return `
    <div class="page-stack">
      <section class="page-section">
        <div class="page-section__head"><div><h2>بياناتي</h2><p>معلومات الجلسة والإعدادات</p></div></div>
        ${session ? `
          <div class="account-card">
            ...
            <div class="account-card__actions">
              ${hasOperationalDashboard(state) ? '<button class="btn btn--primary" type="button" data-action="go-dashboard">لوحة التحكم</button>' : ''}
