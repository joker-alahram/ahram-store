...
export function renderCustomersPage(state) {
  ...
  if (!isRep && !isAdmin) {
    return `<section class="empty-panel"><div class="empty-state">هذه الصفحة متاحة للأدمن أو المندوب فقط</div></section>`;
  }
    ...
    <div class="page-stack">
      <section class="page-section">
        <div class="page-section__head">
          <div>
            <h2>${isAdmin ? 'جميع العملاء' : 'عملائي'}</h2>
            <p>${pendingFlow?.name === 'checkout' ? 'يجب اختيار العميل أولًا' : (isAdmin ? 'قراءة مباشرة من customers' : 'إدارة العملاء المرتبطين بالمندوب')}</p>
          </div>
          ${isRep ? '<button class="btn btn--primary" type="button" data-action="open-customer-modal">إضافة عميل</button>' : ''}
        </div>
      ...
      <section class="page-section">
        ...
        ${session ? `
          <div class="account-card">
            ...
            <div class="account-card__row"><span>النوع</span><strong>${session.userType === 'rep' ? 'مندوب' : (session.userType === 'admin' ? 'أدمن' : 'عميل')}</strong></div>
