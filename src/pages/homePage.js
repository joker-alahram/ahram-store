...
export function renderHomePage(state) {
  ...
  const topProductsReady = Boolean((state.commerce.catalog.top?.products || []).length);
  ...
  const mostRequested = (state.commerce.catalog.top?.products || []).slice(0, 6).map((row) => products.find((product) => String(product.product_id) === String(row.product_id))).filter(Boolean);
