...
function recoveryShelf() {
  return `
    <section class="page-section page-section--dense">
      <div class="empty-state empty-state--recovery">
        <p>لا توجد منتجات</p>
        <button class="btn btn--primary" type="button" data-action="refresh-catalog">تحديث المنتجات والأسعار</button>
      </div>
    </section>
  `;
}
...
export function renderHomePage(state) {
  ...
  return `
    <div class="page-stack page-stack--home">
      ${!products.length ? recoveryShelf() : ''}
      ${shelf('الشركات', 'تصفح الشركات المتاحة', companies.map(companyCard).join('') || '<div class="empty-state">لا توجد شركات</div>', 'page-section--companies', 'company-grid')}
      ...
      ${smartProducts.length ? shelf('الترشيحات الذكية', 'أصناف جاهزة للإضافة بسرعة', smartProducts.map((product) => productCard(product, state.commerce.catalog.tiers?.[0], { unit: state.commerce.unitPrefs[product.product_id], qty: state.commerce.qtyPrefs[product.product_id] || 1, inCart: state.commerce.cart.some((item) => item.type === 'product' && item.id === product.product_id) })).join(''), 'page-section--products', 'product-grid') : ''}
    </div>
  `;
}
