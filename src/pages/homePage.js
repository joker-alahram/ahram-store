...
function rankProducts(state) {
  ...
  if (indexed.length) return indexed.slice().sort((a, b) => String(a.product_name).localeCompare(String(b.product_name), 'ar'));
  return (state.commerce.catalog.products || []).slice().sort((a, b) => String(a.product_name).localeCompare(String(b.product_name), 'ar'));
}
