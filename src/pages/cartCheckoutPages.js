        ...
        <div class="cart-list">
          ${items.length ? items.map((item) => {
            ...
            <article class="cart-item">
              <div class="cart-item__media cart-item__media--empty"></div>
              <div>
                <h3>${item.productName || ''}</h3>
                <p>${item.unitCode || ''}</p>
                <p>${formatMoney(item.finalPrice || 0)} ج.م</p>
                <div class="cart-item__actions">
                  <button type="button" data-action="qty-down" data-key="${key}">-</button><input type="number" min="1" value="${Number(item.qty || 1)}" data-role="cart-qty" data-key="${key}" /><button type="button" data-action="qty-up" data-key="${key}">+</button>
                  <button class="btn btn--ghost" type="button" data-action="remove-item" data-key="${key}">حذف</button>
                </div>
              </div>
            </article>
