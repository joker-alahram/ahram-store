    ...
    <article class="product-card" data-product-id="${dom.escape(product.product_id)}">
      ...
      <div class="product-card__body">
        ...
        <label class="qty-field">
          <span>الكمية</span>
          <input type="number" min="1" value="${Number(qty || 1)}" data-role="product-qty" data-product-id="${dom.escape(product.product_id)}" />
        </label>
        <div class="unit-group">${units}</div>
        <button class="btn btn--primary product-card__cta" type="button" data-action="toggle-product" data-product-id="${dom.escape(product.product_id)}">${inCart ? 'إزالة من السلة' : 'إضافة للسلة'}</button>
      </div>
    </article>
  `;
}
