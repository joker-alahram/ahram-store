  ...
  return `
    <article class="company-card" data-action="open-company" data-company-id="${dom.escape(company.company_id)}">
      <div class="company-card__logo">
        ${company.company_logo ? `<img src="${dom.escape(company.company_logo)}" alt="${dom.escape(company.company_name)}" loading="lazy" decoding="async" />` : `<span>${dom.escape((company.company_name || '').slice(0, 1) || 'A')}</span>`}
      </div>
...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const image = product.product_image ? `<img src="${dom.escape(product.product_image)}" alt="${dom.escape(product.product_name)}" loading="lazy" decoding="async" />` : `<div class="product-card__image-fallback">${dom.escape(product.product_name.slice(0, 1) || 'P')}</div>`;
    ...
    <article class="offer-card ${isFlash ? 'offer-card--flash' : 'offer-card--deal'} ${isFlash ? (runtimeStatus === 'active' ? 'is-active' : '') : ''}">
      ...
      <div class="offer-card__body offer-card__body--compact">
        <button class="offer-card__media" type="button" data-action="open-offer" data-offer-type="${kind}" data-id="${offer.id}">
          ${offer.image ? `<img src="${dom.escape(offer.image)}" alt="${dom.escape(title)}" loading="lazy" decoding="async" />` : `<div class="offer-card__image-fallback">${dom.escape(title.slice(0, 1) || 'O')}</div>`}
        </button>
        <div class="offer-card__content">
          ...
          <div class="offer-card__pricebar">
            ...
            <button class="btn btn--primary offer-card__buy" type="button" data-action="${actionAttr}" data-id="${offer.id}" ${isFlash && runtimeStatus !== 'active' ? 'disabled' : offer.can_buy === false ? 'disabled' : ''}>${cta}</button>
          </div>
