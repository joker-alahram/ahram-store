...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const unitRecord = product.units?.[selectedUnit] || null;
    ...
    <article class="product-card" data-product-id="${dom.escape(product.product_id)}">
      ...
      <div class="product-card__body">
        ...
        <div class="product-card__price-row">
          <span class="price price--main">${formatMoney(unitRecord?.final_price || 0)} ج.م</span>
...
export function invoiceCard(invoice, customerName = '') {
  const number = invoice.order_number || invoice.invoice_number || invoice.id;
  return `
    <article class="invoice-card">
      <div class="invoice-card__top">
        <div>
          <h3>فاتورة #${dom.escape(number)}${customerName ? ` · ${dom.escape(customerName)}` : ''}</h3>
      ...
      <div class="invoice-card__actions">
        <button class="btn btn--ghost" type="button" data-action="open-invoice" data-invoice-id="${dom.escape(String(invoice.id || invoice.order_number || invoice.invoice_number))}">عرض الفاتورة</button>
      </div>
    </article>
  `;
}
