...
import { roleLabel } from '../services/contractUtils.js';

function productPlaceholder(name = 'P') {
  return `<div class="product-card__image-fallback">${dom.escape((name || 'P').slice(0, 1).toUpperCase())}</div>`;
}

function unitBadge(unit) {
  return `<span class="badge badge--soft">${dom.escape(labelForUnit(unit.unit_code))} · ${formatMoney(unit.final_price)} ج.م</span>`;
}
...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const units = (product.units || []).map((row) => `<button class="unit-chip ${row.unit_code === selectedUnit ? 'is-active' : ''}" data-action="set-unit" data-product-id="${dom.escape(product.product_id)}" data-unit="${dom.escape(row.unit_code)}">${dom.escape(labelForUnit(row.unit_code))}</button>`).join('');
  const image = product.product_image ? `<img src="${dom.escape(product.product_image)}" alt="${dom.escape(product.product_name)}" loading="lazy" />` : productPlaceholder(product.product_name);
  const stockLabel = `${formatMoney(display.available_qty || 0)} متاح`;
  ...
  return `
    <article class="product-card" data-product-id="${dom.escape(product.product_id)}">
      ...
      <div class="product-card__body">
        ...
        ${product.category_name ? `<div class="product-card__meta product-card__meta--muted">${dom.escape(product.category_name)}</div>` : ''}
        ...
        <div class="badge-row badge-row--tight">
          <span class="badge">${dom.escape(stockLabel)}</span>
          ${display.minimum_sell_qty > 1 ? `<span class="badge">حد أدنى ${dom.escape(String(display.minimum_sell_qty))}</span>` : ''}
        </div>
...
export function offerCard(offer, kind, inCart = false) {
  const status = kind === 'flash' ? formatStatus(offer.status) : (offer.can_buy ? 'متاح' : 'غير متاح');
  return `
  ...
  return `
    <article class="tier-card ${active ? 'is-active' : ''}">
      <div class="tier-card__head">
        <div>
          <h3>${dom.escape(tier.display_name || tier.visible_label || tier.tier_name)}</h3>
          <p>حد أدنى ${formatMoney(tier.min_order || 0)} ج.م</p>
        </div>
        <span class="badge">${tier.is_default ? 'افتراضي' : 'متاح'}</span>
      </div>
  ...
  return `
    <article class="invoice-card">
      <div class="invoice-card__top">
        ...
        <strong>${formatMoney(invoice.grand_total ?? invoice.total_amount ?? 0)} ج.م</strong>
      </div>
      <div class="invoice-card__meta">
        <span class="chip">${dom.escape(roleLabel(invoice.user_type || 'customer'))}</span>
        <span class="chip">${dom.escape(formatStatus(invoice.status || 'submitted'))}</span>
      </div>
    </article>
  `;
}
...
export function customerCard(customer, selected = false) {
  return `
    <article class="customer-card ${selected ? 'is-selected' : ''}" data-action="select-customer" data-customer-id="${dom.escape(customer.id)}">
      ...
      <div class="customer-card__address">${dom.escape(customer.address || customer.location || 'بدون عنوان')}</div>
      <div class="customer-card__meta">${dom.escape(customer.customer_type || '')}</div>
