...
export function hasOperationalDashboard(state) {
  ...
  const sessionType = String(state?.auth?.session?.userType || state?.auth?.session?.user_type || '').trim().toLowerCase();
  return Boolean(user?.id || user?.phone || user?.username || capabilities.length || sessionType === 'rep');
}
