export function computeCartTotals(state) {
  ...
  const totals = lines.reduce((acc, item) => {
    const amount = Number(item.finalPrice || 0) * Number(item.qty || 0);
    acc.grand += amount;
    const productId = String(item.productId || '').trim();
    if (productId.startsWith('deal:')) acc.deals += amount;
    else if (productId.startsWith('flash:')) acc.flash += amount;
    else acc.products += amount;
    return acc;
