export function parseRoute(hash = location.hash || '#home') {
  ...
  if (path === 'invoice-details') return { name: 'invoice-details', params: { invoiceId: rest[0] || '' } };
...
export function toHash(routeName, params = {}) {
  ...
  if (routeName === 'invoice-details') return `#invoice-details/${encodeURIComponent(params.invoiceId || '')}`;
  return `#${routeName}`;
}
