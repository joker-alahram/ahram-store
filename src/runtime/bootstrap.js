...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'clear-search') { store.patch({ ui: { ...state.ui, search: '' } }); clearTimeout(searchTypingTimer); schedule('header', 'theme', 'banner', 'hero', 'page'); return; }
    if (action === 'set-theme') {
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.id === 'searchInput') {
      ...
      searchTypingTimer = setTimeout(() => schedule('page', 'search'), 500);
      return;
    }
