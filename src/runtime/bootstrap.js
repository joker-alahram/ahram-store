  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'customer-create') {
      ...
      const payload = {
        ...
        customer_type: 'rep',
        sales_rep_id: session.sales_rep_id || session.id,
...
async function loadInvoicesIntoState(store, api) {
  ...
  if (normalizeUserType(session.userType) === 'rep') {
    const customers = state.commerce.customers?.length ? state.commerce.customers : await loadRepCustomers(api, session.sales_rep_id || session.id).catch(() => []);
    const customerIds = Array.from(new Set((customers || []).map((customer) => String(customer.id || '').trim()).filter(Boolean)));
    const canonicalRepId = session.sales_rep_id || session.id;
    const primaryFilters = [`sales_rep_id.eq.${canonicalRepId}`];
    const legacyFilters = [`rep_id.eq.${canonicalRepId}`];
    if (customerIds.length) {
      primaryFilters.push(`customer_id.in.(${customerIds.join(',')})`);
      legacyFilters.push(`customer_id.in.(${customerIds.join(',')})`);
    }
