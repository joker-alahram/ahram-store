...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const refreshedTier = refreshedCatalog.tiers.find((tier) => normalizeTierName(tier.tier_name) === normalizeTierName(nextTier?.tier_name || null)) || refreshedCatalog.tiers.find((tier) => tier.is_default) || refreshedCatalog.tiers[0] || null;
      const refreshedPriceBook = buildPriceBook(Object.values(refreshedCatalog.productIndex || {}), refreshedCatalog.tiers, refreshedTier?.tier_name || null);
      const refreshedCart = recalcCart(store.getState().commerce.cart, refreshedCatalog.productIndex);
      store.update((draft) => {
        draft.commerce.selectedTier = normalizeTierName(refreshedTier?.tier_name || null);
        draft.commerce.catalog = { ...refreshedCatalog, offers: refreshedCatalog.offers };
        draft.commerce.priceBook = { tierName: normalizeTierName(refreshedTier?.tier_name || null), products: refreshedPriceBook };
        draft.commerce.cart = refreshedCart;
      }, { action: 'select-tier', dirty: ['header', 'page', 'drawer', 'hero'] });
      persistSelectedTier(refreshedTier?.tier_name || null);
      persistCart(refreshedCart);
      notify(store, 'success', 'تمت الشريحة', refreshedTier?.visible_label || 'الشريحة الرئيسية');
...
export async function bootstrapApp() {
  ...
  try {
    ...
    const currentTier = catalog.tiers.find((tier) => normalizeTierName(tier.tier_name) === selectedTier) || catalog.tiers.find((tier) => tier.is_default) || catalog.tiers[0] || null;
    ...
    const cart = recalcCart(store.getState().commerce.cart.length ? store.getState().commerce.cart : hydrateCart(), productIndex);
    store.patch({
      commerce: {
