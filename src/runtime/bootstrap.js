...
import { login, logout, registerCustomer, restoreSession } from '../services/authService.js';
...
import { renderAdminPage, renderRepDashboardPage, renderRepOrdersPage } from '../pages/adminPages.js';
...
function renderPage(state, nodes) {
  ...
  switch (route.name) {
    ...
    case 'admin': pageHtml = renderAdminPage(state); break;
    case 'rep-dashboard': pageHtml = renderRepDashboardPage(state); break;
    case 'rep-orders': pageHtml = renderRepOrdersPage(state); break;
    case 'rep-customers': pageHtml = renderCustomersPage(state); break;
    case 'rep-performance': pageHtml = renderRepDashboardPage(state); break;
    case 'my-orders': pageHtml = renderInvoicesPage(state); break;
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'go-admin') return navigateAuthority(store, 'admin');
    if (action === 'go-rep-dashboard') return navigateAuthority(store, 'rep-dashboard');
    if (action === 'go-rep-orders') return navigateAuthority(store, 'rep-orders');
    ...
    if (formType === 'login') {
      ...
      try {
        ...
        if (pendingFlow?.name === 'checkout' && session.userType === 'rep') {
          ...
          clearPendingFlow(store);
          navigateAuthority(store, 'admin');
        } else if (session.userType === 'rep') {
          clearPendingFlow(store);
          navigateAuthority(store, 'rep-dashboard');
        } else {
          clearPendingFlow(store);
          navigateAuthority(store, 'home');
        }
...
async function loadInvoicesIntoState(store, api) {
  ...
  const rows = await api.get('v_orders_status', {
    select: '*',
    [key]: `eq.${session.id}`,
...
async function loadCustomersIntoState(store, api, session = null) {
  const state = store.getState();
  const actor = session || state.auth.session;
  if (!actor) {
    store.update((draft) => { draft.commerce.customers = []; });
    return;
  }
  if (actor.userType === 'admin' || actor.user_type === 'admin') {
    const rows = await api.get('customers', { select: '*', order: 'created_at.desc' }).catch(() => []);
    store.update((draft) => { draft.commerce.customers = Array.isArray(rows) ? rows.map(normalizeCustomerRow) : []; });
    return;
  }
  if (actor.userType !== 'rep' && actor.user_type !== 'sales_rep') {
    store.update((draft) => { draft.commerce.customers = []; });
    return;
  }
  const rows = await loadRepCustomers(api, actor.id);
...
export async function bootstrapApp() {
  ...
  function renderContent() {
    ...
    else if (route === 'admin') html = renderAdminPage(state);
    else if (route === 'rep-dashboard') html = renderRepDashboardPage(state);
    else if (route === 'rep-orders') html = renderRepOrdersPage(state);
    else if (route === 'rep-customers') html = renderCustomersPage(state);
    else if (route === 'rep-performance') html = renderRepDashboardPage(state);
    else if (route === 'my-orders') html = renderInvoicesPage(state);
  ...
  // Restore and validate session against the authoritative auth view
  const restored = await restoreSession(api);
  if (restored) {
    store.patch({ auth: { ...store.getState().auth, session: restored, selectedCustomer: store.getState().auth.selectedCustomer } });
    if (restored.userType === 'rep') {
      await loadCustomersIntoState(store, api, restored);
    }
  }
