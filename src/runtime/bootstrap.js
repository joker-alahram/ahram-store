...
function setBodyMode({ checkout = false } = {}) {
  document.body.classList.toggle('body--checkout', checkout);
}
...
async function ensureCompanyCatalogLoaded(store, api, companyId) {
  ...
  if (companyLoadPromises.has(trimmed)) return companyLoadPromises.get(trimmed);
  ...
  const promise = (async () => {
    ...
    try {
      ...
      store.update((draft) => {
        ...
        draft.commerce.cart = recalcCart(draft.commerce.cart, draft.commerce.catalog.productIndex);
...
function applyShellVisibility(route, nodes, state) {
  ...
  lockBodyScroll(Boolean(state.ui.drawerOpen) || Boolean(state.ui.activeModal) || route.name === 'checkout');
  document.body.classList.toggle('body--overlay', ['login', 'register'].includes(route.name));
  document.body.classList.toggle('body--checkout', route.name === 'checkout');
}
...
async function continuePendingAddToCart(store, schedule, productId) {
  ...
  const rebuilt = recalcCart(result.cart, state.commerce.catalog.productIndex);
  store.update((draft) => {
    draft.commerce.cart = rebuilt;
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'toggle-deal' || action === 'toggle-flash') {
      ...
      const rebuilt = recalcCart(result.cart, state.commerce.catalog.productIndex);
      store.update((draft) => { draft.commerce.cart = rebuilt; }, { action: 'offer-toggle' });
      persistCart(rebuilt);
    ...
    if (action === 'remove-item') {
      ...
      const rebuilt = recalcCart(next, state.commerce.catalog.productIndex);
      store.update((draft) => { draft.commerce.cart = rebuilt; }, { action: 'cart-remove' });
      persistCart(rebuilt);
      schedule('header', 'banner', 'page', 'drawer');
      return;
    }
    ...
    if (action === 'qty-up' || action === 'qty-down') {
      ...
      const rebuilt = recalcCart(next, state.commerce.catalog.productIndex);
      store.update((draft) => { draft.commerce.cart = rebuilt; }, { action: 'cart-qty-step' });
      persistCart(rebuilt);
      schedule('header', 'banner', 'page', 'drawer');
      return;
    }
    ...
    if (action === 'logout') {
      logout();
      store.update((draft) => { draft.auth.session = null; draft.auth.selectedCustomer = null; draft.ui.accountMenuOpen = false; draft.ui.activeModal = null; draft.ui.pendingFlow = null; draft.commerce.cart = []; });
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.getAttribute('data-role') === 'cart-qty') {
      ...
      store.update((draft) => { draft.commerce.cart = recalcCart(updateQty(draft.commerce.cart, key, qty), draft.commerce.catalog.productIndex); }, { action: 'cart-qty-update', dirty: ['page', 'drawer', 'header'] });
      persistCart(store.getState().commerce.cart);
      schedule('header', 'banner', 'page', 'drawer');
      return;
    }
...
async function loadInvoicesIntoState(store, api) {
  ...
  const query = session.userType === 'rep'
    ? { sales_rep_id: `eq.${session.id}` }
    : { customer_id: `eq.${session.id}` };
...
async function performCheckout(store, api, schedule) {
  ...
  try {
    ...
    const whatsappSent = dispatchWhatsAppInvoice(whatsappUrl);
    if (!whatsappSent) {
      notify(store, 'warning', 'تم حفظ الفاتورة لكن تعذر فتح واتساب', '');
    }
    store.update((draft) => {
      draft.commerce.cart = [];
...
export async function bootstrapApp() {
  ...
  function applyBodyFlags() {
    ...
    document.body.classList.toggle('body--checkout', route === 'checkout');
    lockBodyScroll(Boolean(state.ui.drawerOpen) || Boolean(state.ui.activeModal) || route === 'checkout');
  }
  ...
  store.update((draft) => { draft.runtime.loading.catalog = true; draft.runtime.loading.auth = true; });
  purgeLegacyStorage();
  try {
    const [session, catalog] = await Promise.all([
      loadPersistedSession(api),
      loadHomeCatalog(api),
    ]);
    ...
    store.patch({
      auth: { ...store.getState().auth, session: session || null, selectedCustomer: null },
      commerce: {
      ...
      runtime: { ...store.getState().runtime, loading: { ...store.getState().runtime.loading, catalog: false, auth: false }, flashState: computeFlashState(catalog.offers.flash) },
    });
    persistSelectedTier(selectedTier);
    persistCart(cart);
