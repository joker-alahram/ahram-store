...
function isSearchInput(target) {
  ...
  return target.id === 'searchInput' || target.id === 'headerSearchInput' || target.getAttribute('data-role') === 'search-input';
}

function snapshotSearchInput() {
  const active = document.activeElement;
  if (!isSearchInput(active)) return null;
  return {
    id: active.id || null,
    value: active.value,
...
function restoreSearchInput(snapshot) {
  if (!snapshot) return;
  const selector = snapshot.id ? `#${CSS.escape(snapshot.id)}` : '[data-role="search-input"]';
  const input = dom.q(selector);
...
function renderPage(state, nodes) {
  ...
  renderSearchBar(nodes.search, state, { routeName: route.name, show: ['home', 'search'].includes(route.name) });
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'clear-search') { store.patch({ ui: { ...state.ui, search: '' } }); clearTimeout(searchTypingTimer); schedule('header', 'search', 'hero', 'page'); return; }
    if (action === 'set-theme') {
  ...
  document.addEventListener('input', (event) => {
    ...
    if (isSearchInput(target)) {
      ...
      searchTypingTimer = setTimeout(() => schedule('header', 'search', 'page'), 500);
      return;
    }
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    ...
    search: () => renderSearchBar(nodes.search, store.getState(), { routeName: store.getState().app.route.name, show: ['home', 'search'].includes(store.getState().app.route.name) }),
  ...
  function renderContent() {
    const state = store.getState();
    const snapshot = snapshotSearchInput();
    ...
    if (hydrating) {
      nodes.page.innerHTML = '<section class="page-section"><div class="empty-state empty-state--recovery">جارٍ تهيئة البيانات…</div></section>';
  ...
  function applyBodyFlags() {
    ...
    nodes.search.classList.toggle('is-hidden', !['home', 'search'].includes(route));
