...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const catalog = await loadCatalog(api, nextTier?.tier_name || null);
      const latest = store.getState();
      const nextCart = recalcCart(latest.commerce.cart.length ? latest.commerce.cart : hydrateCart(), catalog.productIndex, nextTier || getSelectedTier({ ...latest, commerce: { ...latest.commerce, catalog } }));
      store.patch({
        commerce: {
          ...latest.commerce,
          catalog,
          selectedTier: nextTier?.tier_name || null,
          priceBook: buildPriceBook(Object.values(catalog.productIndex || {}), catalog.tiers, nextTier?.tier_name),
          cart: nextCart,
        },
      });
