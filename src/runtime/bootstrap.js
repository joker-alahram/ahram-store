...
import { normalizeThemeName, persistTheme, themeMetaColor } from '../core/theme.js';
...
function setTheme(theme) {
  ...
  if (meta) meta.setAttribute('content', themeMetaColor(normalized));
}
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'set-theme') {
      const theme = normalizeThemeName(target.getAttribute('data-theme'));
      store.update((draft) => { draft.ui.theme = theme; }, { action: 'set-theme', dirty: ['banner', 'page'] });
      setTheme(theme);
      schedule('banner', 'page');
      return;
    }
