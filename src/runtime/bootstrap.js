...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const refreshedTier = refreshedCatalog.tiers.find((tier) => tier.tier_name === (nextTier?.tier_name || null)) || refreshedCatalog.tiers.find((tier) => tier.is_default) || refreshedCatalog.tiers[0] || null;
      ...
      store.update((draft) => {
        draft.commerce.selectedTier = refreshedTier?.tier_name || null;
        draft.commerce.catalog = { ...refreshedCatalog, offers: refreshedCatalog.offers };
        draft.commerce.priceBook = { tierName: refreshedTier?.tier_name || null, products: refreshedPriceBook };
