...
export async function bootstrapApp() {
  ...
  renderContent();
  purgeLegacyStorage();
  setRuntimePhase(store, RUNTIME_PHASES.HYDRATING_RUNTIME, { pricingReady: false, cartSynced: false });
  try {
    ...
    store.patch({
      ...
      runtime: { ...store.getState().runtime, loading: { ...store.getState().runtime.loading, catalog: false, pricing: true }, flashState: computeFlashState((catalog.offers && catalog.offers.flash) || []) },
      app: { ...store.getState().app, lastError: null },
    });
    setRuntimePhase(store, RUNTIME_PHASES.SYNCING_CART, { pricingReady: true });
    ...
    store.patch({
      ...
      runtime: { ...store.getState().runtime, loading: { ...store.getState().runtime.loading, catalog: false, pricing: true }, flashState: computeFlashState((fallbackCatalog.offers && fallbackCatalog.offers.flash) || []) },
      app: { ...store.getState().app, lastError: null },
    });
    setRuntimePhase(store, RUNTIME_PHASES.SYNCING_CART, { pricingReady: true });
    const runtimeProducts = store.getState().commerce.catalog.productIndex || {};
  ...
  setRuntimePhase(store, RUNTIME_PHASES.READY, { locked: false, pricingReady: true, cartSynced: true });
  store.update((draft) => {
