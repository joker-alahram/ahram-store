...
import { parseRoute, navigate } from '../core/navigation/index.js';
import { createEmitter, createRenderLoop } from '../core/rendering/index.js';
...
import { loadHomeCatalog, loadHomeSupplementary, loadTopSections, loadCompanyCatalog, aggregateRuntimeProducts, projectRuntimeProducts } from '../domains/commerce/services/catalogService.js';
import { buildPriceBook, persistSelectedTier, resolveProductUnit, syncCartPrices, normalizeTierName } from '../domains/commerce/services/pricingService.js';
import { addProductToCart, clearCart, computeTotals, hydrateCart, persistCart, recalcCart, removeItem, toggleOfferInCart, updateQty } from '../domains/commerce/services/cartService.js';
import { login, logout, registerCustomer, normalizeUserType, normalizeSessionRecord } from '../domains/auth/services/authService.js';
import { loadRepCustomers, createCustomer, persistSelectedCustomer } from '../domains/reps/services/customerService.js';
import { computeFlashState, isOfferActive } from '../domains/commerce/services/offerService.js';
import { validateCheckout, submitOrder } from '../domains/commerce/services/orderService.js';
import { buildWhatsAppInvoice, formatMoney, formatStatus, persistInvoices } from '../domains/commerce/services/invoiceService.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      }, { action: 'select-tier-optimistic', dirty: ['header', 'page', 'drawer', 'hero'] });
      ...
      try {
        ...
        if (isRuntimeScopeActive(loadScope) && normalizeTierName(store.getState().commerce.selectedTier) === selectedTier) {
          store.update((draft) => {
            if (!isRuntimeScopeActive(loadScope)) return false;
            draft.commerce.selectedTier = previousTier;
          }, { action: 'select-tier-rollback', dirty: ['header', 'page', 'drawer', 'hero'] });
          schedule('header', 'page', 'drawer', 'hero');
        }
        notify(store, 'warning', 'تعذر تحديث الشريحة', error?.message || 'حاول مرة أخرى');
        return;
      }
      if (!isRuntimeScopeActive(loadScope) || normalizeTierName(store.getState().commerce.selectedTier) !== selectedTier) return false;

      const liveState = store.getState();
      const rebuilt = rebuildLoadedCompanyCatalog({ getState: () => ({ ...liveState, commerce: { ...liveState.commerce, catalog: { ...homeCatalog, productIndex: homeCatalog.productIndex, products: homeCatalog.products } }, runtime: liveState.runtime }) }, selectedTier);
      const refreshedCart = recalcCart(liveState.commerce.cart, rebuilt.productIndex);
      store.update((draft) => {
        if (!isRuntimeScopeActive(loadScope)) return false;
        draft.commerce.selectedTier = selectedTier;
