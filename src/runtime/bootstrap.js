...
function renderPage(state, nodes) {
  ...
  const showSearch = !['login', 'register'].includes(route.name);

  renderHeader(nodes.header, state);
  renderSearchBar(nodes.search, state, { show: showSearch, placeholder: route.name === 'company' ? 'ابحث داخل الشركة' : 'ابحث باسم المنتج أو الشركة' });
...
function applyShellVisibility(route, nodes) {
  ...
  nodes.search.classList.toggle('is-hidden', hideContent);
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'navigate-home') return navigate('home');
    if (action === 'go-companies') return navigate('companies');
    if (action === 'go-offers') return navigate('offers');
    if (action === 'go-tiers') return navigate('tiers');
    if (action === 'go-cart') return navigate('cart');
    if (action === 'go-checkout') return navigate('checkout');
    if (action === 'go-login') return navigate('login');
    if (action === 'go-register') return navigate('register');
    if (action === 'go-customers') return navigate('customers');
    if (action === 'go-invoices') return navigate('invoices');
    if (action === 'go-account') return navigate('account');
    if (action === 'go-flash') return navigate('offers');
    if (action === 'clear-search') { store.patch({ ui: { ...state.ui, search: '' } }); schedule('page', 'search', 'hero'); return; }
    if (action === 'toggle-account-menu') { store.patch({ ui: { ...state.ui, accountMenuOpen: !state.ui.accountMenuOpen } }); schedule('header'); return; }
    if (action === 'open-cart-drawer') { store.patch({ ui: { ...state.ui, drawerOpen: true } }); schedule('drawer'); return; }
    if (action === 'close-cart-drawer') { store.patch({ ui: { ...state.ui, drawerOpen: false } }); schedule('drawer'); return; }
    if (action === 'open-customer-modal') { store.patch({ ui: { ...state.ui, activeModal: 'customer' } }); schedule('modals'); return; }
    if (action === 'close-modal') { store.patch({ ui: { ...state.ui, activeModal: null } }); schedule('modals'); return; }
    ...
    if (action === 'submit-checkout') {
      ...
      if (next) schedule('header', 'page', 'drawer', 'modals');
      return;
    }
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.id === 'searchInput') {
      store.patch({ ui: { ...state.ui, search: target.value } }, { dirty: ['page', 'search', 'hero'] });
      appendBehaviorEvent('search.query', { query: target.value.slice(0, 64) });
      schedule('page', 'search', 'hero');
      return;
    }
    if (target.getAttribute('data-role') === 'product-qty') {
      const productId = target.getAttribute('data-product-id');
      const qty = Math.max(1, Number(target.value || 1));
      store.update((draft) => { draft.commerce.qtyPrefs[productId] = qty; }, { action: 'qty-update', dirty: ['page'] });
      return;
    }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'login') {
      ...
      try {
        ...
        await loadInvoicesIntoState(store, api);
        navigate('home');
        schedule('header', 'page', 'drawer', 'search', 'hero');
      } catch (error) {
        notify(store, 'error', error.message === 'INVALID_PASSWORD' ? 'كلمة المرور غير صحيحة' : 'المستخدم غير موجود', '');
      }
      return;
    }
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    ...
    search: () => renderSearchBar(nodes.search, store.getState(), { show: !['login', 'register'].includes(store.getState().app.route.name), placeholder: 'ابحث باسم المنتج أو الشركة' }),
  ...
  window.addEventListener('hashchange', () => {
    store.patch({ app: { ...store.getState().app, route: parseRoute(location.hash) } });
  });
  ...
  try {
    ...
    persistCart(cart);
    schedule('header', 'search', 'hero', 'page', 'footer', 'drawer', 'modals', 'toast');
  } catch (error) {
    console.error(error);
    ...
    scheduler.schedule('header', 'search', 'hero', 'page', 'footer', 'drawer', 'modals', 'toast');
  }
