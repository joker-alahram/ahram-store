...
import { persistSelectedTier } from '../services/pricingService.js';
import { addProductToCart, clearCart, computeTotals, hydrateCart, normalizeCartItems, persistCart, removeItem, toggleOfferInCart, updateQty, cartItemKey } from '../services/cartService.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'set-unit') {
      const productId = target.getAttribute('data-product-id');
      const unit = target.getAttribute('data-unit');
      store.update((draft) => { draft.commerce.unitPrefs[productId] = unit; }, { action: 'set-unit' });
    ...
    if (action === 'toggle-deal' || action === 'toggle-flash') {
      ...
      if (!offer) return;
      const result = toggleOfferInCart(state.commerce.cart, offer, action === 'toggle-deal' ? 'deal' : 'flash', tier?.tier_name || state.commerce.selectedTier || null);
    ...
    if (action === 'qty-up' || action === 'qty-down') {
      ...
      const item = state.commerce.cart.find((row) => cartItemKey(row) === key);
      if (!item) return;
    ...
    if (action === 'select-tier') {
      ...
      store.patch({
        commerce: {
          ...latest.commerce,
          catalog,
          selectedTier: nextTier?.tier_name || null,
          cart: latest.commerce.cart,
        },
      });
      persistCart(latest.commerce.cart);
...
async function loadInvoiceDetailsIntoState(store, api, invoiceId) {
  ...
  const [customerRows, repRows] = await Promise.all([
    ...
    (current.auth.session && String(current.auth.session.id) === String(order.user_id || order.sales_rep_id || '')) ? Promise.resolve([]) : api.get('v_auth_users', {
  ...
  const whatsappUrl = buildWhatsAppInvoice({
    ...
    items: Array.isArray(itemsRows) ? itemsRows.map((item) => ({
      ...
      productName: String(item.productName || item.title || item.name || item.product_id || '').trim(),
      unitCode: String(item.unit || item.unitCode || 'single').trim(),
      qty: Number(item.qty || 1),
      finalPrice: Number(item.price || item.finalPrice || 0),
      tierName: String(order.tier_name || '').trim() || null,
      snapshotTimestamp: item.snapshotTimestamp || item.created_at || new Date().toISOString(),
    })) : [],
...
export async function bootstrapApp() {
  ...
  try {
    ...
    const cart = normalizeCartItems(store.getState().commerce.cart.length ? store.getState().commerce.cart : hydrateCart(), productIndex, currentTier?.tier_name || selectedTier || null);
    store.patch({
      commerce: {
