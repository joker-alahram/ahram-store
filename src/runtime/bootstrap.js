...
function renderPage(state, nodes) {
  ...
  renderSearchBar(nodes.search, state, { routeName: route.name, show: showSearch });
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'clear-search') { store.patch({ ui: { ...state.ui, search: '' } }); clearTimeout(searchTypingTimer); schedule('header', 'theme', 'banner', 'search', 'hero', 'page'); return; }
    if (action === 'set-theme') {
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.id === 'searchInput') {
      ...
      searchTypingTimer = setTimeout(() => schedule('page', 'search'), 160);
      return;
    }
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    ...
    search: () => renderSearchBar(nodes.search, store.getState(), { routeName: store.getState().app.route.name, show: true }),
  ...
  function applyBodyFlags() {
    ...
    nodes.search.classList.toggle('is-hidden', route === 'search' || ['login', 'register', 'checkout'].includes(route));
