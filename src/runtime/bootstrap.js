...
import { renderCustomersPage, renderInvoicesPage, renderInvoiceDetailsPage, renderAccountPage } from '../pages/customerPages.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'navigate-home') {
      ...
      schedule('header', 'banner', 'search', 'hero', 'page', 'drawer', 'modals');
      return navigateAuthority(store, 'home', {}, { resetSearch: true });
    }
    ...
    if (action === 'go-account') {
      if (!state.auth.session) {
        closeTransientSurfaces(store, { keepDrawer: false });
        store.patch({ ui: { ...store.getState().ui, activeModal: 'login', accountMenuOpen: false } });
        schedule('modals', 'header');
        return;
      }
      return navigateAuthority(store, 'account');
    }
    ...
    if (action === 'close-cart-drawer') { store.patch({ ui: { ...state.ui, drawerOpen: false } }); schedule('drawer', 'header', 'page'); return; }
    ...
    if (action === 'select-tier') {
      ...
      try {
        const refreshedCatalog = await loadCatalog(api, nextTier?.tier_name || null);
        const canonicalTier = refreshedCatalog.tiers.find((tier) => tier.tier_name === (nextTier?.tier_name || null)) || refreshedCatalog.tiers.find((tier) => tier.is_default) || refreshedCatalog.tiers[0] || null;
        const refreshedPriceBook = buildPriceBook(Object.values(refreshedCatalog.productIndex || {}), refreshedCatalog.tiers, canonicalTier?.tier_name || null);
        const syncedCart = recalcCart(store.getState().commerce.cart, refreshedCatalog.productIndex, canonicalTier);
        store.patch({
          commerce: {
            ...store.getState().commerce,
            catalog: { ...refreshedCatalog, offers: refreshedCatalog.offers },
            selectedTier: canonicalTier?.tier_name || nextTier?.tier_name || null,
            priceBook: refreshedPriceBook,
            cart: syncedCart,
          },
          runtime: { ...store.getState().runtime, flashState: computeFlashState(refreshedCatalog.offers.flash) },
        });
        persistCart(syncedCart);
        persistSelectedTier(canonicalTier?.tier_name || nextTier?.tier_name || null);
        notify(store, 'success', 'تمت الشريحة', canonicalTier?.visible_label || nextTier?.visible_label || 'الشريحة الرئيسية');
        schedule('header', 'banner', 'page', 'drawer', 'hero');
      } catch (error) {
        console.error(error);
        notify(store, 'warning', 'تعذر تحديث الأسعار', 'تم الاحتفاظ بالأسعار الحالية مؤقتًا');
        schedule('header', 'banner', 'page', 'drawer', 'hero');
      }
      return;
    }
    ...
    if (action === 'open-invoice-details') {
      const invoiceId = target.getAttribute('data-invoice-id');
      closeTransientSurfaces(store, { keepDrawer: false });
      await loadInvoiceDetailsIntoState(store, api, invoiceId);
      return navigateAuthority(store, 'invoice-details', { invoiceId });
    }
...
async function loadInvoicesIntoState(store, api) {
  ...
  const customerMap = new Map((store.getState().commerce.customers || []).map((customer) => [String(customer.id), customer.name]));
  const invoices = (Array.isArray(rows) ? rows : []).map((row) => ({
    ...row,
    customer_name: row.customer_name || customerMap.get(String(row.customer_id)) || (session.userType === 'customer' ? session.name : ''),
  }));
  store.update((draft) => { draft.commerce.invoices = invoices; });
...
async function performCheckout(store, api, schedule) {
  ...
  try {
    ...
    checkoutBusy = false;
    return navigateAuthority(store, 'invoices');
  } catch (error) {
    console.error(error);
    notify(store, 'error', 'فشل إرسال الطلب', error?.message === 'ORDER_ITEMS_EMPTY' ? 'لا توجد عناصر صالحة في الطلب' : '');
    checkoutBusy = false;
    return false;
  }
}
...
async function loadInvoiceDetailsIntoState(store, api, invoiceId) {
  if (!invoiceId) {
    store.update((draft) => { draft.runtime.lastInvoiceDetail = null; }, { silent: true });
    return null;
  }

  const [ordersRows, itemsRows] = await Promise.all([
    api.get('orders', {
      select: 'id,order_number,invoice_number,created_at,total_amount,products_total,deals_total,flash_total,status,user_type,customer_id,user_id,sales_rep_id,tier_name',
      id: `eq.${invoiceId}`,
      limit: '1',
    }).catch(() => []),
    api.get('order_items', {
  ...
  const order = Array.isArray(ordersRows) ? ordersRows[0] : ordersRows;
  if (!order) {
    store.update((draft) => { draft.runtime.lastInvoiceDetail = null; }, { silent: true });
    return null;
  }
  ...
  const current = store.getState();
  const [customerRows, repRows] = await Promise.all([
    current.commerce.customers?.some((item) => String(item.id) === String(order.customer_id)) ? Promise.resolve([]) : api.get('customers', {
    ...
    (current.auth.session && String(current.auth.session.id) === String(order.user_id || order.sales_rep_id || '')) ? Promise.resolve([]) : api.get('sales_reps', {
      select: 'id,name,phone,region,username,password,created_at',
      id: `eq.${order.user_id || order.sales_rep_id || ''}`,
      limit: '1',
    }).catch(() => []),
  ]);
  const customer = (current.commerce.customers || []).find((item) => String(item.id) === String(order.customer_id))
    || (Array.isArray(customerRows) ? customerRows[0] : customerRows)
    || current.auth.selectedCustomer
    || (current.auth.session && String(current.auth.session.id) === String(order.customer_id) ? current.auth.session : null);
  const rep = (Array.isArray(repRows) ? repRows[0] : repRows)
    || current.auth.session
    || (current.commerce.customers || []).find((item) => String(item.id) === String(order.user_id));

  const whatsappUrl = buildWhatsAppInvoice({
    order,
    items: Array.isArray(itemsRows) ? itemsRows.map((item) => ({
      ...item,
      title: item.title || item.name || item.product_id,
      unitLabel: ({ carton: 'كرتونة', pack: 'دستة', half_pack: 'نصف دستة', piece: 'قطعة' }[String(item.unit || '').trim()] || 'قطعة'),
    })) : [],
    session: rep || current.auth.session,
    customer,
    tierLabel: order.tier_name || getSelectedTier(current)?.visible_label || getSelectedTier(current)?.tier_name,
    supportWhatsapp: api.config.supportWhatsapp,
  });
  ...
  const detail = {
    ...
    repName: rep?.name || rep?.username || '',
    whatsappMessage: decodeURIComponent(String(whatsappUrl).split('text=')[1] || ''),
    whatsappUrl,
  };

  store.update((draft) => { draft.runtime.lastInvoiceDetail = detail; }, { silent: true });
  return detail;
}
...
export async function bootstrapApp() {
  ...
  const schedule = (...keys) => scheduler.schedule(...keys);
  ...
  function renderContent() {
    ...
    else if (route === 'invoice-details') html = renderInvoiceDetailsPage(state);
  ...
  window.addEventListener('hashchange', async () => {
    ...
    store.patch({
      app: { ...current.app, route },
      ui: { ...current.ui, accountMenuOpen: false, activeModal: null, drawerOpen: false },
    });
    if (route.name === 'invoice-details' && route.params?.invoiceId) {
      await loadInvoiceDetailsIntoState(store, api, route.params.invoiceId);
    } else {
      store.update((draft) => { draft.runtime.lastInvoiceDetail = null; }, { silent: true });
    }
  });
  ...
  if (store.getState().app.route.name === 'invoice-details' && store.getState().app.route.params?.invoiceId) {
    await loadInvoiceDetailsIntoState(store, api, store.getState().app.route.params.invoiceId);
  }
