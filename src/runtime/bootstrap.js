  ...
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'toggle-deal' || action === 'toggle-flash') {
      ...
      if (action === 'toggle-flash') {
        ...
        if (runtimeStatus !== 'active' || offer.is_checkout_available === false) {
...
export async function bootstrapApp() {
  ...
  try {
    ...
    const liveCatalog = await loadHomeCatalog(api, store.getState().commerce.selectedTier || null, { includeTop: false }).catch(() => null);
    ...
    if (catalogHasMeaningfulData(catalog)) {
      saveJSON(storageKeys.catalog, compactCatalogForStorage(catalog));
    }
    scheduler.schedule('page', 'toast');
    markPerf('boot:catalog:success');
  } catch (error) {
    console.error(error);
    const cachedCatalog = loadJSON(storageKeys.catalog, null);
    ...
    if (catalogHasMeaningfulData(fallbackCatalog)) {
      saveJSON(storageKeys.catalog, compactCatalogForStorage(fallbackCatalog));
    }
    scheduler.schedule('page', 'toast');
    markPerf('boot:catalog:fallback');
  }
