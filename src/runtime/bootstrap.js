...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-tier') {
      ...
      const resolvedTierName = nextTier?.tier_name || null;
      store.update((draft) => {
        draft.commerce.selectedTier = resolvedTierName;
      }, { action: 'select-tier', dirty: ['header', 'page', 'drawer', 'hero'] });
      await reloadCatalogForTier(store, api, resolvedTierName);
...
async function reloadCatalogForTier(store, api, selectedTierName = null) {
  const current = store.getState();
  const catalog = await loadCatalog(api, selectedTierName || current.commerce.selectedTier || null);
  const selectedTier = selectedTierName || current.commerce.selectedTier || catalog.tiers.find((tier) => tier.is_default)?.tier_name || catalog.tiers[0]?.tier_name || 'base';
  ...
  const sourceCart = current.commerce.cart.length ? current.commerce.cart : hydrateCart();
  const cart = recalcCart(sourceCart, catalog.productIndex, currentTier);
  store.patch({
    commerce: {
      ...current.commerce,
      catalog: { ...catalog, offers: catalog.offers },
    ...
    runtime: {
      ...current.runtime,
      flashState: computeFlashState(catalog.offers.flash),
    },
  });
  persistSelectedTier(currentTier?.tier_name || selectedTier || null);
  persistCart(cart);
  return { catalog, currentTier, selectedTier: currentTier?.tier_name || selectedTier || null, cart };
}
...
export async function bootstrapApp() {
  ...
  try {
    const { catalog, currentTier, selectedTier, cart } = await reloadCatalogForTier(store, api, store.getState().commerce.selectedTier || null);
    store.patch({
