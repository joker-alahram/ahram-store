...
async function performCheckout(store, api, schedule) {
  ...
  try {
    ...
    if (!isRuntimeScopeActive(scope)) {
      try { if (waWindow && !waWindow.closed) waWindow.close(); } catch (error) { void error; }
      return false;
    }
    ...
    if (waWindow && whatsappUrl) {
      try {
        ...
        console.warn('WhatsApp redirect failed, falling back to manual delivery', deliveryError);
    ...
    if (whatsappDelivered) {
      store.update((draft) => {
        ...
        if (targetInvoice) {
          targetInvoice.whatsapp_delivery_status = 'sent';
          targetInvoice.whatsapp_delivery_blocked = false;
        }
      });
    } else {
      store.update((draft) => {
        const targetInvoice = (draft.commerce.invoices || []).find((item) => String(item.id) === String(invoice.id));
        if (targetInvoice) {
          targetInvoice.whatsapp_delivery_status = 'blocked';
          targetInvoice.whatsapp_delivery_blocked = true;
        }
      });
      notify(store, 'warning', 'تم حفظ الفاتورة، وفتح واتساب متاح من فواتيري', `فاتورة ${invoice.order_number || invoice.invoice_number || invoice.id}`);
    }
