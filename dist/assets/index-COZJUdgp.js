...
`)}}}function $t(t,a){return{daily(){return t.getState().data.dailyDeals||[]},flash(){return t.getState().data.flashOffers||[]},recordOpen(e,r){a("company_open",{actor_id:t.getState().auth.user?.id,actor_type:t.getState().auth.user?.user_type||t.getState().auth.role,session_id:t.getState().auth.session?.session_id,payload:{kind:e,id:r}})}}}function Et(t){return{summary(){const a=t.getState(),e=a.data.orders||[],r=a.data.customers||[],n=a.data.products||[],o=e.reduce((l,d)=>l+Number(d.grand_total||0),0),c=e.filter(l=>l.status==="submitted").length,s=e.filter(l=>l.status==="shipped"||l.status==="delivered").length;return{revenue:o,orders:e.length,customers:r.length,products:n.length,submitted:c,shipped:s}},repPerformance(){const a=t.getState();return a.data.authUsers.filter(r=>r.user_type==="sales_rep").map(r=>({id:r.id,name:r.name,orders:a.data.orders.filter(n=>n.sales_rep_id===r.id).length,customers:a.data.customers.filter(n=>n.sales_rep_id===r.id||n.created_by_rep_id===r.id).length}))}}}const i={q(t,a=document){return a.querySelector(t)},qa(t,a=document){return Array.from(a.querySelectorAll(t))},html(t,a){t&&(t.innerHTML=a)},text(t,a){t&&(t.textContent=a)},escape(t){return String(t??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}};function F(t){return t.auth?.role||h.GUEST}function qt(t){return[{company_id:"all",company_name:"جميع الشركات",company_logo:null,color:"#999"},...t.data.companies||[]]}function X(t){return t.data.tiers.find(a=>a.tier_name===t.ui.selectedTier)||t.data.tiers.find(a=>a.is_default)||t.data.tiers[0]||null}function wt(t){const a=(t.ui.search||"").trim().toLowerCase(),e=t.ui.selectedTier,r=t.ui.selectedCompany;let n=(t.data.products||[]).filter(o=>o.runtime_healthy!==!1);return e&&e!=="all"&&(n=n.filter(o=>o.tier_name===e)),r&&r!=="all"&&(n=n.filter(o=>o.company_id===r)),a&&(n=n.filter(o=>[o.product_name,o.company_name,o.product_id,o.category].some(c=>String(c||"").toLowerCase().includes(a)))),n.slice().sort((o,c)=>String(o.company_name).localeCompare(String(c.company_name),"ar")||String(o.product_name).localeCompare(String(c.product_name),"ar"))}function tt(t){const a=z(wt(t));return{products:a,companies:qt(t),tiers:t.data.tiers||[],counts:{products:a.length,companies:(t.data.companies||[]).length,orders:(t.data.orders||[]).length,customers:(t.data.customers||[]).length}}}function kt(t){return t.cart.items||[]}function B(t){const a=kt(t),e=a.reduce((n,o)=>n+(o.unit_price_snapshot||o.unit_price||0)*o.quantity,0),r=a.reduce((n,o)=>n+o.quantity,0);return{subtotal:e,quantity:r,grand_total:e,discount_total:0}}function Dt(t){const a=F(t),e=t.auth.user;return a===h.ADMIN?t.data.customers||[]:a===h.SALES_REP?(t.data.customers||[]).filter(r=>r.sales_rep_id===e?.id||r.created_by_rep_id===e?.id):a===h.CUSTOMER?(t.data.customers||[]).filter(r=>r.id===e?.id||r.username===e?.username):[]}const It=new Intl.DateTimeFormat("ar-EG",{dateStyle:"medium",timeStyle:"short"});function G(t){try{return It.format(new Date(t||Date.now()))}catch{return""}}function Ot(t){const a=[{route:"home",label:"الرئيسية",icon:"⌂"},{route:"offers",label:"العروض",icon:"✦"},{route:"cart",label:"السلة",icon:"🛒"}];return t===h.GUEST?[...a,{route:"login",label:"الدخول",icon:"⇢"}]:t===h.CUSTOMER?[...a,{route:"orders",label:"طلباتي",icon:"▤"},{route:"account",label:"الحساب",icon:"◔"}]:t===h.SALES_REP?[...a,{route:"customers",label:"العملاء",icon:"◫"},{route:"orders",label:"الطلبات",icon:"▤"},{route:"analytics",label:"التحليلات",icon:"⟡"}]:[...a,{route:"customers",label:"العملاء",icon:"◫"},{route:"reps",label:"المناديب",icon:"⚇"},{route:"orders",label:"الطلبات",icon:"▤"},{route:"analytics",label:"التحليلات",icon:"⟡"},{route:"settings",label:"الإعدادات",icon:"⚙"}]}function D(t,a=""){return`<span class="badge ${a}">${i.escape(t)}</span>`}function et(t,a=!1){const e=t.color?`style="--chip-accent:${t.color}"`:"";return`<button class="company-chip ${a?"is-active":""}" ${e} type="button" data-action="select-company" data-company-id="${i.escape(t.company_id)}"><span>${i.escape((t.company_name||"X").slice(0,1))}</span><strong>${i.escape(t.company_name)}</strong></button>`}function Nt(t,a=!1){return`<button class="tier-chip ${a?"is-active":""}" type="button" data-action="select-tier" data-tier-name="${i.escape(t.tier_name)}">${i.escape(t.display_name||t.tier_name)}</button>`}function At(t,a){const e=t.units?.[0];a.cart.items.some(d=>d.kind==="product"&&d.product_id===t.product_id);const r=e?Math.max(0,e.available_qty-e.reserved_qty):0,n=e?.unit_code||"carton",o=e?.final_price||0,c=`${t.product_id}:${n}:${e?.tier_name||"base"}`,l=a.cart.items.find(d=>d.key===c||d.kind==="product"&&d.product_id===t.product_id)?.quantity||1;return`
    <article class="product-card" data-product-id="${i.escape(t.product_id)}">
      ...
      <div class="product-card__meta">
        ${D(pt(n))}
        ${D(`المتاح ${r}`,r>0?"badge--success":"badge--danger")}
        ${D(i.escape(t.tier_name))}
      </div>
  ...
  `}function Y(t,a){const e=t.can_buy!==!1;return`
    <article class="offer-card offer-card--${i.escape(a)}">
      <div class="offer-card__body">
        <div class="offer-card__meta">${D(a==="daily"?"يومي":"فلاش",a==="daily"?"badge--info":"badge--warning")} ${D(e?"متاح":"مغلق",e?"badge--success":"badge--danger")}</div>
  ...
  `}function Ct(t,a){const e=a.data.customers.find(r=>r.id===t.customer_id);return`
    <article class="order-card">
      <div class="order-card__top">
        ...
        ${D(t.status,t.status==="cancelled"?"badge--danger":"badge--info")}
      </div>
  ...
  `}function at(t){return`
    <article class="rep-card">
      <div class="rep-card__top">
        ...
        ${D(t.is_blocked?"موقوف":"نشط",t.is_blocked?"badge--danger":"badge--success")}
      </div>
      <div class="rep-card__meta">${i.escape(t.login_code||"")}</div>
    </article>
  `}function v(t,a){return`
    <article class="summary-card">
      <span>${i.escape(t)}</span>
      <strong>${i.escape(String(a))}</strong>
    </article>
  `}function Rt(t){const a=tt(t),e=X(t),r=a.products.slice(0,t.ui.visibleCount),n=a.products.length;return`
    <section class="hero">
      <div class="hero__copy">
        <div class="hero__eyebrow">${D("Mobile-first","badge--info")} ${D("Render-only runtime","badge--success")}</div>
        ...
        <div class="hero__stats">
          ${v("المنتجات",a.counts.products)}
          ${v("الشركات",a.counts.companies)}
          ${v("الطلبات",a.counts.orders)}
          ${v("الشرائح",t.data.tiers.length)}
        </div>
      </div>
    ...
    <section class="section">
      ...
      <div class="chips chips--companies">
        ${a.companies.map(o=>et(o,t.ui.selectedCompany===o.company_id)).join("")}
      </div>
    </section>
    ...
    <section class="section">
      ...
      <div class="product-grid">
        ${r.map(o=>At(o,t)).join("")||'<div class="empty">لا توجد منتجات مطابقة.</div>'}
      </div>
    </section>
  `}function Ut(t){return`
    <section class="section">
      ...
      <div class="offer-grid">
        ${(t.data.dailyDeals||[]).map(a=>Y(a,"daily")).join("")}
      </div>
    </section>
    <section class="section">
      <div class="section__head">
      ...
      <div class="offer-grid">
        ${(t.data.flashOffers||[]).map(a=>Y(a,"flash")).join("")}
      </div>
    </section>
        ...
        ${(t.data.tiers||[]).map(a=>`
          <article class="tier-card ${t.ui.selectedTier===a.tier_name?"is-active":""}">
            <div class="tier-card__top">
              ...
              ${D(a.is_default?"افتراضية":"إضافية",a.is_default?"badge--success":"badge--info")}
            </div>
  ...
  `}function Mt(t){const a=B(t),e=t.data.customers.find(r=>r.id===t.cart.customer_id);return`
    <section class="section">
      <div class="section__head">
  ...
  `}function Pt(t){return`
    <section class="section">
      ...
      <div class="order-grid">
        ${(t.data.orders||[]).map(e=>Ct(e,t)).join("")||'<div class="empty">لا توجد طلبات.</div>'}
      </div>
    </section>
  `}function xt(t){return`
    <section class="section">
      ...
      <div class="customer-grid">
        ${Dt(t).map(e=>Tt(e,t.cart.customer_id===e.id)).join("")||'<div class="empty">لا يوجد عملاء.</div>'}
      </div>
    </section>
  `}function Ft(t){return`
    <section class="section">
      ...
      <div class="rep-grid">
        ${(t.data.authUsers||[]).filter(e=>e.user_type===h.SALES_REP).map(e=>at(e)).join("")}
      </div>
    </section>
  `}function Bt(t,a){const e=a.analytics.summary();return`
    <section class="section">
      <div class="section__head">
        <div><h2>التحليلات</h2><p>لوحة قراءات مشتقة من سجل التشغيل.</p></div>
      </div>
      <div class="summary-grid">
        ${v("الإيراد",`${w(e.revenue)} ج.م`)}
        ${v("الطلبات",e.orders)}
        ${v("العملاء",e.customers)}
        ${v("المنتجات",e.products)}
        ${v("submitted",e.submitted)}
        ${v("shipped/delivered",e.shipped)}
      </div>
      <div class="section section--compact">
        <div class="section__head"><div><h3>أداء المناديب</h3></div></div>
        <div class="rep-grid">
          ${a.analytics.repPerformance().map(r=>at(r)).join("")}
        </div>
      </div>
    </section>
  `}function Gt(t){const a=t.auth.role!==h.GUEST;return`
    <section class="section">
      <div class="section__head">
      ...
      ${a?`
        <div class="summary-grid">
          ${v("الدور",t.auth.role)}
          ${v("المستخدم",t.auth.user?.name||"")}
          ${v("انتهاء الجلسة",G(t.auth.session?.expires_at))}
        </div>
        <div class="card-actions">
  ...
  `}function jt(t,a){const e=B(t),r=t.data.customers.find(n=>n.id===t.cart.customer_id);return`
    <section class="section">
      <div class="section__head">
  ...
  `}function Ht(t){return`
    <section class="section">
      ...
      <div class="summary-grid">
        ${v("Theme",t.ui.theme)}
        ${v("WhatsApp",t.app.config.supportWhatsapp)}
        ${v("Source",t.runtime.source)}
        ${v("Sync",G(t.runtime.lastSyncAt))}
      </div>
      <div class="card-actions">
        <button type="button" class="btn btn--ghost" data-action="toggle-theme">تبديل الثيم</button>
      </div>
    </section>
  `}function Kt(t,a){switch(t.ui.route){case"offers":return Ut(t);case"tiers":return Lt(t);case"cart":return Mt(t);case"orders":return Pt(t);case"customers":return xt(t);case"reps":return Ft(t);case"analytics":return Bt(t,a);case"login":case"account":return Gt(t);case"checkout":return jt(t);case"settings":return Ht(t);case"home":default:return Rt(t)}}function Vt(t,a){if(!t.ui.modal)return"";const e=t.ui.modal;if(e.type==="product"){const r=e.product;return`
      <section class="modal is-open" data-modal="product">
        <div class="modal__sheet">
          ...
          <div class="modal__body">
            <div class="summary-grid">
              ${v("الشركة",r.company_name||"")}
              ${v("الشريحة",r.tier_name||"")}
              ${v("السعر",`${w(r.units?.[0]?.final_price||0)} ج.م`)}
              ${v("المتاح",r.units?.[0]?Math.max(0,r.units[0].available_qty-r.units[0].reserved_qty):0)}
            </div>
      ...
      <section class="modal is-open" data-modal="order">
        <div class="modal__sheet">
          ...
          <div class="modal__body">
            <div class="summary-grid">
              ${v("الحالة",r.status)}
              ${v("الإجمالي",`${w(r.grand_total)} ج.م`)}
              ${v("العميل",r.customer_id)}
              ${v("المندوب",r.sales_rep_id||"-")}
            </div>
    ...
    `}return""}function Yt(t){return`<div class="toast-stack">${t.data.uiEvents.slice(-3).map((e,r)=>`
  ...
  `).join("")}</div>`}function Qt(t,a){return`
    <nav class="bottom-nav" aria-label="main navigation">
      ${Ot(F(t)).map(r=>`<button type="button" class="bottom-nav__item ${t.ui.route===r.route?"is-active":""}" data-action="go" data-route="${i.escape(r.route)}"><span>${i.escape(r.icon)}</span><strong>${i.escape(r.label)}</strong></button>`).join("")}
    </nav>
  `}function Wt(t,a){const e=tt(t),r=F(t),n=X(t);return`
    <div class="app-shell theme-${i.escape(t.ui.theme||"dark")}" data-role="${i.escape(r)}">
      <header class="topbar">
        ...
        <div class="tier-strip" aria-label="tiers">
          ${t.data.tiers.map(o=>Nt(o,n?.tier_name===o.tier_name)).join("")}
        </div>

        <div class="company-strip">
          ${e.companies.map(o=>et(o,t.ui.selectedCompany===o.company_id)).join("")}
        </div>
      </header>
  ...
  `}const Q="ahram-b2b-runtime:theme";async function Jt(){const t=ot(),a=lt(t),e=dt(t),r=document.getElementById("app"),n=(y,u={})=>{const b=e.getState(),g={event_type:y,actor_id:u.actor_id||b.auth.user?.id||null,actor_type:u.actor_type||b.auth.user?.user_type||b.auth.role,session_id:u.session_id||b.auth.session?.session_id||null,timestamp:new Date().toISOString(),payload:u.payload||{},source:"ui",runtime_context:{route:b.ui.route,theme:b.ui.theme,cart_count:b.cart.items.length}};return e.setState(_=>({data:{uiEvents:[..._.data.uiEvents,g].slice(-200)}}),{action:y}),g},o=bt(e,n),c=ft(e,n),s=gt(e,n),l=St(e,s,n),d=$t(e,n),p=Et(e);async function S(){const y=await a.bootstrap(),u={...y,products:mt(y.products||[]),companies:yt(y.products||[]),productsIndex:z(y.products||[])};e.setState(_=>({app:{ready:!0,online:!0,config:t},data:{..._.data,...u},ui:{..._.ui,theme:_.ui.theme||J(Q,t.theme||"dark"),route:P().path}}),{action:"hydrate"});const g=e.getState().auth.session;g?.expires_at&&new Date(g.expires_at).getTime()<Date.now()&&o.logout(),s.reconcilePricing()}function m(){const y=e.getState().ui.theme||"dark";document.documentElement.dataset.theme=y,document.body.dataset.theme=y,Z(Q,y)}function f(y){e.setState(u=>({ui:{...u.ui,modal:y}}),{action:"open_modal"})}function U(){e.setState(y=>({ui:{...y.ui,modal:null}}),{action:"close_modal"})}function q(y){e.setState(u=>({ui:{...u.ui,route:y}}),{action:"route"})}function j(){r&&(r.innerHTML=Wt(e.getState(),{analytics:p}),m())}function rt(y){const u=e.getState(),b=u.data.productsIndex?.find(g=>g.product_id===y);return b||u.data.products.find(g=>g.product_id===y)||null}function nt(y,u){const b=e.getState();return(y==="daily"?b.data.dailyDeals:b.data.flashOffers).find(_=>String(_.id)===String(u))||null}function L(y){const u=e.getState().auth.role||h.GUEST;return y==="login"&&u!==h.GUEST?"account":y==="customers"&&![h.ADMIN,h.SALES_REP].includes(u)||y==="reps"&&u!==h.ADMIN||y==="analytics"&&![h.ADMIN,h.SALES_REP].includes(u)?u===h.GUEST?"login":"home":y==="checkout"&&u===h.GUEST?"login":y}window.addEventListener("hashchange",()=>{const{path:y}=P(),u=L(y);u!==y&&N(u),q(u)}),document.addEventListener("click",y=>{const u=y.target.closest("[data-action]");if(!u)return;const b=u.dataset.action,g=e.getState();try{if(b==="go"){const _=L(u.dataset.route);N(_),q(_);return}if(b==="select-tier"){const _=u.dataset.tierName;e.setState($=>({ui:{...$.ui,selectedTier:$.ui.selectedTier===_?"base":_,visibleCount:12}}),{action:"select_tier"}),s.reconcilePricing();return}if(b==="select-company"){const _=u.dataset.companyId;e.setState($=>({ui:{...$.ui,selectedCompany:$.ui.selectedCompany===_?"all":_,visibleCount:12}}),{action:"select_company"}),n("company_open",{payload:{company_id:_}});return}if(b==="load-more"){e.setState(_=>({ui:{..._.ui,visibleCount:_.ui.visibleCount+12}}),{action:"load_more"});return}if(b==="clear-search"){e.setState(_=>({ui:{..._.ui,search:"",visibleCount:12}}),{action:"clear_search"});return}if(b==="toggle-theme"){const _=g.ui.theme==="dark"?"light":"dark";e.setState($=>({ui:{...$.ui,theme:_}}),{action:"toggle_theme"}),m();return}if(b==="logout"){o.logout(),N("home"),q("home");return}if(b==="open-product"){const _=rt(u.dataset.productId);_&&f({type:"product",product:_});return}if(b==="close-modal"){U();return}if(b==="add-product"){const _=u.closest("[data-product-id]"),$=u.dataset.productId,I=u.dataset.unitCode,A=u.dataset.tierName,C=g.data.products.find(T=>T.product_id===$&&T.unit_code===I&&T.tier_name===A)||g.data.products.find(T=>T.product_id===$);if(!C)return;const M=_?.querySelector('[data-role="cart-qty"]'),st=Math.max(1,Number(M?.value||1));s.addProduct(C,st);return}if(b==="set-cart-qty"){const _=u.dataset.productKey,$=Number(u.dataset.delta||0),I=g.cart.items.find(A=>A.key===_);if(I)s.setQuantity(_,I.quantity+$);else{const A=u.closest("[data-product-id]");if(A){const C=A.querySelector('[data-role="cart-qty"]'),M=Math.max(1,Number(C?.value||1)+$);C&&(C.value=String(M))}}return}if(b==="remove-cart-item"){s.remove(u.dataset.productKey);return}if(b==="clear-cart"){s.clear();return}if(b==="add-offer"){const _=u.dataset.offerKind,$=nt(_,u.dataset.offerId);$&&s.addOffer($,_);return}if(b==="open-order"){const _=l.get(u.dataset.orderId);_&&f({type:"order",order:_});return}if(b==="send-whatsapp"){const _=l.get(u.dataset.orderId);if(!_)return;const $=l.whatsappMessage(_),I=`https://wa.me/${t.supportWhatsapp}?text=${encodeURIComponent($)}`;window.open(I,"_blank","noopener,noreferrer"),n("whatsapp_send",{payload:{order_id:_.id}});return}if(b==="submit-checkout"){const _=l.createFromCart();f({type:"order",order:_}),N("orders"),q("orders");return}if(b==="select-customer"){e.setState(_=>({cart:{..._.cart,customer_id:u.dataset.customerId}}),{action:"select_customer"});return}}catch(_){console.error(_),n("representative_action",{payload:{error:_.code||_.message}}),e.setState($=>({runtime:{...$.runtime,diagnostics:{...$.runtime.diagnostics,hasConsoleError:!0}}}),{action:"error"})}}),document.addEventListener("input",y=>{const u=y.target;if(!(u instanceof HTMLInputElement))return;if(u.dataset.action==="search"){e.setState(g=>({ui:{...g.ui,search:u.value,visibleCount:12}}),{action:"search"}),n("search",{payload:{query:u.value}});return}if(u.dataset.role==="cart-qty"){const g=u.dataset.productKey,_=Math.max(1,Number(u.value||1));e.getState().cart.items.find(I=>I.key===g)?s.setQuantity(g,_):u.value=String(_);return}}),document.addEventListener("submit",y=>{const u=y.target;if(!(u instanceof HTMLFormElement))return;const b=u.dataset.form;y.preventDefault();try{if(b==="login"){const g=new FormData(u);o.login(String(g.get("identity")||""),String(g.get("password")||"")),N("home"),q("home");return}if(b==="customer-create"){const g=new FormData(u),_=c.create({name:String(g.get("name")||"").trim(),phone:String(g.get("phone")||"").trim(),location:String(g.get("location")||"").trim()});u.reset(),_&&e.setState($=>({cart:{...$.cart,customer_id:_.id}}),{action:"select_customer"});return}}catch(g){console.error(g),n("representative_action",{payload:{error:g.code||g.message}})}}),e.subscribe(()=>{j()}),await S();const H=L(P().path);N(H),q(H),j(),window.__B2B_APP__={getState:()=>e.getState(),services:{auth:o,customers:c,cart:s,orders:l,offers:d,analytics:p},navigate:N,eventLog:n}}Jt();
