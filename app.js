...
function rowMatchesTier(row, selectedTier) {
  ...
  return String(row.tier_name || '').trim() === String(selectedTier).trim();
}

function selectRuntimeRow(rows = [], selectedTier = null) {
  const list = Array.isArray(rows) ? rows.filter(Boolean) : [];
  if (!list.length) return null;
  const exact = selectedTier ? list.find((row) => rowMatchesTier(row, selectedTier)) : null;
  if (exact) return exact;
  const baseRow = list.find((row) => String(row.tier_name || '').trim().toLowerCase() === 'base');
  return baseRow || list[0] || null;
}
...
function refreshProductRuntimePricing(selectedTier = tierName()) {
  ...
  state.products = (state.products || []).map((product) => {
    if (!Array.isArray(product.unitRows) || !product.unitRows.length) return product;
    const { unitPricing, unitCodes } = buildUnitPricingFromRows(product.unitRows, activeTier);
    const next = {
      ...product,
      unitPricing,
      unitCodes: unitCodes.length ? unitCodes : product.unitCodes || [],
      has_carton: unitCodes.includes('carton') || product.has_carton,
      has_pack: unitCodes.includes('pack') || product.has_pack,
      has_unit: unitCodes.includes('piece') || product.has_unit,
    };
    return next;
  });
}
...
function openProductModal(productId) {
  ...
  const unitText = labelForUnit(unit);
...
function resolveProductPrice(product, unit) {
  ...
  if (pricing && Number.isFinite(Number(pricing.finalPrice)) && Number(pricing.finalPrice) > 0) return Number(pricing.finalPrice);

  const productId = String(product?.product_id || '').trim();
  const normalizedUnit = normalizeUnitCode(unit);
  const tierMapPrice = Number(state.tierPrices?.[normalizedUnit]?.get(productId) || 0);
...
function availableUnits(product) {
  ...
  const fromRows = Array.isArray(product?.unitRows) ? product.unitRows.map((row) => runtimeRowUnitCode(row)) : [];
  const fromUnits = Array.isArray(product?.unitCodes) ? product.unitCodes : [];
  const fallback = [];
  if (product?.has_carton || Number(product?.carton_price) > 0) fallback.push('carton');
  if (product?.has_pack || Number(product?.pack_price) > 0) fallback.push('pack');
  if (product?.has_halfpack || Number(product?.half_pack_price) > 0 || Number(product?.halfpack_price) > 0) fallback.push('halfpack');
  if (product?.has_unit || Number(product?.piece_price) > 0) fallback.push('piece');
  return sortUnitCodes([...fromPricing, ...fromUnits, ...fromRows, ...fallback]);
}
...
function unitCanRenderInCard(product, unit) {
  ...
  if (pricing?.allowSale === false) return false;
  const price = resolveProductPrice(product, normalized);
  return Number.isFinite(Number(price)) && Number(price) > 0;
}

function visibleProductCardUnits(product) {
  return availableUnits(product).filter((unit) => unitCanRenderInCard(product, unit));
}

function currentVisibleUnitForProduct(product) {
  const visible = visibleProductCardUnits(product);
  const saved = normalizeUnitCode(state.unitPrefs[product.product_id]);
  if (saved && visible.includes(saved)) return saved;
  const current = normalizeUnitCode(currentUnitForProduct(product));
  if (current && visible.includes(current)) return current;
  return visible[0] || currentUnitForProduct(product);
}
...
function normalizeProductsFromTables(runtimeRows = [], rawProducts = [], cartonRows = [], packRows = []) {
  const currentTier = tierName();
  const hasRuntimeRows = Array.isArray(runtimeRows) && runtimeRows.some((row) => row && row.unit_code);

  if (hasRuntimeRows) {
    const rawById = new Map((rawProducts || []).map((row) => [String(row.product_id || '').trim(), row]));
    ...
    for (const row of runtimeRows || []) {
      ...
      const raw = rawById.get(productId) || {};
      const current = grouped.get(productId) || {
        product_id: productId,
        product_name: raw.product_name || row.product_name || '',
        company_id: raw.company_id || row.company_id || '',
        category: raw.category || row.category || '',
        product_image: raw.product_image || row.product_image || '',
        status: raw.status || row.status || 'active',
        visible: raw.visible !== false && row.allow_sale !== false,
        type: raw.type || row.type || 'normal',
        allow_discount: raw.allow_discount ?? row.apply_discount ?? true,
        discount_carton: raw.discount_carton ?? true,
        discount_pack: raw.discount_pack ?? true,
        discount_unit: raw.discount_unit ?? true,
        has_carton: Boolean(raw.has_carton),
        has_pack: Boolean(raw.has_pack),
        has_unit: Boolean(raw.has_unit),
        rows: [],
        unitRows: [],
        unitCodes: new Set(),
        unitPricing: {},
      ...
      current.unitRows.push(row);
      current.unitCodes.add(unitCode);
      if (unitCode === 'carton') current.has_carton = true;
      if (unitCode === 'pack') current.has_pack = true;
      if (unitCode === 'piece') current.has_unit = true;
      if (row.allow_sale !== false) current.visible = true;
      grouped.set(productId, current);
    }

    const products = [...grouped.values()].map((group) => {
      const unitCodes = sortUnitCodes([...group.unitCodes]);
      const unitRowsByCode = new Map();
      for (const row of group.unitRows) {
        const code = runtimeRowUnitCode(row);
        if (!code) continue;
        if (!unitRowsByCode.has(code)) unitRowsByCode.set(code, []);
        unitRowsByCode.get(code).push(row);
      }

      const unitPricing = {};
      for (const unitCode of unitCodes) {
        const chosenRow = selectRuntimeRow(unitRowsByCode.get(unitCode) || [], currentTier);
        if (!chosenRow) continue;
        unitPricing[unitCode] = {
          basePrice: Number(chosenRow.base_price || 0),
          finalPrice: Number(chosenRow.final_price || chosenRow.base_price || 0),
      ...
      return {
        ...
        unitCodes,
        unitPricing,
        has_carton: group.has_carton || unitCodes.includes('carton'),
        has_pack: group.has_pack || unitCodes.includes('pack'),
        has_unit: group.has_unit || unitCodes.includes('piece'),
        carton_price: unitPricing.carton?.basePrice ?? Number(group.carton_price || 0),
        pack_price: unitPricing.pack?.basePrice ?? Number(group.pack_price || 0),
        piece_price: unitPricing.piece?.basePrice ?? Number(group.piece_price || 0),
      };
    });
  ...
  return sortProducts((rawProducts || []).map((product) => {
    const productId = String(product.product_id || '').trim();
    ...
    const piecePrice = Number(product.piece_price ?? product.unit_price ?? 0);
    ...
    return {
      ...
      has_unit: product.has_unit ?? piecePrice > 0,
      ...
      unitCodes: sortUnitCodes([
        ...(product.has_carton || cartonPrice > 0 ? ['carton'] : []),
        ...(product.has_pack || packPrice > 0 ? ['pack'] : []),
        ...(product.has_unit || piecePrice > 0 ? ['piece'] : []),
      ]),
      unitPricing: {},
      unitRows: [],
    };
...
async function loadData() {
  try {
    ...
    const [
      ...
      fetchCollection('v_products_full', { select: 'product_id,product_name,company_id,category,unit_code,tier_name,allow_sale,apply_discount,base_price,final_price', order: 'product_id.asc,unit_code.asc,tier_name.asc' }, []),
...
async function getLiveCheckoutSnapshot() {
  ...
  const [freshProducts, freshTier] = await Promise.all([
    ids.length ? apiGet('v_products_full', {
