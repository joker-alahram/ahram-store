...
async function syncInvoiceSequence() {
  ...
  safeStorageSet(INVOICE_COUNTER_STORAGE, String(nextValue));
  return nextValue;
}
...
async function reserveInvoiceNumber() {
  ...
  safeStorageSet(INVOICE_COUNTER_STORAGE, String(safeCurrent + 1));
  return safeCurrent;
}

let behaviorPersistTimer = null;

function persistBehaviorEvents() {
  clearTimeout(behaviorPersistTimer);
  behaviorPersistTimer = setTimeout(() => {
    saveJSON(STORAGE.behavior, state.behaviorEvents.slice(-300));
  }, 250);
}
...
function compactProductCardHtml(product, options = {}) {
  ...
  const priceText = displayPriceText(product, unit);
  const topBadge = badge || (currentCartCompanyIds().has(String(product.company_id)) ? 'مناسب لسلتك' : 'موصى به');
  ...
  return `
    <article class="product-tile" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body compact">
        <div class="badge-row">
          <span class="badge">${escapeHtml(topBadge)}</span>
        </div>
...
function persistSelectedCustomer() {
  if (state.selectedCustomer) {
    ...
    safeStorageRemove('b2b_selected_customer');
  }
}
...
function updateHeader() {
  ...
  if (els.cartLabel) els.cartLabel.textContent = 'إجمالي المشتريات اضغط لأتمام الشراء';
...
function updateFlashHeader() {
  ...
  if (!flashState.offer) {
    els.flashBtnText.textContent = 'عرض ساعة';
...
async function apiPatch(path, body, params = {}, timeoutMs = 12000) {
  const url = new URL(`${CONFIG.baseUrl}/${path}`);
  Object.entries(params).forEach(([k, v]) => {
    if (v === undefined || v === null || v === '') return;
    url.searchParams.set(k, v);
...
async function loadTierPrices(tier, preloadedRows = null) {
  const tierNameValue = String(tier?.tier_name || tierName() || '').trim();
  let cartonRows = preloadedRows?.cartonRows;
  let packRows = preloadedRows?.packRows;

  if (!Array.isArray(cartonRows) || !Array.isArray(packRows)) {
    [cartonRows, packRows] = await Promise.all([
...
function buildDatasetCacheSnapshot() {
  return {
    companies: (state.companies || []).map((company) => ({
      ...company,
      company_logo: sanitizeMediaValue(company.company_logo),
    })),
    products: (state.products || []).map((product) => ({
      ...product,
      product_image: sanitizeMediaValue(product.product_image),
    })),
    dailyDeals: (state.dailyDeals || []).map((deal) => ({
      ...deal,
      image: sanitizeMediaValue(deal.image),
    })),
    flashOffers: (state.flashOffers || []).map((offer) => ({
      ...offer,
      image: sanitizeMediaValue(offer.image),
    })),
    tiers: state.tiers || [],
...
function persistDatasetCache() {
  const snapshot = buildDatasetCacheSnapshot();
  if (saveJSON(STORAGE.dataCache, snapshot)) return true;

  const trimmed = {
    ...snapshot,
    companies: snapshot.companies.map(({ company_logo, ...rest }) => rest),
    products: snapshot.products.map(({ product_image, ...rest }) => rest),
    dailyDeals: snapshot.dailyDeals.map(({ image, ...rest }) => rest),
    flashOffers: snapshot.flashOffers.map(({ image, ...rest }) => rest),
    topProducts: snapshot.topProducts.slice(0, 30),
    topCompanies: snapshot.topCompanies.slice(0, 30),
  };

  return saveJSON(STORAGE.dataCache, trimmed);
}
...
async function loadData() {
  ...
  try {
    ...
    const companyRows = Array.isArray(companies) && companies.length ? companies : state.companies;
    const productRows = normalizeProductsFromTables(
      Array.isArray(viewProducts) && viewProducts.length ? viewProducts : (rawProducts || []),
      cartonRows || [],
      packRows || []
    );
    ...
    state.dailyDeals = sortDeals(dailyDealRows || []);
    state.flashOffers = sortFlashOffers(flashOfferRows || []);
    state.tiers = sortTiers(tiers || []);
    state.settings = settings || [];
    state.settingMap = new Map((state.settings || []).map((row) => [String(row.key || '').trim(), row.value]));
    state.companyMap = new Map((state.companies || []).map((row) => [String(row.company_id).trim(), row]));
    ...
    if (state.selectedTier) {
      ...
      await loadTierPrices(matched, { cartonRows, packRows });
    } else {
      await loadTierPrices(null, { cartonRows, packRows });
    }
    ...
    void (async () => {
      const [salesOrders, salesItems] = await Promise.all([
        fetchCollection('orders', { select: 'id,status,order_number,created_at,customer_id,user_id,user_type', order: 'created_at.desc', limit: '1500' }, []),
        fetchCollection('order_items', { select: 'order_id,product_id,type,qty,price,unit,created_at', order: 'created_at.desc', limit: '5000' }, []),
      ]);

      if ((!state.topProducts || !state.topProducts.length) && Array.isArray(salesOrders) && Array.isArray(salesItems)) {
      ...
      if ((!state.topCompanies || !state.topCompanies.length) && Array.isArray(salesOrders) && Array.isArray(salesItems)) {
    ...
    setTimeout(() => {
      syncInvoiceSequence().catch((error) => console.warn('invoice sync failed', error));
    }, 0);
  } catch (error) {
    console.error(error);
      ...
      <section class="banner-card home-hero">
        <div class="home-hero-copy">
          ...
          <div class="hero-actions">
            ...
            <button class="ghost-btn hero-login" type="button" data-action="open-login">تسجيل الدخول</button>
          </div>
...
(function () {
  ...
  function getThemeKey() {
    const saved = String(localStorage.getItem(THEME_STORAGE_KEY) || 'white').trim().toLowerCase();
    return THEMES[saved] ? saved : 'white';
  }

  function applyTheme(themeKey = getThemeKey(), persist = true) {
    const key = THEMES[themeKey] ? themeKey : 'white';
    ...
    if (persist) safeStorageSet(THEME_STORAGE_KEY, key);
    updateThemeDots();
    return key;
  }
  ...
  function createThemeSwitcher() {
    ...
    wrap.innerHTML = `
      <span class="theme-switcher-label">اختر الثيم</span>
