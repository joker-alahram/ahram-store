...
const SELLABLE_RUNTIME_VIEW = 'v_master_sellable_products';
...
const UNIT_META = {
  carton: { label: 'كرتونة', priority: 1 },
  pack: { label: 'دستة', priority: 2 },
  halfpack: { label: 'نص دستة', priority: 2.5 },
  piece: { label: 'قطعة', priority: 3 },
};
...
function normalizeUnitCode(unitCode) {
  const raw = String(unitCode || '').trim().toLowerCase();
  if (!raw) return '';
  return UNIT_ALIASES[raw] || raw;
}

function normalizeTierKey(tierName) {
...
function unitMeta(unitCode) {
  const normalized = normalizeUnitCode(unitCode);
  return UNIT_META[normalized] || null;
}

function labelForUnit(unitCode) {
  const meta = unitMeta(unitCode);
  return meta?.label || String(unitCode || '');
}

function unitPriority(unitCode) {
  const meta = unitMeta(unitCode);
...
function displayUnitPriceInfo(product, unit) {
  const normalized = normalizeUnitCode(unit);
  const pricing = unitPricingForProduct(product, normalized);
  if (!pricing || pricing.allowSale === false) {
    return { base: null, final: null };
  }

  const base = finiteNumber(pricing.basePrice);
  const final = finiteNumber(pricing.finalPrice);
  if (final === null || final <= 0) {
    return { base: null, final: null };
  }

  return {
    base: base !== null && base > 0 ? base : final,
    final,
  };
}

function unitHasSellablePrice(product, unit) {
  const normalized = normalizeUnitCode(unit);
  if (!normalized) return false;
  const pricing = unitPricingForProduct(product, normalized);
  if (!pricing || pricing.allowSale === false) return false;
  return Number.isFinite(Number(pricing.finalPrice)) && Number(pricing.finalPrice) > 0;
}

function sortUnitCodes(units = []) {
  return [...new Set((units || []).map(normalizeUnitCode).filter(Boolean))]
    .sort((a, b) => unitPriority(a) - unitPriority(b) || compareNatural(a, b));
}

function productAvailableUnits(product) {
  const fromPricing = product?.unitPricing ? Object.keys(product.unitPricing) : [];
  const fromCodes = Array.isArray(product?.unitCodes) ? product.unitCodes : [];
  const fromRows = Array.isArray(product?.runtimeRows)
    ? product.runtimeRows.map((row) => runtimeRowUnitCode(row))
    : Array.isArray(product?.unitRows)
      ? product.unitRows.map((row) => runtimeRowUnitCode(row))
      : [];
  return sortUnitCodes([...fromPricing, ...fromCodes, ...fromRows]);
}

function productSellableUnits(product) {
  const visible = Array.isArray(product?.visibleUnits) ? product.visibleUnits : [];
  if (visible.length) return sortUnitCodes(visible);

  const sellable = Array.isArray(product?.sellableUnits) ? product.sellableUnits : [];
  if (sellable.length) return sortUnitCodes(sellable);

  return productAvailableUnits(product).filter((unit) => unitHasSellablePrice(product, unit));
}

function productHasSellableUnits(product) {
  return productSellableUnits(product).length > 0;
}

function productResolvedUnit(product) {
  const preferred = normalizeUnitCode(product?.selectedUnit || state.unitPrefs?.[product?.product_id]);
  const sellable = productSellableUnits(product);
  if (preferred && sellable.includes(preferred)) return preferred;
  const resolved = normalizeUnitCode(product?.resolvedSelectedUnit || product?.currentUnit);
  if (resolved && sellable.includes(resolved)) return resolved;
  return sellable[0] || null;
}

function deriveProductRuntimeState(product, selectedTier = tierName()) {
  const runtimeRows = Array.isArray(product?.runtimeRows)
    ? product.runtimeRows.filter(Boolean)
    : Array.isArray(product?.unitRows)
      ? product.unitRows.filter(Boolean)
      : [];

  const unitRowsByCode = new Map();
  for (const row of runtimeRows) {
    const unitCode = runtimeRowUnitCode(row);
    if (!unitCode) continue;
    if (!unitRowsByCode.has(unitCode)) unitRowsByCode.set(unitCode, []);
    unitRowsByCode.get(unitCode).push(row);
  }

  const availableUnits = sortUnitCodes([...unitRowsByCode.keys()]);
  const unitPricing = {};
  const visibleUnits = [];

  for (const unitCode of availableUnits) {
    const chosenRow = selectRuntimeRow(unitRowsByCode.get(unitCode) || [], selectedTier);
    if (!chosenRow) continue;

    const finalPrice = Number(chosenRow.final_price ?? chosenRow.base_price ?? 0);
    const basePrice = Number(chosenRow.base_price ?? chosenRow.final_price ?? 0);
    const allowSale = chosenRow.allow_sale !== false;
    const sellable = allowSale && Number.isFinite(finalPrice) && finalPrice > 0;
    if (!sellable) continue;

    unitPricing[unitCode] = {
      basePrice,
      finalPrice,
      allowSale,
      applyDiscount: chosenRow.apply_discount !== false,
      tierName: chosenRow.tier_name || '',
      label: labelForUnit(unitCode),
      priority: unitPriority(unitCode),
    };
    visibleUnits.push(unitCode);
  }

  const selectedUnit = normalizeUnitCode(product?.selectedUnit || state.unitPrefs?.[product?.product_id]);
  const resolvedSelectedUnit = visibleUnits.includes(selectedUnit) ? selectedUnit : visibleUnits[0] || null;
  const hasSellableUnits = visibleUnits.length > 0;

  return {
    ...product,
    runtimeRows,
    unitRows: runtimeRows,
    availableUnits,
    visibleUnits,
    sellableUnits: visibleUnits,
    selectedUnit: selectedUnit || null,
    resolvedSelectedUnit,
    visible: hasSellableUnits,
    has_carton: visibleUnits.includes('carton'),
    has_pack: visibleUnits.includes('pack'),
    has_unit: visibleUnits.includes('piece'),
    unitCodes: visibleUnits,
    unitPricing,
    carton_price: unitPricing.carton?.finalPrice ?? null,
    pack_price: unitPricing.pack?.finalPrice ?? null,
    piece_price: unitPricing.piece?.finalPrice ?? null,
  };
}

function cartItemUnitLabel(item) {
  if (!item) return '';
  if (item.unit === 'single') return 'دستة';
  return labelForUnit(item.unit);
}

function rowMatchesTier(row, selectedTier) {
  if (!row || !selectedTier) return false;
  return normalizeTierKey(row.tier_name) === normalizeTierKey(selectedTier);
}

function runtimeRowSellable(row) {
  if (!row) return false;
  if (row.allow_sale === false) return false;
  const finalPrice = finiteNumber(row.final_price);
  const basePrice = finiteNumber(row.base_price);
  const resolved = finalPrice ?? basePrice;
  return resolved !== null && resolved > 0;
}

function selectRuntimeRow(rows = [], selectedTier = null) {
  const list = Array.isArray(rows) ? rows.filter(Boolean) : [];
  if (!list.length) return null;

  const preferred = normalizeTierKey(selectedTier);
  const exactRows = preferred ? list.filter((row) => rowMatchesTier(row, preferred)) : [];
  const baseRows = list.filter((row) => normalizeTierKey(row?.tier_name) === 'base');

  const bestExact = exactRows.find(runtimeRowSellable) || null;
  if (bestExact) return bestExact;

  const bestBase = baseRows.find(runtimeRowSellable) || null;
  if (bestBase) return bestBase;

  return exactRows[0] || baseRows[0] || null;
}

function runtimeRowUnitCode(row) {
  return normalizeUnitCode(row?.unit_code || row?.unit || row?.unit_code_name || '');
}

function buildUnitPricingFromRows(rows = [], selectedTier = null) {
  const unitRows = new Map();
  for (const row of rows || []) {
    const unitCode = runtimeRowUnitCode(row);
    if (!unitCode) continue;
    if (!unitRows.has(unitCode)) unitRows.set(unitCode, []);
    unitRows.get(unitCode).push(row);
  }

  const unitPricing = {};
  const unitCodes = sortUnitCodes(unitRows.keys());
  for (const unitCode of unitCodes) {
    const chosenRow = selectRuntimeRow(unitRows.get(unitCode), selectedTier);
    if (!chosenRow) continue;
    unitPricing[unitCode] = {
      basePrice: Number(chosenRow.base_price || 0),
      finalPrice: Number(chosenRow.final_price || chosenRow.base_price || 0),
      allowSale: chosenRow.allow_sale !== false,
      applyDiscount: chosenRow.apply_discount !== false,
      tierName: chosenRow.tier_name || '',
      label: labelForUnit(unitCode),
      priority: unitPriority(unitCode),
  ...
  return { unitPricing, unitCodes };
}

function refreshProductRuntimePricing(selectedTier = tierName()) {
  const activeTier = selectedTier || null;
  state.products = (state.products || []).map((product) => deriveProductRuntimeState(product, activeTier));
}
...
const els = {
  ...
  searchCartTotal: document.getElementById('searchCartTotal'),
  userBtn: document.getElementById('userBtn'),
  userWrap: document.querySelector('.user-wrap'),
  userMenu: document.getElementById('userMenu'),
  mobileFooterNav: document.getElementById('mobileFooterNav'),
...
function rankProducts(products, limit = 8, scorer = productScore) {
  return [...products]
    .filter((product) => Boolean(product) && productHasSellableUnits(product))
...
function topViewedProducts(limit = 8) {
  ...
  return [...state.products]
    .filter((product) => productHasSellableUnits(product))
    .sort((a, b) => {
...
function topBestSellingProducts(limit = 8) {
  ...
  for (const row of state.topProducts || []) {
    ...
    if (!product || !productHasSellableUnits(product)) continue;
...
function cartCompanionProducts(limit = 8) {
  ...
  const base = [...state.products].filter((product) => productHasSellableUnits(product) && !cartProductIds.has(String(product.product_id)));
...
function companyBackedProducts(companyId, limit = 8) {
  return [...state.products]
    .filter((product) => productHasSellableUnits(product) && String(product.company_id) === String(companyId))
...
function bestCompanyCards(limit = 8) {
  ...
  for (const product of state.products || []) {
    if (!productHasSellableUnits(product)) continue;
...
function openProductModal(productId) {
  ...
  const unit = currentVisibleUnitForProduct(product);
  const hasSellableUnit = Boolean(unit);
  const inCart = hasSellableUnit ? Boolean(findCartItem(cartKey({ type: 'product', id: product.product_id, unit }))) : false;
  const priceText = hasSellableUnit ? displayPriceText(product, unit) : 'غير متاح';
  const unitText = hasSellableUnit ? labelForUnit(unit) : 'لا توجد وحدة بسعر صالح';
  ...
  if (hasSellableUnit) {
    ...
    els.productModalAction.removeAttribute('disabled');
    els.productModalAction.removeAttribute('aria-disabled');
  } else {
    els.productModalAction.textContent = 'غير متاح';
    els.productModalAction.setAttribute('data-action', 'noop');
    els.productModalAction.removeAttribute('data-product-id');
    els.productModalAction.setAttribute('disabled', 'disabled');
    els.productModalAction.setAttribute('aria-disabled', 'true');
  }
...
function compactProductCardHtml(product, options = {}) {
  ...
  const units = visibleProductCardUnits(product);
  const unit = currentVisibleUnitForProduct(product);
  const hasSellableUnit = Boolean(unit);
  const key = hasSellableUnit ? cartKey({ type: 'product', id: product.product_id, unit }) : null;
  const inCart = key ? Boolean(findCartItem(key)) : false;
  ...
  const unitHtml = units.length
    ? units.map((itemUnit) => `
        <button class="unit-chip ${itemUnit === unit ? 'active' : ''}" data-action="set-unit" data-product-id="${escapeHtml(product.product_id)}" data-unit="${escapeHtml(itemUnit)}">
          ${escapeHtml(labelForUnit(itemUnit))}
        </button>
      `).join('')
    : `<span class="product-sub">لا توجد وحدة بسعر صالح</span>`;

  return `
    <article class="product-tile ${hasSellableUnit ? '' : 'is-unavailable'}" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body compact">
        ...
        <div class="unit-strip compact-unit-strip">${unitHtml}</div>
        <div class="card-actions compact-actions">
          <button class="primary-btn" data-action="${hasSellableUnit ? 'toggle-product' : 'noop'}" data-product-id="${escapeHtml(product.product_id)}" ${hasSellableUnit ? '' : 'disabled aria-disabled="true"'}>${hasSellableUnit ? (inCart ? 'إزالة من السلة' : escapeHtml(actionLabel)) : 'غير متاح'}</button>
        </div>
      </div>
    </article>
  `;
...
function resolveProductPrice(product, unit) {
  ...
  const pricing = unitPricingForProduct(product, normalizedUnit);
  if (!pricing || pricing.allowSale === false) return null;
  const finalPrice = finiteNumber(pricing.finalPrice);
  return finalPrice !== null && finalPrice > 0 ? finalPrice : null;
}
...
function syncCartPricesFromCurrentState() {
  ...
  state.cart = state.cart.map((item) => {
    ...
    const unitLabel = cartItemUnitLabel(item);
...
function updateHeader() {
  ...
  if (els.cartLabel) els.cartLabel.textContent = 'اتمام الدفع';
  if (els.cartValue) els.cartValue.textContent = integer(cartTotal());
  if (els.searchCartTotal) els.searchCartTotal.textContent = integer(cartTotal());
  updateFlashHeader();
  syncCheckoutButton();
}
...
function getSelectedTierObject() {
  ...
  if (typeof state.selectedTier === 'string') {
    const selectedKey = normalizeTierKey(state.selectedTier);
    return state.tiers.find((tier) => normalizeTierKey(tier.tier_name) === selectedKey) || { tier_name: state.selectedTier };
  }
...
function currentUnitForProduct(product) {
  return productResolvedUnit(product);
}

function availableUnits(product) {
  return productAvailableUnits(product);
}

function unitCanRenderInCard(product, unit) {
  return productSellableUnits(product).includes(normalizeUnitCode(unit));
}

function visibleProductCardUnits(product) {
  return productSellableUnits(product);
}

function currentVisibleUnitForProduct(product) {
  return productResolvedUnit(product);
}

function unitPricingForProduct(product, unit) {
  const normalized = normalizeUnitCode(unit);
  return product?.unitPricing?.[normalized] || null;
}

function baseUnitPrice(product, unit) {
  const pricing = unitPricingForProduct(product, unit);
  if (pricing && Number.isFinite(Number(pricing.basePrice))) return Number(pricing.basePrice);

  const normalized = normalizeUnitCode(unit);
  if (normalized === 'pack') return Number(product.pack_price || 0);
  if (normalized === 'halfpack') return Number(product.half_pack_price || product.halfpack_price || product.half_pack || 0);
  if (normalized === 'piece') return Number(product.piece_price || 0);
  return Number(product.carton_price || 0);
}

function tierUnitPrice(product, unit) {
  const pricing = unitPricingForProduct(product, unit);
  if (pricing && Number.isFinite(Number(pricing.finalPrice))) return Number(pricing.finalPrice);
...
function displayPriceBlock(product, unit) {
  const info = displayUnitPriceInfo(product, unit);
  const unitLabel = labelForUnit(unit);

  if (info.base === null || info.final === null) {
    return `
      <div class="price-wrap">
        <span class="price-main">غير متاح</span>
        <span class="product-sub">${unitLabel || 'لا توجد وحدة بسعر صالح'}</span>
      </div>
    `;
  }

  if (info.final < info.base) {
    return `
      <div class="price-wrap">
        <span class="price-old">${num(info.base)} ج.م</span>
        <span class="price-new">${num(info.final)} ج.م</span>
  ...
  return `
    <div class="price-wrap">
      <span class="price-main">${num(info.final)} ج.م</span>
      <span class="product-sub">${unitLabel}</span>
    </div>
  `;
}
...
function displayPriceText(product, unit) {
  ...
  return Number.isFinite(Number(price)) && Number(price) > 0 ? num(price) : 'غير متاح';
}
...
async function loadTierPrices(tier) {
  ...
  refreshProductRuntimePricing(tierNameValue);
...
function pickTierPrice(rows = [], tierName = null) {
  const preferred = normalizeTierKey(tierName);
  ...
  for (const row of rows || []) {
    ...
    const tierKey = normalizeTierKey(row.tier_name);
    const current = byProduct.get(productId) || null;

    if (!current) {
      if (!preferred && tierKey !== 'base') continue;
      byProduct.set(productId, { price, tierKey });
      continue;
    }

    if (preferred && tierKey === preferred && current.tierKey !== preferred) {
      byProduct.set(productId, { price, tierKey });
      continue;
    }

    if (tierKey === 'base' && current.tierKey !== 'base') {
      byProduct.set(productId, { price, tierKey });
    }
  }

  return new Map([...byProduct.entries()].map(([productId, entry]) => [productId, entry.price]));
}

function normalizeProductsFromTables(runtimeRows = [], rawProducts = []) {
  ...
  for (const product of rawProducts || []) {
    ...
    productsMap.set(productId, {
      ...
      catalog_visible: product.visible !== false,
      visible: product.visible !== false,
      type: product.type || 'normal',
      allow_discount: product.allow_discount ?? (product.discount_carton !== false || product.discount_pack !== false || product.discount_unit !== false),
      discount_carton: product.discount_carton ?? true,
      discount_pack: product.discount_pack ?? true,
      discount_unit: product.discount_unit ?? true,
  ...
  for (const row of runtimeRows || []) {
    if (!row) continue;
    const productId = String(row.product_id || '').trim();
    if (!productId || !productsMap.has(productId)) continue;
    const product = productsMap.get(productId);
    product.runtimeRows.push(row);
    product.unitRows.push(row);
  }

  const products = [...productsMap.values()].map((product) => deriveProductRuntimeState(product, tierName()));
...
async function loadData() {
  try {
    ...
    const [
      ...
      fetchCollection(SELLABLE_RUNTIME_VIEW, { select: '*', order: 'product_id.asc,unit_code.asc,tier_name.asc' }, []),
      ...
      fetchCollection('products', { select: 'product_id,product_name,company_id,category,product_image,status,visible,has_carton,has_pack,has_unit,type,discount_carton,discount_pack,discount_unit', order: 'product_name.asc' }, []),
    ...
    refreshProductRuntimePricing(tierKey);
    ...
    if (state.selectedTier) {
      const matched = state.tiers.find((tier) => normalizeTierKey(tier.tier_name) === normalizeTierKey(tierKey)) || state.selectedTier;
...
function filteredProducts() {
  let items = [...state.products].filter((product) => productHasSellableUnits(product));
  const q = normalizeText(state.search);
...
function productCardHtml(product) {
  ...
  return `
    <article class="product-card ${hasSellableUnit ? '' : 'is-unavailable'}" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body">
        ...
        ${hasSellableUnit ? displayPriceBlock(product, unit) : `<div class="price-wrap"><span class="price-main">غير متاح</span><span class="product-sub">لا توجد وحدة بسعر صالح</span></div>`}
        <label class="qty-pick">
          ...
          <input
            ...
            inputmode="numeric"
            ${hasSellableUnit ? '' : 'disabled'}
          />
        </label>
        <div class="unit-strip">${unitHtml}</div>
        <div class="card-actions">
          <button class="primary-btn" data-action="${hasSellableUnit ? 'toggle-product' : 'noop'}" data-product-id="${escapeHtml(product.product_id)}" ${hasSellableUnit ? '' : 'disabled aria-disabled="true"'}>
            ${hasSellableUnit ? (inCart ? 'إزالة من السلة' : 'شراء') : 'غير متاح'}
          </button>
        </div>
      </div>
    </article>
  ...
  setSearchBarHtml(`
    <section class="section-card search-card">
      ...
      <div class="search-row">
        ...
        <span class="search-total mono" id="searchCartTotal">${integer(cartTotal())}</span>
...
function renderApp() {
  ...
  if (els.mobileFooterNav) els.mobileFooterNav.classList.toggle('hidden', isRegister);
...
async function handleSelectTier(tier) {
  ...
  if (current === tier.tier_name) {
    ...
    state.tierPrices = { carton: new Map(), pack: new Map(), piece: new Map() };
    refreshProductRuntimePricing(null);
    resetCheckoutStage();
    syncCartPricesFromCurrentState();
    renderCart();
    renderApp();
...
function toggleProductFromCard(productId) {
  ...
  const unit = card?.querySelector('.unit-chip.active')?.getAttribute('data-unit') || currentVisibleUnitForProduct(product);
  if (!unit || !unitHasSellablePrice(product, unit)) {
    smartToast('product.unavailable', 'هذا المنتج لا يملك وحدة بسعر صالح حالياً', true);
    return;
  }
  ...
  state.cart.push({
    ...
    unit,
    unitLabel: cartItemUnitLabel({ unit }),
    price,
    qty,
  });
  persistCart();
  ...
  els.cartItems.innerHTML = state.cart.map((item) => `
    <div class="cart-item">
      ...
      <div>
        ...
        <div class="cart-price">${escapeHtml(item.unitLabel || cartItemUnitLabel(item) || item.unit || '')} · ${num(item.price)} ج.م</div>
        ${item.unit === 'single' ? `
...
async function getLiveCheckoutSnapshot() {
  ...
  const [freshProducts, freshTier] = await Promise.all([
    ids.length ? apiGet(SELLABLE_RUNTIME_VIEW, {
      select: 'product_id,product_name,unit_code,tier_name,base_price,final_price,allow_sale,apply_discount',
      product_id: `in.(${ids.join(',')})`,
      ...(tierKey ? { tier_name: `eq.${tierKey}` } : {}),
  ...
  for (const row of freshProducts || []) {
    if (!row) continue;
    const productId = String(row.product_id || '').trim();
    const unitCode = normalizeUnitCode(row.unit_code || '');
    if (!productId || !unitCode) continue;
    if (!productsMap.has(productId)) productsMap.set(productId, new Map());
    productsMap.get(productId).set(unitCode, row);
  }
...
async function validateCartAgainstDatabase() {
  ...
  for (const item of state.cart) {
    if (item.type !== 'product') continue;
    const unitCode = normalizeUnitCode(item.unit);
    const productUnits = snapshot.productsMap.get(String(item.id));
    const product = productUnits?.get(unitCode) || null;
    if (!product) {
      return { ok: false, message: `المنتج ${item.title} غير موجود الآن` };
    }

    const livePrice = Number(product.final_price || 0);
...
async function insertOrderItems(orderId) {
  const items = state.cart.map((item) => ({
    ...
    price: Number(item.price || 0),
    unit: item.type === 'product' ? (normalizeUnitCode(item.unit) || 'piece') : item.unit === 'carton' ? 'carton' : item.unit === 'pack' ? 'pack' : 'piece',
...
`;
  ...
  items.forEach((i) => {
    const unitLabel = cartItemUnitLabel(i) || labelForUnit(i.unit);
...
function syncUnitPreference(productId, unit) {
  state.unitPrefs[productId] = normalizeUnitCode(unit) || unit;
  saveJSON(STORAGE.unitPrefs, state.unitPrefs);
}



let mobileFooterResizeObserver = null;

function syncMobileFooterHeight() {
  const footer = els.mobileFooterNav || document.getElementById('mobileFooterNav');
  if (!footer || footer.classList.contains('hidden')) {
    document.documentElement.style.setProperty('--mobile-footer-h', '0px');
    return;
  }
  const height = Math.ceil(footer.getBoundingClientRect().height || 0);
  document.documentElement.style.setProperty('--mobile-footer-h', `${height}px`);
}

function setupMobileFooterNavigation() {
  let footer = els.mobileFooterNav || document.getElementById('mobileFooterNav');
  if (!footer) {
    footer = document.createElement('nav');
    footer.id = 'mobileFooterNav';
    footer.className = 'mobile-footer-nav';
    footer.setAttribute('aria-label', 'التنقل السفلي');
    document.body.appendChild(footer);
  }
  els.mobileFooterNav = footer;

  const userWrap = els.userWrap || els.userBtn?.closest('.user-wrap') || document.querySelector('.user-wrap');
  els.userWrap = userWrap || null;

  const nodes = [els.homeBtn, els.tierBtn, userWrap, els.dealsBtn, els.cartBtn].filter(Boolean);
  nodes.forEach((node) => {
    if (node.parentElement !== footer) footer.appendChild(node);
  });

  syncMobileFooterHeight();

  if (mobileFooterResizeObserver) mobileFooterResizeObserver.disconnect();
  if (window.ResizeObserver) {
    mobileFooterResizeObserver = new ResizeObserver(() => {
      window.requestAnimationFrame(syncMobileFooterHeight);
    });
    mobileFooterResizeObserver.observe(footer);
  } else {
    window.addEventListener('resize', syncMobileFooterHeight, { passive: true });
  }
}
