...
function contactStripHtml() {
  const supportWhatsapp = String(CONFIG.supportWhatsapp || '201040880002').replace(/\D/g, '');
  return `
    <div class="contact-strip" aria-label="وسائل التواصل السريع">
      <a class="contact-icon-btn contact-call" href="tel:01040880002" aria-label="اتصال" title="اتصال">Call</a>
      <a class="contact-icon-btn contact-whatsapp" href="https://wa.me/${supportWhatsapp}" target="_blank" rel="noopener noreferrer" aria-label="واتساب" title="واتساب">WhatsApp</a>
      <a class="contact-icon-btn contact-facebook" href="https://www.facebook.com/alahram2014/" target="_blank" rel="noopener noreferrer" aria-label="فيسبوك" title="فيسبوك">Facebook</a>
    </div>`;
}
...
function updateHeader() {
  ...
  if (els.socialBar) els.socialBar.innerHTML = contactStripHtml();
  updateFlashHeader();
  syncCheckoutButton();
}
...
function homeHeroState() {
  ...
  if (activeFlash) {
    return {
      ...
      metricLabel: 'العروض المتبقية',
      metricValue: activeFlash.stock ? `${integer(activeFlash.stock)} عروض` : (flashState.remaining || '—'),
      progress: 0,
      accent: 'flash',
    };
  }
...
function brandingBannerHtml() {
  return `
    <section class="company-banner-surface" aria-label="بانر الشركة">
      <img class="company-banner-image" src="${escapeHtml(appBannerImage())}" alt="بانر الشركة" loading="eager" />
    </section>
  `;
}
...
function homeBannerStripHtml(hero) {
  ...
  return `
    <section class="banner-strip-surface" aria-label="شريط تعريفي ثانوي">
      ...
      <button class="ghost-btn banner-strip-cta" type="button" data-action="home-cta">${escapeHtml(stripCta)}</button>
    </section>
  `;
}
      ...
      <section class="banner-card home-hero home-hero-${escapeHtml(hero.mode)}">
        <div class="home-hero-copy">
          ...
          <div class="hero-actions">
            <button class="primary-btn hero-cta" type="button" data-action="home-cta">${escapeHtml(hero.actionLabel)}</button>
...
function renderCategoryShelf(categories, title = 'الأقسام', subLabel = 'تصفح المنتجات حسب التصنيف') {
  ...
  return `
    <section class="shelf-card category-shelf">
      <div class="shelf-head">
        <div>
          <h2>${escapeHtml(title)}</h2>
          ${subLabel ? `<div class="helper-text">${escapeHtml(subLabel)}</div>` : ''}
...
function renderHomePage() {
  ...
  if (els.banner) {
    els.banner.innerHTML = brandingBannerHtml();
  }

  setSearchBarHtml(`
    <section class="section-card search-card search-shell">
  ...
  els.pageContent.innerHTML = `
    <div class="home-surface">
      ...
      <section class="shelf-card home-deals-shelf">
        <div class="shelf-head">
          <div>
            <h2>صفقة اليوم</h2>
...
function renderApp() {
  ...
  if (els.banner) {
    ...
    if (!isRegister) els.banner.innerHTML = brandingBannerHtml();
  }
