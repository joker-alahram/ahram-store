...
function updateHeader() {
  ...
  if (els.tierBtn) {
    const tierText = state.selectedTier ? tierLabel : 'اختار شريحتك';
    els.tierBtn.textContent = tierText;
    els.tierBtn.title = `${tierLabel} · اختار شريحتك`;
  ...
  if (els.cartValue) els.cartValue.textContent = `${integer(cartQuantity())} • ${integer(cartTotal())}`;
  if (els.cartBtn) els.cartBtn.setAttribute('aria-label', `السلة، ${integer(cartQuantity())} منتج، الإجمالي ${integer(cartTotal())}`);
  const tierStatusValue = document.getElementById('tierStatusValue');
  if (tierStatusValue) tierStatusValue.textContent = state.selectedTier ? tierLabel : 'غير مختارة';
  updateFlashHeader();
  syncCheckoutButton();
}
    ...
    els.banner.innerHTML = `
      <section class="banner-card home-hero">
        <div class="home-hero-copy">
          ...
          <p>حلول شراء احترافية للتجار والشركات مع أسعار شريحة، سلة واضحة، وعروض لحظية بلا ازدحام بصري.</p>
          ...
          <div class="hero-pills">
            <span class="badge">شركات</span>
            <span class="badge">شحن سريع</span>
            <span class="badge">ضمان استرجاع</span>
...
async function init() {
  ...
  const syncHeaderScrollState = () => {
    if (els.mainHeader) els.mainHeader.classList.toggle('is-scrolled', window.scrollY > 12);
  };
...
(function () {
  ...
  function decorateThemeSwitcher() {
    const mount = document.getElementById('themeSwitchMount');
    if (mount && !mount.querySelector('.theme-switcher')) {
      mount.innerHTML = '';
      mount.appendChild(createThemeSwitcher());
      return;
    }
  ...
  const observer = new MutationObserver(() => decorateThemeSwitcher());
  if (document.body) {
