...
function contactQuickSectionHtml() {
  return `
...
function syncStickyOpsLayout() {
  if (!els.stickyOpsBar || !els.searchBar) return;
  const hasSearch = Boolean(els.searchBar.innerHTML && els.searchBar.innerHTML.trim());
  els.stickyOpsBar.classList.toggle('has-search', hasSearch);
}

function syncChromeOffsets() {
  requestAnimationFrame(() => {
    const opsHeight = els.stickyOpsBar && !els.stickyOpsBar.classList.contains('hidden')
      ? Math.ceil(els.stickyOpsBar.getBoundingClientRect().height || els.stickyOpsBar.offsetHeight || 0)
      : 0;
    const footerHeight = els.bottomNav && !els.bottomNav.classList.contains('hidden')
      ? Math.ceil(els.bottomNav.getBoundingClientRect().height || els.bottomNav.offsetHeight || 0)
      : 0;
    document.documentElement.style.setProperty('--ops-bar-h', `${opsHeight}px`);
...
function updateBottomNavState() {
  ...
  const activeKey = route === 'company' || route === 'home' ? 'home' : route === 'tiers' ? 'tiers' : route === 'deals' ? 'deals' : route === 'flash' ? 'quick' : route === 'invoices' || route === 'my-customers' ? 'account' : 'home';
  els.bottomNav.querySelectorAll('[data-nav-key]').forEach((btn) => {
    btn.classList.toggle('active', btn.getAttribute('data-nav-key') === activeKey);
  });
}
...
function updateHeader() {
  ...
  if (els.cartLabel) els.cartLabel.textContent = 'إجمالي المشتريات';
  if (els.cartValue) els.cartValue.textContent = integer(cartTotal());
  if (els.cartBtn) els.cartBtn.setAttribute('aria-label', `إتمام الشراء · إجمالي المشتريات ${integer(cartTotal())}`);
  updateFlashHeader();
  syncCheckoutButton();
}
      ...
      <section class="banner-card home-hero">
        <div class="home-hero-copy">
          ...
          <div class="hero-actions">
            ...
            <button class="ghost-btn hero-login-btn" type="button" data-action="open-login">تسجيل الدخول</button>
          </div>
...
function renderApp() {
  ...
  if (els.stickyOpsBar) els.stickyOpsBar.classList.toggle('hidden', isRegister);
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'nav-quick') {
      ...
      setTimeout(() => {
        ...
        else document.getElementById('home-recommended')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 80);
      return;
    }
...
async function init() {
  ...
  syncCheckoutButton();
  syncChromeOffsets();
  if (window.ResizeObserver && els.stickyOpsBar) {
    const chromeObserver = new ResizeObserver(() => syncChromeOffsets());
    chromeObserver.observe(els.stickyOpsBar);
    if (els.bottomNav) chromeObserver.observe(els.bottomNav);
    window.addEventListener('beforeunload', () => chromeObserver.disconnect(), { once: true });
  } else {
