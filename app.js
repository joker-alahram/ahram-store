  ...
  els.pageContent.innerHTML = `
    <div class="home-surface">
      ...
      <section class="banner-strip-surface home-banner-strip" aria-label="شريط تعريفي ثانوي">
        ...
        <button class="ghost-btn banner-strip-cta" type="button" data-action="home-cta">${escapeHtml(hero.mode === 'flash' ? 'اعرف العرض' : 'اعرف المزيد')}</button>
      </section>
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'nav-categories') {
      ...
      toggleUserMenu(false);
      navigate('#home');
      setTimeout(() => document.getElementById('categories-shelf')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      return;
    }
    if (action === 'nav-deals') {
      closeNavDrawer();
