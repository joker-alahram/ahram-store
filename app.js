...
function renderHomePage() {
  ...
  const quickCards = [
    { title: 'الشرائح', hint: 'اختيار التسعير', action: 'tiers', count: state.tiers.length },
    { title: 'صفقة اليوم', hint: 'العروض الجاهزة', action: 'deals', count: state.dailyDeals.length },
    { title: 'عرض الساعة', hint: 'خصومات لحظية', action: 'flash', count: state.flashOffers.length },
    { title: 'فواتيري', hint: 'التقارير والحركة', action: 'invoices', count: state.invoices.length || '—' },
    { title: 'بياناتي', hint: 'معلومات الحساب', action: 'my-data', count: state.session ? '✓' : '—' },
    { title: 'السلة', hint: 'مراجعة الطلب', action: 'cart', count: state.cart.length },
  ];
      ...
      <section class="dashboard-panel">
        <div class="dashboard-head">
          <div>
            ...
            <div class="helper-text">وصول مباشر للقرارات، المراجعة، والتقارير</div>
          </div>
          <div class="badge">${escapeHtml(getSelectedTierLabel())}</div>
        </div>
        <div class="control-grid">
          ${quickCards.map((card) => `
            <button class="control-card" data-action="${escapeHtml(card.action)}" type="button">
              <div class="mini-count">${escapeHtml(card.count)}</div>
