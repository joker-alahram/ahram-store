...
const hasCachedDataset = Boolean(
  cachedDataset && (
    (cachedDataset.products?.length || 0) ||
    (cachedDataset.companies?.length || 0) ||
    (cachedDataset.dailyDeals?.length || 0) ||
    (cachedDataset.flashOffers?.length || 0)
  )
);
...
const els = {
  ...
  homeBtnLabel: document.getElementById('homeBtnLabel'),
...
const RUNTIME = {
  ...
  headerSnapshot: '',
  headerCompact: false,
  operationalHeaderInstalled: false,
  flashSnapshot: '',
};

const requestInflight = new Map();
const backgroundTaskQueue = [];
let backgroundTaskActive = 0;
const backgroundTaskLimit = 2;

const idleRequest = typeof window !== 'undefined' && typeof window.requestIdleCallback === 'function'
  ? window.requestIdleCallback.bind(window)
  : (callback, options = {}) => setTimeout(() => callback({
      didTimeout: true,
      timeRemaining: () => 0,
    }), Math.min(Number(options.timeout || 120), 120));

function wait(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

function scheduleIdleWork(task, timeout = 160) {
...
function enqueueBackgroundTask(task) {
  return new Promise((resolve, reject) => {
    backgroundTaskQueue.push({ task, resolve, reject });
    drainBackgroundTaskQueue();
  });
}

function drainBackgroundTaskQueue() {
  while (backgroundTaskActive < backgroundTaskLimit && backgroundTaskQueue.length) {
    const job = backgroundTaskQueue.shift();
    backgroundTaskActive += 1;
    scheduleIdleWork(job.task, 650)
      .then(job.resolve, job.reject)
      .finally(() => {
        backgroundTaskActive = Math.max(0, backgroundTaskActive - 1);
        drainBackgroundTaskQueue();
      });
  }
}
...
function runBackgroundTask(task) {
  return enqueueBackgroundTask(task);
}
...
function createTimedSignal(timeoutMs = 15000) {
...
async function fetchJson(url, options = {}) {
  const {
    ...
    method = 'GET',
    body = null,
    cacheKey = method === 'GET' ? url : '',
  } = options;

  if (cacheKey && requestInflight.has(cacheKey)) {
  ...
  const task = (async () => {
    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      const { signal, clear } = createTimedSignal(timeoutMs);
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(`${method} ${url} failed: ${res.status} ${text}`.trim());
        }
        ...
        if (attempt < retries && (aborted || !String(error?.message || '').includes('failed: 4'))) {
          const backoff = retryDelay * (2 ** attempt);
          const jitter = Math.min(180, Math.floor(Math.random() * 90));
...
function saveJSON(key, value) {
  try {
    ...
    console.warn('saveJSON failed:', key, error);
  }
}
...
function placeholderImage(seed = 'item') {
  const text = String(seed).slice(0, 24) || 'item';
  const safeText = text.replace(/[<>&"']/g, '');
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" role="img" aria-label="${safeText}">
      <defs>
      ...
      <circle cx="320" cy="236" r="148" fill="url(#r)"/>
      <path d="M180 388h280a24 24 0 0 1 24 24v0a24 24 0 0 1-24 24H180a24 24 0 0 1-24-24v0a24 24 0 0 1 24-24Z" fill="rgba(212,175,55,.18)"/>
      <path d="M220 210h200l-20 120H240z" fill="none" stroke="url(#g)" stroke-width="10" stroke-linejoin="round"/>
      <path d="M270 210c0-26 20-48 50-48s50 22 50 48" fill="none" stroke="url(#g)" stroke-width="10" stroke-linecap="round"/>
      <text x="320" y="480" text-anchor="middle" font-family="Cairo,Arial,sans-serif" font-size="44" font-weight="700" fill="#f8f4ea">${safeText}</text>
    </svg>`;
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
}

function normalizeImageUrl(url, seed = 'item') {
  const candidate = String(url ?? '').trim();
  if (!candidate) return placeholderImage(seed);
  if (/^(data:|https?:|\/\/)/i.test(candidate)) return candidate;
  return placeholderImage(seed);
}

function imageTag(url, alt, className, seed, eager = false, extraAttrs = '') {
  const src = normalizeImageUrl(url, seed || alt || 'item');
  const fallback = placeholderImage(seed || alt || 'item');
  return `<img class="${className}" src="${escapeHtml(src)}" alt="${escapeHtml(alt || '')}" loading="${eager ? 'eager' : 'lazy'}" decoding="async" ${eager ? 'fetchpriority="high"' : 'fetchpriority="low"'} onerror='this.onerror=null;this.src=${JSON.stringify(fallback)}' ${extraAttrs} />`;
}
...
function recordUiEvent(type, detail = {}) {
  ...
  void runBackgroundTask(() => apiPost('ui_events', {
    event_type: type,
...
function openProductModal(productId) {
  ...
  els.productModalBody.innerHTML = `
    <div class="modal-hero">
      ${imageTag(product.product_image, product.product_name, 'modal-image', product.product_name, true)}
...
function openOfferModal(type, id) {
  ...
  els.productModalBody.innerHTML = `
    <div class="modal-hero">
      ${imageTag(item.image, item.title, 'modal-image', item.title, true)}
      <div class="modal-meta">
        <div class="badge-row">
...
function normalizeProductsFromTables(products = []) {
  return sortProducts((products || []).map((product) => ({
    ...
    company_id: String(product?.company_id || '').trim(),
    product_image: String(product?.product_image || '').trim(),
    status: String(product?.status || 'active').trim(),
    visible: product?.visible !== false,
    has_carton: product?.has_carton ?? Number(product?.carton_price || 0) > 0,
    has_pack: product?.has_pack ?? Number(product?.pack_price || 0) > 0,
    carton_price: Number(product?.carton_price || 0),
    pack_price: Number(product?.pack_price || 0),
    allow_discount: product?.allow_discount ?? (product?.discount_carton !== false || product?.discount_pack !== false),
    type: String(product?.type || 'normal').trim(),
  ...
  return `
    <article class="product-tile" data-product-id="${escapeHtml(product.product_id)}">
      <button class="tile-image-wrap" type="button" data-action="open-product" data-product-id="${escapeHtml(product.product_id)}">
        ${imageTag(image, product.product_name, 'product-image', product.product_name)}
      </button>
...
function tierStatusLine() {
  ...
  if (remaining > 0) return `متبقي لتحقيق الشريحة ${num(remaining)} ج.م`;
...
function resolveProductPrice(product, unit) {
  ...
  const isPackUnit = String(unit || '') === 'pack';

  if (isPackUnit && product?.discount_pack === false) {
...
function updateHeader() {
  ...
  const remaining = tier ? Math.max(0, minOrder - eligible) : 0;
  const progress = tier && minOrder > 0 ? Math.max(0, Math.min(100, (eligible / minOrder) * 100)) : 0;
  const discount = Number(tier?.discount_percent || 0);
  const flashState = getFlashState();
  const flashSnapshot = `${flashState.status}|${flashState.remaining || ''}|${flashState.endedAt || ''}`;
  const headerSnapshot = [
    ...
    integer(state.cart.reduce((sum, item) => sum + Number(item.qty || 1), 0)),
    num(cartTotal()),
    state.search,
    flashSnapshot,
  ].join('|');

  if (state.__headerSnapshot === headerSnapshot) return;
  state.__headerSnapshot = headerSnapshot;
  ...
  if (els.tierContextStatus) els.tierContextStatus.textContent = tier ? statusLine : 'اختر شريحتك';
  if (els.tierDiscountState) els.tierDiscountState.textContent = tier ? `الخصم ${num(discount)}%` : 'الخصم —';
  if (els.tierMinState) els.tierMinState.textContent = tier ? `الحد الأدنى ${num(minOrder)} ج.م` : 'الحد الأدنى —';
  ...
  if (els.cartLabel) els.cartLabel.textContent = 'الإجمالي';
  if (els.cartValue) els.cartValue.textContent = num(cartTotal()) + ' ج.م';
...
function updateFlashHeader() {
  ...
  const flashSnapshot = `${flashState.status}|${flashState.remaining || ''}|${flashState.endedAt || ''}`;
  if (state.__flashSnapshot === flashSnapshot) return;
  state.__flashSnapshot = flashSnapshot;
  ...
  if (els.flashBtn) {
    ...
    els.flashBtn.title = isActive
      ? `عرض الساعة نشط · المتبقي ${flashState.remaining}`
      : flashState.status === 'expired'
        ? 'عرض الساعة منتهي'
        : 'عرض الساعة';
    els.flashBtn.setAttribute('aria-label', els.flashBtn.title);
  }
...
function companyName(companyId) {
  ...
  if (!id) return '';
  const mapped = state.companyMap.get(id);
  if (mapped?.company_name) return mapped.company_name;
  const fallback = state.companies.find((company) => String(company.company_id || '').trim() === id);
  return fallback?.company_name || id;
}

function companyCardHtml(company) {
  const companyId = String(company?.company_id || '').trim();
  const title = escapeHtml(company?.company_name || companyId || '');
  const logo = company?.company_logo ? escapeHtml(company.company_logo) : '';

  return `
    <button class="company-card company-card-home" type="button" data-action="open-company" data-company-id="${escapeHtml(companyId)}" aria-label="فتح شركة ${title}">
      <div class="company-logo-wrap" aria-hidden="true">
        ${logo ? imageTag(company.company_logo, '', 'company-logo', companyId) : `<div class="company-logo company-logo-fallback"></div>`}
      </div>
...
async function apiPatch(path, body, params = {}, options = {}) {
  const url = buildRequestUrl(path, params);
  return fetchJson(url, {
    ...options,
    method: 'PATCH',
...
function lookupUser(table, identifier) {
  const trimmed = String(identifier || '').trim();
  return apiGet(table, {
    select: 'id,name,phone,username,password',
    or: `(phone.eq.${trimmed},username.eq.${trimmed})`,
    limit: '1',
  }, { timeoutMs: 12000, retries: 1 }).then((rows) => rows?.[0] || null).catch(async () => {
    const phone = await apiGet(table, { select: 'id,name,phone,username,password', phone: `eq.${trimmed}`, limit: '1' }, { timeoutMs: 12000, retries: 0 });
    if (phone?.length) return phone[0];
    const username = await apiGet(table, { select: 'id,name,phone,username,password', username: `eq.${trimmed}`, limit: '1' }, { timeoutMs: 12000, retries: 0 });
...
async function loadTierPrices(tier) {
  ...
  const [cartonRows, packRows] = await Promise.allSettled([
  ...
  state.tierPrices = {
    carton: pickTierPrice(cartonRows.status === 'fulfilled' ? cartonRows.value : [], tierNameValue),
    pack: pickTierPrice(packRows.status === 'fulfilled' ? packRows.value : [], tierNameValue),
  };
...
async function fetchCollection(path, params = {}, fallback = []) {
  try {
    const rows = await apiGet(path, params, { timeoutMs: 12000, retries: 2 });
...
async function loadData() {
  ...
  const criticalBoot = async () => {
    const [companiesResult, productsResult, tiersResult, settingsResult] = await Promise.allSettled([
    ...
    const viewProducts = productsResult.status === 'fulfilled' ? productsResult.value : [];
    ...
    const productRows = normalizeProductsFromTables(viewProducts || []);
    ...
    if (state.selectedTier) {
      ...
      void runBackgroundTask(async () => {
        try {
          ...
          renderCart();
          updateHeader();
          if (state.view.type !== 'register') renderApp();
        } catch (error) {
          console.warn('tier price background refresh failed:', error);
        }
      });
    } else {
      state.tierPrices = {
  ...
  const hydrateBackground = async () => {
    ...
    await runBackgroundTask(async () => {
      const [dealsResult, flashResult, topProductsResult, topCompaniesResult] = await Promise.allSettled([
      ...
      const dailyDeals = dealsResult.status === 'fulfilled' ? dealsResult.value : [];
      const flashOffers = flashResult.status === 'fulfilled' ? flashResult.value : [];
      const topProductsView = topProductsResult.status === 'fulfilled' ? topProductsResult.value : [];
      const topCompaniesView = topCompaniesResult.status === 'fulfilled' ? topCompaniesResult.value : [];

      if (dailyDeals.length) state.dailyDeals = dailyDeals;
      if (flashOffers.length) state.flashOffers = flashOffers;
      if (topProductsView.length) state.topProducts = topProductsView;
      if (topCompaniesView.length) state.topCompanies = topCompaniesView;

      if (!topProductsView.length || !topCompaniesView.length) {
        const [salesOrdersResult, salesItemsResult] = await Promise.allSettled([
        ...
        const salesItems = salesItemsResult.status === 'fulfilled' ? salesItemsResult.value : [];
        const approvedOrders = new Set(
          (salesOrders || [])
            .filter((order) => ['confirmed', 'processing', 'shipped', 'delivered', 'paid', 'completed'].includes(String(order.status || '').trim()))
    ...
    try {
      const [analyticsRows, invoicesRows] = await Promise.allSettled([
        fetchCollection('ui_events', { select: 'id,event_type,entity_type,created_at', order: 'created_at.desc', limit: '50' }, []),
        state.session ? fetchCollection('orders', { select: 'id,status,order_number,created_at,total_amount,user_type,customer_id,user_id,updated_at', order: 'created_at.desc' }, []) : Promise.resolve([]),
      ]);

      if (analyticsRows.status === 'fulfilled' && Array.isArray(analyticsRows.value)) {
        state.analyticsSnapshot = analyticsRows.value;
      }
      if (invoicesRows.status === 'fulfilled' && Array.isArray(invoicesRows.value)) {
        state.invoices = invoicesRows.value;
      ...
      console.warn('background data hydration failed:', backgroundError);
    } finally {
...
function renderHomePage() {
  ...
  const q = normalizeText(state.search);
  const shelfState = state.homeShelfExpanded || (state.homeShelfExpanded = {});
  const loadingHome = !state.dataReady && !state.companies.length && !state.products.length && !state.dailyDeals.length && !state.flashOffers.length;

  const companyRows = (q ? filteredCompanies() : sortCompanies(state.companies));
  const recommended = state.dataReady ? topRecommendedProducts(12) : [];
  const viewed = state.dataReady ? topViewedProducts(12) : [];
  const bestSelling = state.dataReady ? topBestSellingProducts(12) : [];
  const companion = state.dataReady ? cartCompanionProducts(12) : [];

  const bannerHtml = banner
    ? imageTag(banner, 'بانر الصفحة الرئيسية', 'banner-image', 'banner', true)
    : `<div class="banner-fallback">اكتشف أفضل العروض الآن</div>`;

  const sliceShelf = (items, key, limit = 6) => {
    const expanded = Boolean(shelfState[key]);
    const visible = expanded ? items : items.slice(0, limit);
    return { expanded, visible, remaining: Math.max(0, items.length - visible.length) };
  };

  const shelfButton = (key, items, limit = 6) => {
    if (!items.length || items.length <= limit) return '';
    const { expanded, visible, remaining } = sliceShelf(items, key, limit);
    return `
      <button class="ghost-btn shelf-more" type="button" data-action="toggle-home-shelf" data-shelf-key="${escapeHtml(key)}">
        ${expanded ? 'إخفاء' : `عرض المزيد${remaining ? ` (${remaining})` : ''}`}
      </button>
    `;
  };

  const renderShelf = (title, subtitle, key, items, renderer, emptyText, limit = 6, extraClass = '') => {
    const { visible } = sliceShelf(items, key, limit);
    return `
      <section class="shelf-card shelf-rail-card ${extraClass}">
        <div class="shelf-head">
          <div>
            <h2>${escapeHtml(title)}</h2>
            ${subtitle ? `<div class="helper-text">${escapeHtml(subtitle)}</div>` : ''}
          </div>
          ${shelfButton(key, items, limit)}
        </div>
        <div class="shelf-rail">
          ${visible.length ? visible.map(renderer).join('') : `<div class="empty-state">${escapeHtml(emptyText)}</div>`}
        </div>
      </section>
    `;
  };
  ...
  if (els.banner) {
    els.banner.innerHTML = loadingHome
      ? `
        <section class="banner-card home-hero home-hero-skeleton">
          <div class="home-hero-copy">
            <div class="hero-kicker">B2B FAST COMMERCE</div>
            <h1>جارٍ تجهيز الواجهة</h1>
            <p>يتم تحميل البيانات الأساسية تدريجيًا لتظهر الصفحة أسرع بدون انتظار كامل الجداول.</p>
            <div class="hero-actions">
              <button class="primary-btn" type="button" disabled>تحميل سريع</button>
              <button class="ghost-btn" type="button" disabled>جارٍ المزامنة</button>
            </div>
            <div class="hero-pills">
              <span class="badge skeleton-pill">منتجات</span>
              <span class="badge skeleton-pill">شركات</span>
              <span class="badge skeleton-pill">عروض</span>
            </div>
          </div>
          <div class="home-hero-art">
            <div class="banner-fallback banner-skeleton"></div>
          </div>
        </section>
      `
      : `
  ...
  setSearchBarHtml(`
    <section class="section-card search-card ${loadingHome ? 'search-card-skeleton' : ''}">
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      ${loadingHome ? `
        <section class="shelf-card skeleton-shelf">
          <div class="shelf-head">
            <div>
              <h2>جاري التحميل</h2>
              <div class="helper-text">يتم تجهيز البيانات الأساسية</div>
            </div>
          </div>
          <div class="shelf-grid skeleton-grid">
      ...
      ` : `
        ${renderShelf('المقترحات المميزة', 'منتجات مرتفعة التفاعل ومناسبة للتسعير الحالي', 'recommended', recommended, (product) => compactProductCardHtml(product), 'لا توجد منتجات بعد', 6)}
        ${renderShelf('الشركات', 'تصفح مباشرة حسب الشركة', 'companies', companyRows, companyCardHtml, 'لا توجد شركات مطابقة', 6, 'company-shelf-home')}
        ${renderShelf('الأكثر مشاهدة', 'ترتيب مباشر بناء على التفاعل', 'viewed', viewed, (product) => compactProductCardHtml(product), 'لا توجد بيانات مشاهدة بعد', 6)}
        ${renderShelf('الأكثر مبيعًا', 'يعرض المنتجات ذات النشاط الأقوى', 'bestSelling', bestSelling, (product) => compactProductCardHtml(product), 'لا توجد بيانات مبيعات بعد', 6)}
        ${renderShelf('تكملة السلة', 'اقتراحات قريبة من سلة العميل الحالية', 'companion', companion, (product) => compactProductCardHtml(product), 'أضف منتجات أولًا', 6)}
      `}
    </div>
  `;
}
...
function renderCompanyPage() {
  ...
  const tier = getSelectedTierObject();
  const remaining = tier ? Math.max(0, tierRemainingAmount() || 0) : null;
  const remainingLabel = tier
    ? `متبقي لتحقيق الشريحة: ${num(remaining)} ج.م`
    : 'اختر شريحة تجارية لتظهر قيمة المتبقي';
    ...
    <section class="section-card search-card">
      <div class="section-head">
        <div>
          ...
          <div class="helper-text tier-company-line">
            <span>${escapeHtml(getSelectedTierLabel())}</span>
            <span class="tier-remaining-chip">${escapeHtml(remainingLabel)}</span>
          </div>
        </div>
      </div>
      <div class="search-row">
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="grid product-grid" id="companyProductGrid">
        ${products.length ? `<div class="empty-state">جارٍ تحميل المنتجات</div>` : `<div class="empty-state">لا توجد منتجات تطابق البحث أو الشركة المحددة</div>`}
      </section>
    </div>
  `;

  if (products.length) {
    hydrateChunkedGrid('companyProductGrid', products, productCardHtml, { chunkSize: 10, initialSize: 8 });
  }
}
  ...
  return `
    <article class="deal-card">
      <button class="deal-image-wrap" type="button" data-action="open-offer" data-offer-type="${escapeHtml(type)}" data-id="${escapeHtml(item.id)}">
        ${imageTag(image, item.title, 'deal-image', item.title)}
      </button>
      <div class="deal-body">
        <div class="badge-row">
...
const renderAppImmediate = renderApp;
let renderAppFramePending = false;

function scheduleRenderApp() {
  renderAppFramePending = true;
  if (RUNTIME.renderQueued) return;
  RUNTIME.renderQueued = true;
  requestAnimationFrame(() => {
    RUNTIME.renderQueued = false;
    if (!renderAppFramePending) return;
    renderAppFramePending = false;
    renderAppImmediate();
  });
}

renderApp = function renderAppScheduled() {
  scheduleRenderApp();
};

function syncOperationalHeaderState(force = false) {
  const compact = (window.scrollY || document.documentElement.scrollTop || 0) > 12;
  if (!force && RUNTIME.headerCompact === compact) return;
  RUNTIME.headerCompact = compact;
  document.body.classList.toggle('has-floating-header', compact);
  if (els.mainHeader) els.mainHeader.classList.toggle('is-compact', compact);
}

function installOperationalHeaderSync() {
  if (RUNTIME.operationalHeaderInstalled) return;
  RUNTIME.operationalHeaderInstalled = true;
  syncOperationalHeaderState(true);
  window.addEventListener('scroll', () => syncOperationalHeaderState(false), { passive: true });
  window.addEventListener('resize', () => syncOperationalHeaderState(true), { passive: true });
}

function hydrateChunkedGrid(containerId, items, renderItem, options = {}) {
  ...
  const appendChunk = () => {
    ...
    const limit = index === 0 ? Math.min(initialSize, total) : Math.min(index + chunkSize, total);

    for (; index < limit; index += 1) {
      const wrap = document.createElement('div');
      wrap.innerHTML = renderItem(items[index]);
    ...
    if (index < total) {
      scheduleIdleWork(appendChunk, 90);
    }
  };
...
function renderCart() {
  ...
  const summaryHtml = `
    <div class="cart-summary cart-summary-premium">
      <div class="cart-summary-icon">
        <span class="cart-summary-badge">${integer(state.cart.reduce((sum, item) => sum + Number(item.qty || 1), 0))}</span>
        <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
          <path d="M7.5 17.5a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3Zm8.8 0a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 0-3ZM6 6.2h13.7a1 1 0 0 1 .98 1.2l-1.35 6.4a2 2 0 0 1-1.95 1.58H8.3a2 2 0 0 1-1.94-1.51L5.1 4.8H3.8a1 1 0 1 1 0-2H5.9a1 1 0 0 1 .97.78z" />
        </svg>
      </div>
      <div class="cart-summary-copy">
        <div class="cart-summary-title">الإجمالي</div>
        <strong class="cart-summary-amount mono">${num(fullTotal)} ج.م</strong>
        <div class="cart-summary-note">اضغط لإتمام الشراء</div>
      </div>
      <div class="cart-summary-rail" aria-hidden="true"></div>
    </div>
  `;

  if (!state.cart.length) {
    els.cartItems.innerHTML = `${summaryHtml}<div class="empty-state">السلة فارغة الآن</div>`;
  ...
  els.cartItems.innerHTML = `${summaryHtml}${state.cart.map((item) => {
    const badgeText = describeCartBadge(item);
    const badgeHtml = badgeText ? `<span class="cart-item-badge ${item.type === 'product' ? 'badge-product' : item.type === 'deal' ? 'badge-deal' : 'badge-flash'}">${escapeHtml(badgeText)}</span>` : '';
    return `
    <div class="cart-item">
      ${imageTag(item.image, item.title, 'modal-image', item.title, true)}
      <div>
...
async function validateCartAgainstDatabase() {
  ...
  if (snapshot.tierRow && Number(CART_ENGINE.eligibleTierTotal(state.cart)) < Number(snapshot.tierRow.min_order || 0)) {
    ...
    return {
      ok: false,
      message: `متبقي لك ${num(remaining)} لتحقيق الشريحة`,
    };
  }
...
function validateBeforeOrder() {
  ...
  if (total < Number(tier.min_order || 0)) {
    ...
    smartToast(`لسه فاضلك مشتريات بمبلغ ${num(diff)} ج.م لتحقيق الشريحة`, undefined, true);
    return false;
  }
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'toggle-home-shelf') {
      const shelfKey = target.getAttribute('data-shelf-key');
      if (shelfKey) {
        state.homeShelfExpanded = state.homeShelfExpanded || {};
        state.homeShelfExpanded[shelfKey] = !state.homeShelfExpanded[shelfKey];
        renderApp();
      }
      return;
    }
...
function triggerSmartBehavior() {
  ...
  if (!tier && now - state.behavior.lastTierPrompt > 1800000) {
    state.behavior.lastTierPrompt = now;
    smartToast('حدد الشريحة المناسبة للحصول على أفضل سعر', undefined, true);
    return;
  }

  if (state.cart.length === 0 && now - state.behavior.lastCartActivity > 1800000 && now - state.behavior.lastCartPrompt > 1800000) {
    state.behavior.lastCartPrompt = now;
    smartToast('أبدأ بإضافة المنتجات للسلة لإتمام الطلب بسرعة', undefined, true);
    return;
  }

  if (!state.behavior.visitedDeals && now - state.behavior.lastDealsPrompt > 1800000) {
    state.behavior.lastDealsPrompt = now;
    smartToast('تصفح عروض الساعة قبل انتهاء الوقت', undefined, true);
    return;
  }

  const flashState = getFlashState();
  if (flashState.status === 'active' && now - state.behavior.lastFlashPrompt > 1800000) {
    state.behavior.lastFlashPrompt = now;
    smartToast('عرض الساعة نشط الآن', undefined, true);
  }
}
...
async function init() {
  wireGlobalEvents();
  installOperationalHeaderSync();
  renderCart();
  updateHeader();
  state.dataReady = Boolean(state.companies.length || state.products.length || state.dailyDeals.length || state.flashOffers.length);
  renderApp();
...
(function () {
  ...
  wrap.innerHTML = `
    <span class="theme-switcher-label">اختار الثيم المفضل</span>
    <span class="theme-switcher-dots">
  ...
  function createContactStrip() {
    const wrap = document.createElement('div');
    wrap.className = 'contact-strip hero-contact-strip';
    wrap.setAttribute('aria-label', 'وسائل التواصل السريع');
    wrap.innerHTML = `
      <a class="contact-icon-btn" href="tel:01040880002" aria-label="اتصال مباشر" title="اتصال مباشر">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.6 10.8c1.4 2.8 3.4 4.8 6.2 6.2l2.1-2.1c.3-.3.7-.4 1.1-.3 1.2.4 2.6.6 3.9.6.6 0 1 .4 1 1v3.3c0 .6-.4 1-1 1C10.8 20.5 3.5 13.2 3.5 4c0-.6.4-1 1-1h3.3c.6 0 1 .4 1 1 0 1.4.2 2.7.6 3.9.1.4 0 .8-.3 1.1l-1.5 1.8z"/></svg>
      </a>
  ...
  function decorateHero() {
    ...
    if (!hero.querySelector('.hero-utility-row')) {
      const utilityRow = document.createElement('div');
      utilityRow.className = 'hero-utility-row';
      utilityRow.appendChild(createThemeSwitcher());
      utilityRow.appendChild(createContactStrip());
      actions.insertAdjacentElement('afterend', utilityRow);
    } else {
      const utilityRow = hero.querySelector('.hero-utility-row');
      if (utilityRow) {
        const switcher = utilityRow.querySelector('.theme-switcher');
        const contactStrip = utilityRow.querySelector('.hero-contact-strip');
        if (!switcher) utilityRow.prepend(createThemeSwitcher());
        if (!contactStrip) utilityRow.appendChild(createContactStrip());
      }
    }
  }
