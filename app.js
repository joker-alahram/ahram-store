...
function setSearchBarHtml(html = '') {
  ...
  els.searchBar.innerHTML = html;
  els.headerSearchInput = document.getElementById('headerSearchInput');
  els.headerClearSearchBtn = document.getElementById('headerClearSearchBtn');
}
...
function headerSearchHtml() {
  return `
    <div class="search-card header-search-card">
      <div class="search-row">
        <input id="headerSearchInput" type="search" placeholder="ابحث باسم المنتج أو الشركة" value="${escapeHtml(state.search)}" />
        <button class="mini-btn icon-btn" id="headerClearSearchBtn" type="button" aria-label="مسح البحث">×</button>
      </div>
    </div>
  `;
}
...
function renderApp() {
  ...
  if (els.searchBar && !els.searchBar.querySelector('#headerSearchInput')) {
    setSearchBarHtml(headerSearchHtml());
  }
