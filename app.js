...
function fetchWithTimeout(input, init = {}, timeoutMs = 12000) {
...
async function loadTierPrices(tier) {
  ...
  try {
    ...
    console.warn('Load tier prices failed', error);
    state.tierPrices = {
      carton: new Map(),
      pack: new Map(),
    };
...
async function loadData() {
  ...
  const coreLoads = await Promise.allSettled([
  ...
  const [companiesRes, viewProductsRes, dailyDealsViewRes, flashOffersViewRes, tiersRes, settingsRes, topProductsViewRes, topCompaniesViewRes] = coreLoads.map((entry) => entry.status === 'fulfilled' ? entry.value : []);

  const companyRows = Array.isArray(companiesRes) && companiesRes.length ? companiesRes : state.companies;
  const viewProducts = Array.isArray(viewProductsRes) ? viewProductsRes : [];
  const dailyDealRows = Array.isArray(dailyDealsViewRes) ? dailyDealsViewRes : [];
  const flashOfferRows = Array.isArray(flashOffersViewRes) ? flashOffersViewRes : [];

  state.companies = sortCompanies(companyRows || []);
  state.dailyDeals = dailyDealRows;
  state.flashOffers = flashOfferRows;
  state.tiers = Array.isArray(tiersRes) && tiersRes.length ? tiersRes : state.tiers;
  state.settings = Array.isArray(settingsRes) && settingsRes.length ? settingsRes : state.settings;
  ...
  const needsSupplementalProducts = !viewProducts.length;
  const needsAnalyticsFallback = !(Array.isArray(topProductsViewRes) && topProductsViewRes.length) || !(Array.isArray(topCompaniesViewRes) && topCompaniesViewRes.length);

  const supplementalTasks = [];

  if (needsSupplementalProducts) {
  ...
  const supplementalResults = supplementalTasks.length ? await Promise.allSettled(supplementalTasks) : [];
  const fulfilledSupplemental = supplementalResults.map((entry) => entry.status === 'fulfilled' ? entry.value : []);
  ...
  if (needsSupplementalProducts) {
    rawProducts = Array.isArray(fulfilledSupplemental[supplementalIndex]) ? fulfilledSupplemental[supplementalIndex] : [];
    cartonRows = Array.isArray(fulfilledSupplemental[supplementalIndex + 1]) ? fulfilledSupplemental[supplementalIndex + 1] : [];
    packRows = Array.isArray(fulfilledSupplemental[supplementalIndex + 2]) ? fulfilledSupplemental[supplementalIndex + 2] : [];
    supplementalIndex += 3;
  }

  if (needsAnalyticsFallback) {
    salesOrders = Array.isArray(fulfilledSupplemental[supplementalIndex]) ? fulfilledSupplemental[supplementalIndex] : [];
    salesItems = Array.isArray(fulfilledSupplemental[supplementalIndex + 1]) ? fulfilledSupplemental[supplementalIndex + 1] : [];
  }
  ...
  if (Array.isArray(topProductsViewRes) && topProductsViewRes.length) {
    state.topProducts = topProductsViewRes;
  } else {
  ...
  if (Array.isArray(topCompaniesViewRes) && topCompaniesViewRes.length) {
    state.topCompanies = topCompaniesViewRes;
  } else {
  ...
  } else if (!Object.keys(state.tierPrices.carton || {}).length && !Object.keys(state.tierPrices.pack || {}).length) {
    state.tierPrices = { carton: new Map(), pack: new Map() };
  }

  await syncInvoiceSequence().catch(() => {});
  syncCartPricesFromCurrentState();
  resetCheckoutStage();
  renderCart();
  renderApp();
