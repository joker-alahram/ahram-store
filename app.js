...
const els = {
  ...
  themeStrip: document.getElementById('themeStrip'),
  headerHero: document.getElementById('headerHero'),
  companiesBtn: document.getElementById('companiesBtn'),
  categoriesBtn: document.getElementById('categoriesBtn'),
  bottomAccountLabel: document.getElementById('bottomAccountLabel'),
  tierPromptModal: document.getElementById('tierPromptModal'),
...
function renderMyDataContent() {
  ...
  const repActions = s.userType === 'rep' ? `
    <div class="account-actions">
      <button type="button" class="primary-btn" data-action="my-customers">عرض العملاء</button>
      <button type="button" class="primary-btn" data-action="open-add-customer">إضافة عميل</button>
      <button type="button" class="primary-btn" data-action="invoices">عرض فواتيري</button>
    </div>
  ` : '';
  if (!els.myDataContent) return;
  els.myDataContent.innerHTML = `
    <div class="profile-grid profile-grid-data">
...
function closeTierPrompt() {
  setOverlay(els.tierPromptModal, false);
}

function openTierPrompt() {
  setOverlay(els.tierPromptModal, true);
}

function renderGlobalSearchBar() {
  if (!els.searchBar) return;
  els.searchBar.innerHTML = `
    <section class="sticky-search-card">
      <div class="search-row">
        <input id="searchInput" type="search" placeholder="ابحث عن منتج أو شركة أو صنف" value="${escapeHtml(state.search)}" />
        <button class="mini-btn icon-btn" id="clearSearchBtn" type="button">×</button>
      </div>
      <button class="cart-chip search-cart-chip" id="cartBtn" type="button" data-action="cart" aria-label="إجمالي المشتريات">
        <span class="cart-value" id="cartValue">${integer(cartTotal())}</span>
        <span class="cart-label" id="cartLabel">إجمالي المشتريات — اضغط لإتمام الشراء</span>
      </button>
    </section>
  `;
  els.cartBtn = document.getElementById('cartBtn');
  els.cartLabel = document.getElementById('cartLabel');
  els.cartValue = document.getElementById('cartValue');
}

function renderHeaderHero() {
  if (!els.headerHero) return;
  if (state.view.type !== 'home') {
    els.headerHero.innerHTML = '';
    return;
  }

  const flashState = getFlashState();
  const activeFlash = flashState.status === 'active' && Boolean(flashState.offer);
  ...
  const achieved = Math.max(0, cartAmount);
  const remaining = Math.max(0, minOrder - achieved);
  const progress = minOrder > 0 ? Math.min(100, Math.round((achieved / minOrder) * 100)) : (state.selectedTier ? 100 : 0);

  if (activeFlash) {
    els.headerHero.innerHTML = `
      <section class="hero-card hero-card-flash">
        <div class="hero-clock" aria-hidden="true">
          <span class="clock-ring"></span>
          <span class="clock-hands"></span>
        </div>
        <div class="hero-copy">
          <div class="hero-kicker">عرض الساعة</div>
          <div class="hero-countdown mono">${escapeHtml(flashState.remaining || '--:--:--')}</div>
          <div class="hero-caption">متبقي على انتهاء العرض</div>
        </div>
      </section>
    `;
    return;
  ...
  els.headerHero.innerHTML = `
    <section class="hero-card hero-card-tier">
      <div class="hero-copy">
        <div class="hero-kicker">أنت الآن على شريحة</div>
        <div class="hero-title">${escapeHtml(tierLabel)}</div>
        <div class="hero-progress-wrap">
          <div class="hero-progress"><span style="width:${progress}%"></span></div>
          <div class="hero-progress-meta">
            <span>ما تم تحقيقه ${num(achieved)}</span>
            <span>متبقي لتحقيق الشريحة ${num(remaining)}</span>
          </div>
        </div>
      </div>
      <div class="hero-tier-badge">${escapeHtml(tier ? tierLabel : 'الشريحة الرئيسية')}</div>
    </section>
  `;
}

function renderBottomNavState() {
  const current = parseHash();
  const items = document.querySelectorAll('#bottomNav .bottom-nav-btn');
  items.forEach((btn) => btn.classList.remove('active'));
  const active = {
    home: current.type === 'home' || current.type === 'company' || current.type === 'register',
    tier: current.type === 'tiers',
    deals: current.type === 'deals' || current.type === 'flash',
  };
  const homeBtn = document.querySelector('#bottomNav [data-action="goto-home"]');
  const tierBtn = document.querySelector('#bottomNav [data-action="goto-tier"]');
  const dealsBtn = document.querySelector('#bottomNav [data-action="goto-flash"]');
  if (homeBtn && active.home) homeBtn.classList.add('active');
  if (tierBtn && active.tier) tierBtn.classList.add('active');
  if (dealsBtn && active.deals) dealsBtn.classList.add('active');
  if (els.bottomAccountLabel) els.bottomAccountLabel.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
}

function updateHeaderCollapseState() {
  const compact = state.view.type !== 'home' || window.scrollY > 14;
  document.body.classList.toggle('header-collapsed', compact);
}

function buildSmartCategoryChips() {
  const candidates = ['شامبو', 'كريم', 'رول أون', 'اسبراي', 'صبغات', 'غسول', 'بلسم', 'زيوت'];
  const found = new Set();
  const names = (state.products || []).map((p) => normalizeText(p.product_name || ''));
  for (const name of names) {
    for (const candidate of candidates) {
      if (name.includes(normalizeText(candidate))) found.add(candidate);
    }
    if (found.size >= 8) break;
  }
  return [...found, ...candidates].slice(0, 8);
}
...
function updateHeader() {
  ...
  if (els.userBtn) els.userBtn.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
  if (els.bottomAccountLabel) els.bottomAccountLabel.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
  if (els.cartLabel) els.cartLabel.textContent = 'إجمالي المشتريات — اضغط لإتمام الشراء';
...
function renderHomePage() {
  ...
  const bannerHtml = banner
    ? `<img class="banner-image" src="${escapeHtml(banner)}" alt="بانر الشركة" loading="eager" />`
    : `<div class="banner-fallback"><span>متجر الأهرام للتجارة والتوزيع</span></div>`;

  if (els.banner) {
    els.banner.innerHTML = `
      <section class="banner-card home-banner">
        ${bannerHtml}
      </section>
    `;
  }
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="section-card categories-section" id="categoriesShelf">
        <div class="section-head">
          <div>
            <h2>الأقسام</h2>
            <div class="helper-text">مسار سريع للتصفح التشغيلي</div>
          </div>
        </div>
        <div class="category-grid">
          <article class="category-card" data-action="open-companies">
            <div class="category-icon">🏢</div>
            <h3>الشركات</h3>
          </article>
          <article class="category-card" data-action="open-best-selling">
            <div class="category-icon">📦</div>
            <h3>الأكثر مبيعًا</h3>
          </article>
          <article class="category-card" data-action="open-top-viewed">
            <div class="category-icon">🔥</div>
            <h3>الأكثر طلبًا</h3>
          </article>
          <article class="category-card category-card-wide" data-action="open-smart-categories">
            <div class="category-icon">⌗</div>
            <h3>تصفح المنتجات حسب الأصناف</h3>
            <div class="smart-tags">
              ${buildSmartCategoryChips().map((chip) => `<span class="smart-chip">${escapeHtml(chip)}</span>`).join('')}
            </div>
          </article>
        </div>
      </section>

      <section class="shelf-card" id="companiesShelf">
        <div class="shelf-head">
          <div>
            <h2>الشركات المتاحة</h2>
            <div class="helper-text">اختيار سريع للشركات الأكثر حضورًا</div>
          </div>
          <button class="ghost-btn shelf-more" type="button" data-action="open-companies">عرض الكل</button>
        </div>
...
function toggleProductFromCard(productId) {
  ...
  if (!product) return;
  if (!getSelectedTierObject() && state.cart.length === 0) {
    smartToast('tier.missing', 'يجب اختيار شريحتك أولًا', true);
    openTierPrompt();
    navigate('#tiers');
    return;
  }
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (closeTarget === 'tierPromptModal') return closeTierPrompt();
    ...
    if (action === 'open-companies') {
      navigate('#home');
      setTimeout(() => document.getElementById('companiesShelf')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
      return;
    }
    if (action === 'open-categories') {
      navigate('#home');
      setTimeout(() => document.getElementById('categoriesShelf')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
      return;
    }
    if (action === 'open-best-selling') {
      navigate('#home');
      setTimeout(() => document.getElementById('home-best-selling')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
      return;
    }
    if (action === 'open-top-viewed') {
      navigate('#home');
      setTimeout(() => document.getElementById('home-viewed')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
      return;
    }
    if (action === 'open-smart-categories') {
      navigate('#home');
      setTimeout(() => document.getElementById('categoriesShelf')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
      return;
    }
    if (action === 'open-tiers') {
      closeTierPrompt();
      navigate('#tiers');
      return;
    }
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!clickedInsideModal && event.target === els.tierPromptModal) closeTierPrompt();
  ...
  if (els.companiesBtn) els.companiesBtn.addEventListener('click', () => navigate('#home'));
  els.dealsBtn.addEventListener('click', () => navigate('#deals'));
  els.flashBtn.addEventListener('click', () => navigate('#flash'));
  if (els.categoriesBtn) els.categoriesBtn.addEventListener('click', () => {
    const target = document.getElementById('categoriesShelf');
  ...
  document.querySelectorAll('#bottomNav [data-action="goto-home"]').forEach((btn) => btn.addEventListener('click', () => { state.search = ''; navigate('#home'); }));
  document.querySelectorAll('#bottomNav [data-action="goto-tier"]').forEach((btn) => btn.addEventListener('click', () => navigate('#tiers')));
  document.querySelectorAll('#bottomNav [data-action="goto-flash"]').forEach((btn) => btn.addEventListener('click', () => navigate('#flash')));
  document.querySelectorAll('#bottomNav [data-action="quick-shop"]').forEach((btn) => btn.addEventListener('click', () => {
    navigate('#home');
    setTimeout(() => document.getElementById('home-recommended')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
  }));
  document.querySelectorAll('#bottomNav [data-action="account"]').forEach((btn) => btn.addEventListener('click', () => {
    if (!state.session) {
      openLogin();
      return;
    }
...
(function () {
  ...
  function decorateHero() {
    ...
    if (!strip) return;
    if (!strip.querySelector('.theme-switcher')) {
      strip.innerHTML = '';
      strip.appendChild(createThemeSwitcher());
    }
  }
