...
let lastTime = 0;
const SMART_TOAST_COOLDOWN_MS = 30 * 60 * 1000;

function canShowMessage() {
  const now = Date.now();
  if (now - lastTime < SMART_TOAST_COOLDOWN_MS) return false;
  lastTime = now;
  return true;
}
...
function openProductModal(productId) {
  ...
  const unitText = unitDisplayLabel(unit);
...
function resolveProductPrice(product, unit) {
  ...
  if (code !== 'carton') {
    return baseUnitPrice(product, code);
  }

  const productId = String(product?.product_id || '').trim();
  const tierMapPrice = Number(state.tierPrices?.carton?.get(productId) || 0);
  if (Number.isFinite(tierMapPrice) && tierMapPrice > 0) return tierMapPrice;

  const tierPrice = tierUnitPrice(product, code);
...
function syncCartPricesFromCurrentState() {
  ...
  state.cart = state.cart.map((item) => {
    ...
    const unitLabel = unitDisplayLabel(item.unit);
...
function normalizeProductsFromTables(products = [], cartonRows = [], packRows = []) {
  ...
  return sortProducts((products || []).map((product) => {
    ...
    const halfPackPrice = Number(product.half_pack_price ?? 0);
    const piecePrice = Number(product.piece_price ?? 0);
    const sellableUnits = Array.isArray(product.sellable_units) ? product.sellable_units : [];
    ...
    return {
      ...
      has_half_pack: product.has_half_pack ?? halfPackPrice > 0,
      has_piece: product.has_piece ?? piecePrice > 0,
      carton_price: cartonPrice,
      pack_price: packPrice,
      half_pack_price: halfPackPrice,
      piece_price: piecePrice,
      sellable_units: sellableUnits.length ? sellableUnits.map((unit) => String(unit).trim().toLowerCase()) : [
        ...(cartonPrice > 0 ? ['carton'] : []),
        ...(packPrice > 0 ? ['pack'] : []),
        ...(halfPackPrice > 0 ? ['half_pack'] : []),
        ...(piecePrice > 0 ? ['piece'] : []),
      ],
...
async function loadInvoices() {
  ...
  state.invoices = (rows || []).map((row) => ({
    ...row,
    customer_name: row.customer_id ? (customerMap.get(String(row.customer_id))?.name || state.session?.name || 'عميل') : (state.session?.name || 'عميل'),
  }));
  state.invoicesLoaded = true;
}
...
async function loadData() {
  try {
    ...
    const [
      ...
      fetchCollection('api_products', { select: 'product_id,product_name,product_image,company_id,category,has_carton,has_pack,has_half_pack,has_piece,carton_price,pack_price,half_pack_price,piece_price,allow_discount,visible,status,sellable_units', order: 'product_name.asc' }, []),
    ...
    const productRows = normalizeProductsFromTables(
      (apiProducts || []).length ? apiProducts : (rawProducts || []),
      cartonRows || [],
      packRows || []
    );
  ...
  const unitHtml = units.length
    ? units.map((itemUnit) => `
        <button class="unit-chip ${itemUnit === unit ? 'active' : ''}" data-action="set-unit" data-product-id="${escapeHtml(product.product_id)}" data-unit="${escapeHtml(itemUnit)}">
          ${escapeHtml(unitDisplayLabel(itemUnit))}
        </button>
      `).join('')
...
function renderHomePage() {
  ...
  if (els.banner) {
    els.banner.innerHTML = activeFlash ? `
      <section class="banner-card live-hero-card">
        <div class="live-hero-copy">
          ...
          <h1>عرض الساعة</h1>
          <p>خصومات حصرية لفترة محدودة</p>
          <div class="live-timer-shell">
            <div class="live-timer-frame">
              <div class="live-radar"></div>
              <div class="live-timer mono">${escapeHtml(flashState.remaining || '--:--:--')}</div>
              <div class="live-segments">
                <span><strong>ساعة</strong></span>
                <span><strong>دقيقة</strong></span>
                <span><strong>ثانية</strong></span>
              </div>
            </div>
          </div>
          <button class="primary-btn hero-cta" type="button" data-action="open-offer" data-offer-type="flash" data-id="${escapeHtml(activeFlash.id)}">شراء العرض الآن</button>
        </div>
        <div class="live-hero-art">
          <div class="live-product-orbit">
            <img class="live-product-image" src="${escapeHtml(activeFlash.image || appBannerImage() || placeholderImage(activeFlash.title || 'offer'))}" alt="${escapeHtml(activeFlash.title || 'عرض الساعة')}" loading="eager" />
          </div>
        </div>
      </section>
    ` : '';
  ...
  setSearchBarHtml(buildSearchBarHtml('ابحث باسم المنتج أو الكود'));
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            ...
            <div class="helper-text">تشكيل متنوع من أكبر الشركات</div>
          </div>
        </div>
    ...
    <div class="page-stack">
      <section class="section-card">
        <div class="section-head">
          <div>
            <h2>فواتيري</h2>
            <div class="helper-text">رقم الفاتورة، التاريخ، اسم العميل، القيمة، والحالة</div>
          </div>
    ...
    <article class="invoice-card">
      <div class="invoice-grid">
        <div class="invoice-cell">
          ...
          <strong class="invoice-value">${escapeHtml(order.order_number)}</strong>
        </div>
        <div class="invoice-cell">
          <span class="invoice-label">التاريخ</span>
          <strong class="invoice-value">${escapeHtml(formatDateTime(order.created_at))}</strong>
        </div>
        <div class="invoice-cell">
          <span class="invoice-label">اسم العميل</span>
          <strong class="invoice-value">${escapeHtml(order.customer_name || 'غير متاح')}</strong>
        </div>
        <div class="invoice-cell">
          <span class="invoice-label">قيمة الفاتورة</span>
          <strong class="invoice-value mono">${num(order.total_amount)}</strong>
        </div>
        <div class="invoice-cell">
          <span class="invoice-label">الحالة</span>
          <strong class="invoice-value"><span class="status-pill ${statusClass(order.status)}">${escapeHtml(getStatusLabel(order.status))}</span></strong>
        </div>
      </div>
    </article>
  `;
  ...
  return `
    <section class="section-card">
      ...
      <div class="search-row">
        ...
        <button class="mini-btn icon-btn" id="clearSearchBtn" type="button" aria-label="مسح البحث">×</button>
      </div>
    </section>
  `;
}
...
function renderApp() {
  ...
  if (els.mainHeader) els.mainHeader.classList.add('hidden');
...
async function getLiveCheckoutSnapshot() {
  ...
  const [freshProducts, freshTier] = await Promise.all([
    ids.length ? apiGet('api_products', {
      select: 'product_id,product_name,company_id,has_carton,has_pack,has_half_pack,has_piece,carton_price,pack_price,half_pack_price,piece_price,allow_discount,sellable_units',
...
async function validateCartAgainstDatabase() {
  ...
  for (const item of state.cart) {
    ...
    const liveBase = item.unit === 'carton'
      ? Number(product.carton_price || 0)
      : item.unit === 'pack'
        ? Number(product.pack_price || 0)
        : item.unit === 'half_pack'
          ? Number(product.half_pack_price || 0)
          : Number(product.piece_price || 0);
    const liveDiscount = snapshot.tierRow && item.unit === 'carton' && product.allow_discount !== false
...
async function insertOrderItems(orderId) {
  const items = state.cart.map((item) => ({
    ...
    price: Number(item.price || 0),
    unit: ['carton', 'pack', 'half_pack', 'piece'].includes(String(item.unit || '')) ? item.unit : 'piece',
...
function setupMobileFooterNavigation() {
  ...
  const nodes = [els.homeBtn, els.tierBtn, els.dealsBtn, userWrap, els.cartBtn].filter(Boolean);
  nodes.forEach((node) => {
    if (node.parentElement !== footer) footer.appendChild(node);
  });

  ensureFooterButtonMarkup();
...
/* === Runtime commerce stabilization patch === */
const UI_STABILIZATION_STYLE_ID = 'b2b-runtime-ui-styles';

function ensureRuntimeUiStabilizationStyles() {
  if (document.getElementById(UI_STABILIZATION_STYLE_ID)) return;
  const style = document.createElement('style');
  style.id = UI_STABILIZATION_STYLE_ID;
  style.textContent = `
    .search-row.search-row-inline {
      display: flex;
      align-items: center;
      gap: 0.6rem;
      flex-wrap: nowrap;
    ...
    .search-row.search-row-inline input {
      flex: 1 1 auto;
      min-width: 0;
    }
    .search-summary-chip {
    ...
    .search-summary-chip .search-summary-value {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
    ...
    .search-summary-chip .search-summary-currency {
      font-size: clamp(0.68rem, 1.9vw, 0.82rem);
      opacity: 0.88;
      white-space: nowrap;
    }
    .search-summary-chip .search-summary-icon {
      width: 1rem;
      height: 1rem;
      flex: 0 0 auto;
      opacity: 0.9;
    }
    .search-summary-chip svg {
      display: block;
      width: 100%;
      height: 100%;
    }
    .mobile-footer-nav .footer-compact-btn,
    .mobile-footer-nav button,
    .mobile-footer-nav .user-wrap {
      min-width: 0;
      border-radius: 18px;
      overflow: hidden;
      text-overflow: ellipsis;
    ...
    .mobile-footer-nav #cartLabel,
    .mobile-footer-nav #cartValue {
      display: none !important;
    }
    .mobile-footer-nav #cartBtn {
      padding-inline: 0.7rem;
      gap: 0;
    }
    .mobile-footer-nav #cartBtn .button-text,
    .mobile-footer-nav #cartBtn .label,
    .mobile-footer-nav #cartBtn [data-role="label"],
    .mobile-footer-nav #cartBtn [data-role="value"] {
      display: none !important;
    }
    ...
    .invoice-card .invoice-topline {
      display: flex;
      align-items: flex-start;
      justify-content: space-between;
      gap: 1rem;
    }
    .invoice-card .invoice-main {
      min-width: 0;
      display: grid;
      gap: 0.3rem;
    }
    .invoice-card .invoice-meta-row {
      display: flex;
      flex-wrap: wrap;
      gap: 0.5rem 0.75rem;
      color: var(--muted);
    ...
    .invoice-card .invoice-meta-row span {
      min-width: 0;
      overflow: hidden;
      text-overflow: ellipsis;
      white-space: nowrap;
    }
    .invoice-card .invoice-status-row {
      display: flex;
      flex-wrap: wrap;
      gap: 0.5rem;
      align-items: center;
...
const UNIT_META = {
  ...
  half_pack: { label: 'نص دستة', priority: 3 },
  piece: { label: 'قطعة', priority: 4 },
};

function unitDisplayLabel(unit) {
  const code = normalizeUnitCode(unit);
  return UNIT_META[code]?.label || code || '';
}

function unitPriority(unit) {
  const code = normalizeUnitCode(unit);
  return UNIT_META[code]?.priority ?? 99;
}

function sortUnitCodes(units = []) {
  return [...new Set(coerceUnitList(units).map(normalizeUnitCode).filter(Boolean))]
    .sort((a, b) => unitPriority(a) - unitPriority(b) || compareNatural(a, b));
}

function visibleProductCardUnits(product) {
  const fromRuntime = sortUnitCodes(product?.sellable_units);
  if (fromRuntime.length) return fromRuntime;

  const units = [];
  if (product?.has_carton || Number(product?.carton_price) > 0) units.push('carton');
  if (product?.has_pack || Number(product?.pack_price) > 0) units.push('pack');
  if (product?.has_half_pack || Number(product?.half_pack_price) > 0) units.push('half_pack');
  if (product?.has_piece || Number(product?.piece_price) > 0) units.push('piece');
  return sortUnitCodes(units);
}

function productHasSellableUnits(product) {
  return visibleProductCardUnits(product).length > 0;
}

function currentVisibleUnitForProduct(product) {
  const saved = normalizeUnitCode(state.unitPrefs?.[product?.product_id]);
  const available = visibleProductCardUnits(product);
  if (saved && available.includes(saved)) return saved;
  if (available.includes('carton')) return 'carton';
  if (available.includes('pack')) return 'pack';
  if (available.includes('half_pack')) return 'half_pack';
  if (available.includes('piece')) return 'piece';
  return available[0] || null;
}

function availableUnits(product) {
  return visibleProductCardUnits(product);
}

function baseUnitPrice(product, unit) {
  const code = normalizeUnitCode(unit);
  if (code === 'pack') return Number(product?.pack_price || 0);
  if (code === 'half_pack') return Number(product?.half_pack_price || 0);
  if (code === 'piece') return Number(product?.piece_price || 0);
  return Number(product?.carton_price || 0);
}

function tierUnitPrice(product, unit) {
  const code = normalizeUnitCode(unit);
  if (code !== 'carton') return null;
  ...
  const base = baseUnitPrice(product, code);
  if (!base || base <= 0) return null;
...
function unitPricingForProduct(product, unit) {
  const code = normalizeUnitCode(unit);
  const basePrice = baseUnitPrice(product, code);
  if (!Number.isFinite(basePrice) || basePrice <= 0) return null;
  const tierPrice = tierUnitPrice(product, code);
  const finalPrice = Number.isFinite(tierPrice) && tierPrice > 0 ? tierPrice : basePrice;
  return {
    basePrice,
    finalPrice,
    allowSale: true,
    unitCode: code,
    label: unitDisplayLabel(code),
  };
}
...
function displayPriceBlock(product, unit) {
  ...
  if (!pricing) {
    return `
      <div class="price-wrap">
        ...
        <span class="product-sub">${escapeHtml(unitDisplayLabel(unit))}</span>
      </div>
    `;
  }

  const { basePrice, finalPrice, label } = pricing;
  if (finalPrice !== basePrice) {
    return `
      <div class="price-wrap">
        <span class="price-old">${num(basePrice)} ج.م</span>
        <span class="price-new">${num(finalPrice)} ج.م</span>
        <span class="product-sub">${escapeHtml(label)}</span>
      </div>
    `;
  }
  ...
  return `
    <div class="price-wrap">
      <span class="price-main">${num(finalPrice)} ج.م</span>
      <span class="product-sub">${escapeHtml(label)}</span>
    </div>
  `;
}
...
function resolveProductPrice(product, unit) {
  const pricing = unitPricingForProduct(product, unit);
  if (pricing && Number.isFinite(pricing.finalPrice) && pricing.finalPrice > 0) return Number(pricing.finalPrice);
...
function purchaseSummaryCapsuleHtml() {
  return `<button class="purchase-value-chip mono" id="purchaseValueChip" type="button" aria-label="قيمة المشتريات">${integer(cartTotal())}</button>`;
}

function applyFooterCompactMode() {
  const footer = els.mobileFooterNav || document.getElementById('mobileFooterNav');
  if (!footer) return;
  footer.classList.add('footer-compact-mode');
  if (els.cartBtn) {
    els.cartBtn.classList.add('footer-compact-btn');
    els.cartBtn.setAttribute('aria-label', 'السلة');
    els.cartBtn.title = 'السلة';
  }
  if (els.cartLabel) {
    els.cartLabel.textContent = '';
    els.cartLabel.setAttribute('aria-hidden', 'true');
  }
  if (els.cartValue) {
    els.cartValue.textContent = '';
    els.cartValue.setAttribute('aria-hidden', 'true');
  }
}
...
function updateHeader() {
  const tierLabel = getSelectedTierLabel();
  if (els.tierBtn) {
    els.tierBtn.title = tierLabel;
    els.tierBtn.setAttribute('aria-label', 'اختر شريحتك');
    els.tierBtn.dataset.currentTier = tierLabel;
  }
  if (els.userBtn) {
    els.userBtn.title = state.session ? getSessionLabel() : 'تسجيل دخول';
  }
  if (els.cartValue) {
    els.cartValue.textContent = integer(cartTotal());
    els.cartValue.setAttribute('aria-hidden', 'false');
  }
  const purchaseChip = document.getElementById('purchaseValueChip');
  if (purchaseChip) purchaseChip.textContent = integer(cartTotal());
  updateFlashHeader();
  syncCheckoutButton();
}
...
function productCardHtml(product) {
  ...
  const key = unit ? cartKey({ type: 'product', id: product.product_id, unit }) : null;
  ...
  return `
    <article class="product-card ${unit ? '' : 'is-unavailable'}" data-product-id="${escapeHtml(product.product_id)}">
      <button class="product-image-wrap" type="button" data-action="open-product" data-product-id="${escapeHtml(product.product_id)}">
        <img class="product-image" src="${escapeHtml(image)}" alt="${escapeHtml(product.product_name)}" loading="lazy" />
      </button>
      <div class="product-body">
        ...
        ${unit ? displayPriceBlock(product, unit) : `<div class="price-wrap"><span class="price-main">غير متاح</span><span class="product-sub">لا توجد وحدة بسعر صالح</span></div>`}
        <label class="qty-pick">
          <span class="qty-label">الكمية</span>
          <input
            type="number"
...
function insertOrderItems(orderId) {
  const items = state.cart.map((item) => ({
    ...
    qty: Number(item.qty || 1),
    price: Number(item.price || 0),
    unit: item.type === 'product'
      ? (['carton', 'pack', 'half_pack', 'piece'].includes(String(item.unit)) ? item.unit : 'piece')
      : (item.unit || 'single'),
    type: item.type === 'deal' ? 'deal' : item.type === 'flash' ? 'flash' : 'product',
  }));
  if (!items.length) return;
  return apiPost('order_items', items);
}
...
async function loadInvoices() {
  ...
  const customerRows = customerIds.length
    ? await apiGet('customers', { select: 'id,name,phone', id: `in.(${customerIds.join(',')})` }).catch(() => [])
    : [];
  const customerMap = new Map((customerRows || []).map((row) => [String(row.id), row]));

  state.invoices = (rows || []).map((row) => ({
    ...row,
    customer_name: customerMap.get(String(row.customer_id))?.name || row.customer_name || '',
  }));
  state.invoicesLoaded = true;
}
...
function renderSearchablePageWrapper(title, subtitle) {
  return `
    <section class="section-card">
      ...
      <div class="search-row">
        <input id="searchInput" type="search" placeholder="ابحث" value="${escapeHtml(state.search)}" />
        <button class="purchase-value-chip mono" id="purchaseValueChip" type="button" aria-label="قيمة المشتريات">${integer(cartTotal())}</button>
        <button class="mini-btn icon-btn" id="clearSearchBtn" type="button" aria-label="مسح البحث">×</button>
      </div>
    </section>
  `;
...
async function loadData() {
  try {
    ...
    const [
      ...
      fetchCollection('companies', { select: 'company_id,company_name,company_logo,visible,allow_discount', visible: 'eq.true', order: 'company_id.asc' }, []),
      fetchCollection('api_products', {
        select: 'product_id,product_name,company_id,product_image,carton_price,pack_price,half_pack_price,piece_price,has_carton,has_pack,has_half_pack,has_piece,allow_discount,visible,status,category,sellable_units',
    ...
    const companyRows = (companies || []).length ? companies : state.companies;
    const productRowsSource = (apiProductsView || []).length ? apiProductsView : (rawProducts || []);
    const productRows = normalizeProductsFromTables(
      productRowsSource,
      cartonRows || [],
      packRows || []
    ...
    state.settingMap = new Map((state.settings || []).map((row) => [row.key, row.value]));
    state.topProducts = Array.isArray(topProductsView) ? topProductsView : [];
    state.topCompanies = Array.isArray(topCompaniesView) ? topCompaniesView : [];
    state.companyMap = new Map(state.companies.map((c) => [String(c.company_id), c]));

    // Keep runtime tier data available for legacy flows, but pack discounts are disabled in UI.
    state.tierPrices = {
      carton: pickTierPrice(cartonRows, tierKey),
      pack: pickTierPrice(packRows, tierKey),
    };
...
function normalizeProductsFromTables(products = [], cartonRows = [], packRows = []) {
  ...
  return sortProducts((products || []).map((product) => {
    ...
    const sellableUnits = sortUnitCodes(baseCandidate.sellable_units?.length ? baseCandidate.sellable_units : visibleProductCardUnits(baseCandidate));
...
function openProductModal(productId) {
  ...
  const unit = currentVisibleUnitForProduct(product);
  const inCart = unit ? Boolean(findCartItem(cartKey({ type: 'product', id: product.product_id, unit }))) : false;
  const priceText = unit ? displayPriceText(product, unit) : 'غير متاح';
  const unitText = unitDisplayLabel(unit) || 'لا توجد وحدة سعر صالح';
  ...
  els.cartItems.innerHTML = state.cart.map((item) => `
    <div class="cart-item">
      ...
      <div>
        <h4 class="cart-title">${escapeHtml(item.title)}</h4>
        <div class="cart-meta">${escapeHtml(item.company || '')}</div>
        <div class="cart-price">${escapeHtml(item.unitLabel || unitDisplayLabel(item.unit) || item.unit || '')} · ${num(item.price)} ج.م</div>
        ${item.unit === 'single' ? `
          <div class="qty-row single-item-row">
            <button class="ghost-btn" data-action="remove-item" data-key="${escapeHtml(item.key)}" type="button">إزالة</button>
          </div>
...
async function validateCartAgainstDatabase() {
  ...
  for (const item of state.cart) {
    ...
    const liveBase = baseUnitPrice(product, unit);
    const livePrice = unit === 'carton' && snapshot.tierRow && product.allow_discount !== false
      ? Number((liveBase * (1 - Number(snapshot.tierRow.discount_percent || 0) / 100)).toFixed(2))
      : liveBase;
