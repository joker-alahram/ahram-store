...
function homeHeroState() {
  ...
  if (activeFlash) {
    return {
      ...
      note: activeFlash.description || (activeFlash.stock ? `المتبقي ${num(activeFlash.stock, 0)} قطعة` : 'عرض محدود للشراء السريع'),
      badge: 'Flash Hero',
      actionLabel: 'فتح عرض الساعة',
      actionHash: '#flash',
      secondaryLabel: 'الصفقة التالية',
      secondaryHash: '#deals',
      image: activeFlash.image || banner,
      metricLabel: activeFlash.stock ? 'المخزون المتبقي' : 'المتبقي الآن',
      metricValue: activeFlash.stock ? `${num(activeFlash.stock, 0)} قطعة` : flashState.remaining || '—',
      progress: 0,
      accent: 'flash',
    };
  }
  ...
  if (selectedTier) {
    return {
      ...
      note: `المؤهل ${num(eligible)} ج.م · المتبقي ${num(remaining)} ج.م للوصول إلى الحد الأدنى`,
  ...
  return {
    ...
    note: 'الوصول السريع للشراء، الشريحة، وصفقة اليوم، والشركات من مكان واحد',
    badge: 'B2B Commerce',
...
function renderHomePage() {
  ...
  const bannerHtml = hero.image
    ? `<img class="banner-image" src="${escapeHtml(hero.image)}" alt="بانر الصفحة الرئيسية" loading="eager" />`
