...
function renderHeaderSearchBar(label = 'ابحث عن منتج أو شركة') {
  setSearchBarHtml(`
    <section class="section-card search-card search-surface">
      <div class="search-row">
        <button class="mini-btn icon-btn search-icon-btn" type="button" aria-label="البحث" data-action="focus-search">⌕</button>
        <input id="searchInput" type="search" placeholder="${escapeHtml(label)}" value="${escapeHtml(state.search)}" />
...
function surfaceHeroHtml() {
  const flashState = getFlashState();
  const flashCount = Math.max(0, filteredFlashOffers().length || state.flashOffers.length || 0);
  const currentTier = getSelectedTierObject();
  const tierLabel = getSelectedTierLabel();
  const remainingValue = tierRemainingAmount();
  const tierMin = selectedTierMinimum();
  const tierProgress = tierMin > 0 && currentTier ? Math.max(0, Math.min(100, ((tierMin - (remainingValue || 0)) / tierMin) * 100)) : 0;

  if (flashState.status === 'active' && flashState.offer) {
    const offer = flashState.offer;
    return `
      <section class="surface-hero flash-hero-surface">
        <div class="surface-hero-copy">
          <div class="hero-kicker">Flash Offer</div>
          <h1>عرض الساعة مباشر</h1>
          <p>متبقي ${flashCount} عروض · العد التنازلي داخل الشاشة</p>
          <div class="hero-metrics">
            <div class="hero-metric">
              <b class="mono">${escapeHtml(flashState.remaining || '--:--:--')}</b>
              <small>الوقت المتبقي</small>
            </div>
            <div class="hero-metric">
              <b>${integer(flashCount)}</b>
              <small>متبقي ${flashCount} عروض</small>
            </div>
          </div>
          <div class="hero-actions">
            <button class="primary-btn" type="button" data-action="goto-flash">استعرض العرض الآن</button>
            <button class="ghost-btn" type="button" data-action="shelf-more" data-hash="#flash">عرض التفاصيل</button>
          </div>
        </div>
        <div class="surface-hero-art">
          <img class="surface-hero-image" src="${escapeHtml(offer.image || placeholderImage(offer.title))}" alt="${escapeHtml(offer.title)}" loading="eager" />
        </div>
      </section>
    `;
  }
  ...
  return `
    <section class="surface-hero tier-hero-surface">
      <div class="surface-hero-copy">
        <div class="hero-kicker">Tier Progress</div>
        <h1>${escapeHtml(currentTier ? tierLabel : 'شريحتك التجارية')}</h1>
        <p>${escapeHtml(currentTier ? tierStatusLine() : 'اختر شريحة تجارية لبدء التسعير')}</p>
        <div class="hero-metrics">
          <div class="hero-metric">
            <b>${escapeHtml(currentTier ? `${num(remainingValue || 0)} ج.م` : '—')}</b>
            <small>المتبقي</small>
          </div>
          <div class="hero-metric">
            <b>${escapeHtml(currentTier ? `${num(tierProgress)}%` : '0%')}</b>
            <small>مؤشر التقدم</small>
          </div>
        </div>
        <div class="hero-actions">
          <button class="primary-btn" type="button" data-action="open-tier-modal">اختيار الشريحة</button>
          <button class="ghost-btn" type="button" data-action="shelf-more" data-hash="#tiers">عرض الشرائح</button>
        </div>
      </div>
      <div class="surface-hero-art">
        <div class="tier-hero-panel">
          <div class="tier-hero-line">
            <span class="tier-hero-label">الشريحة الحالية</span>
            <strong>${escapeHtml(currentTier ? tierLabel : 'غير محددة')}</strong>
          </div>
          <div class="tier-hero-track">
            <span class="tier-hero-fill" style="width:${tierProgress}%"></span>
          </div>
          <div class="tier-hero-stats">
            <span><b>${escapeHtml(currentTier ? `${num(tierMin)} ج.م` : '—')}</b><small>الحد الأدنى</small></span>
            <span><b>${escapeHtml(currentTier ? `${num(remainingValue || 0)} ج.م` : '—')}</b><small>متبقي</small></span>
          </div>
        </div>
      </div>
    </section>
...
function brandStripHtml() {
  const currentTier = getSelectedTierObject();
  const tierLabel = getSelectedTierLabel();
  const dealCount = filteredDailyDeals().length;
  const flashCount = Math.max(0, filteredFlashOffers().length || state.flashOffers.length || 0);
  return `
    <section class="brand-strip">
      <div class="brand-strip-copy">
        <div class="brand-strip-title">الأهرام للتجارة والتوزيع</div>
        <div class="brand-strip-sub">B2B Operating Interface · Mobile only</div>
      </div>
      <div class="brand-strip-pills">
        <span class="brand-pill">${escapeHtml(currentTier ? tierLabel : 'بدون شريحة')}</span>
        <span class="brand-pill">${integer(dealCount)} صفقة</span>
        <span class="brand-pill">${integer(flashCount)} Flash</span>
      </div>
    </section>
  `;
}
...
function categoryGridHtml() {
  ...
  return `
    <section class="category-surface section-card">
      <div class="section-head">
        <div>
          <h2>الأقسام</h2>
          <div class="helper-text">شبكة تشغيلية سريعة للوصول إلى الفئات الأساسية</div>
        </div>
      </div>
      <div class="category-grid">
        ${categories.map(([title, sub]) => `
          <button class="category-card" type="button" data-action="category-filter" data-category="${escapeHtml(title)}">
...
function renderHomePage() {
  ...
  const dealRows = filteredDailyDeals();

  renderHeaderSearchBar('ابحث عن منتج أو شركة');
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack mobile-surface-stack">
      <section class="section-card compact-deal-surface">
        <div class="section-head">
          <div>
            <h2>صفقة اليوم</h2>
            <div class="helper-text">باكدج يومي مضغوط داخل شاشة الموبايل</div>
          </div>
          <button class="ghost-btn shelf-more" type="button" data-action="shelf-more" data-hash="#deals">عرض المزيد</button>
        </div>
        <div class="deal-list compact-deal-list">
          ${dealRows.length ? dealRows.slice(0, 1).map((deal) => dealCardHtml(deal, 'deal')).join('') : `<div class="empty-state">لا توجد صفقة اليوم الآن</div>`}
        </div>
      </section>
      ...
      <section class="section-card">
        <div class="section-head">
          <div>
            <h2>الشركات</h2>
            <div class="helper-text">شبكة حقيقية 2-عمود داخل الـ viewport</div>
          </div>
        </div>
        <div class="company-grid">
          ${companyRows.length ? companyRows.slice(0, 6).map((company) => `
...
function renderCompanyPage() {
  ...
  renderHeaderSearchBar(title ? `ابحث داخل ${title}` : 'ابحث باسم المنتج أو الكود');
...
function renderTierPage() {
  const current = tierName();
  renderHeaderSearchBar('ابحث عن الشريحة أو منتج');
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="section-card">
        <div class="section-head">
          <div>
            <h2>اختيار الشريحة التجارية</h2>
            <div class="helper-text">سياق التسعير التشغيلي للموبايل فقط</div>
          </div>
        </div>
      </section>
...
function renderDealsPage() {
  renderHeaderSearchBar('ابحث عن صفقة اليوم');
    ...
    <div class="page-stack">
      <section class="section-card">
        <div class="section-head">
          <div>
            <h2>صفقة اليوم</h2>
            <div class="helper-text">باكدج مضغوط وجاهز للشراء</div>
          </div>
        </div>
      </section>
...
function renderFlashPage() {
  ...
  const flashCount = Math.max(0, filteredFlashOffers().length || state.flashOffers.length || 0);
  renderHeaderSearchBar('ابحث عن عرض الساعة');
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="flash-hero flash-page-hero">
...
function renderApp() {
  ...
  if (els.banner && state.view.type !== 'home' && !isRegister) els.banner.innerHTML = '';
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'category-filter') {
      state.search = target.getAttribute('data-category') || '';
      renderApp();
      return;
    }
  ...
  if (els.menuBtn) els.menuBtn.addEventListener('click', (event) => {
  ...
  if (els.dealsBtn) els.dealsBtn.addEventListener('click', () => navigate('#deals'));
