...
(function () {
        ...
        <section class="exact-offers-grid">
          ...
          <article class="exact-offer-card exact-offer-flash" data-action="open-offer" data-offer-type="flash" data-id="${escapeHtml(flashOffer?.id || state.flashOffers[0]?.id || '')}">
            ...
            <div>
              ...
              <div class="exact-flash-countdown mono">${escapeHtml(flashText)}</div>
  ...
  document.addEventListener('click', (event) => {
    const target = event.target.closest('[data-action="scroll-companies"], [data-action="open-deals-page"]');
    if (!target) return;
    const action = target.getAttribute('data-action');
  ...
  if (bottomCheckoutBtn) {
    bottomCheckoutBtn.addEventListener('click', async () => {
      await handleCheckout();
    });
  }
