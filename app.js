...
function openProductModal(productId) {
  ...
  const priceValue = resolveProductPrice(product, unit);
  ...
  els.productModalBody.innerHTML = `
    <div class="modal-hero">
      ...
      <div class="modal-meta">
        ...
        ${Number.isFinite(priceValue) && priceValue > 0 ? `<div class="price-main">${escapeHtml(priceText)} ج.م</div>` : `<div class="price-main price-unavailable">غير متاح</div>`}
        <p class="helper-text">${escapeHtml(Number.isFinite(priceValue) && priceValue > 0 ? 'السعر من api_products فقط' : 'لا يوجد سعر متاح لهذه الوحدة الآن')}</p>
      </div>
    </div>
  `;

  els.productModalAction.textContent = inCart ? 'إزالة من السلة' : (Number.isFinite(priceValue) && priceValue > 0 ? 'شراء' : 'غير متاح');
  ...
  return `
    <article class="product-tile" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body compact">
        ...
        ${Number.isFinite(priceValue) && priceValue > 0 ? `<div class="price-wrap"><span class="price-main">${num(priceValue)} ج.م</span></div>` : `<div class="price-wrap price-unavailable"><span class="product-sub">غير متاح</span></div>`}
        <div class="card-actions compact-actions">
          <button class="primary-btn" data-action="toggle-product" data-product-id="${escapeHtml(product.product_id)}">${inCart ? 'إزالة من السلة' : (resolveProductPrice(product, unit) ? escapeHtml(actionLabel) : 'غير متاح')}</button>
        </div>
      </div>
    </article>
  `;
...
function getLoadedPrice(productId, unit) {
  const priceMap = state.priceRuntime?.prices?.get(String(productId || '').trim());
  if (!priceMap) return null;
  const price = priceMap.get(String(unit || '').trim());
...
function availableUnits(product) {
  const units = [];
  const cartonPrice = getLoadedPrice(product?.product_id, 'carton');
  const packPrice = getLoadedPrice(product?.product_id, 'pack');
  if (Number.isFinite(cartonPrice) && cartonPrice > 0) units.push('carton');
  if (Number.isFinite(packPrice) && packPrice > 0) units.push('pack');
  return units;
}
...
function baseUnitPrice(product, unit) {
  return resolveProductPrice(product, unit);
}
...
function displayPriceBlock(product, unit) {
  ...
  if (!(Number.isFinite(price) && price > 0)) {
    return `
      <div class="price-wrap price-unavailable">
        <span class="product-sub">غير متاح</span>
      </div>
    `;
  }
  ...
  return `
    <div class="price-wrap">
      <span class="price-main">${num(price)} ج.م</span>
...
function resolveProductPrice(product, unit) {
  ...
  if (!productId || !unitCode) return null;

  const price = state.priceRuntime?.prices?.get(productId)?.get(unitCode);
...
function displayPriceText(product, unit) {
  const price = resolveProductPrice(product, unit);
  return price === null ? 'غير متاح' : num(price);
}
...
async function loadTierPrices(tier) {
  const tierNameValue = String(tier?.tier_name || tierName() || 'base').trim() || 'base';
  if (state.priceRuntime?.loadedTier === tierNameValue) return;
  if (state.priceRuntime?.loadingTier === tierNameValue) return;

  state.priceRuntime.loadingTier = tierNameValue;

  const rows = await fetchCollection('api_products', {
    select: 'product_id,unit_code,tier_name,final_price',
    tier_name: `eq.${tierNameValue}`,
  ...
  for (const row of rows || []) {
    ...
    if (!productId || !unitCode || !Number.isFinite(finalPrice) || finalPrice <= 0) continue;
    if (!prices.has(productId)) prices.set(productId, new Map());
    prices.get(productId).set(unitCode, Number(finalPrice.toFixed(2)));
  }

  state.priceRuntime = {
    loadedTier: tierNameValue,
    loadingTier: null,
    prices,
  };
...
function normalizeProductsFromTables(products = []) {
  return sortProducts((products || []).map((product) => ({
    ...
    has_carton: product.has_carton !== false,
...
async function loadData() {
  try {
    const tierKey = String(tierName() || 'base').trim() || 'base';
    const [
      companies,
      productsRaw,
      dailyDealsView,
      ...
      fetchCollection('api_products', { select: 'product_id,unit_code,tier_name,final_price', tier_name: `eq.${tierKey}`, order: 'product_id.asc,unit_code.asc' }, []),
    ...
    const productRows = normalizeProductsFromTables(Array.isArray(productsRaw) ? productsRaw : []);
    ...
    state.tiers = Array.isArray(tiers) && tiers.length ? tiers : state.tiers;
    state.settings = Array.isArray(settings) && settings.length ? settings : state.settings;
    ...
    for (const row of apiPrices || []) {
      const productId = String(row?.product_id || '').trim();
      const unitCode = String(row?.unit_code || '').trim();
      const finalPrice = Number(row?.final_price);
      if (!productId || !unitCode || !Number.isFinite(finalPrice) || finalPrice <= 0) continue;
      if (!priceMap.has(productId)) priceMap.set(productId, new Map());
      priceMap.get(productId).set(unitCode, Number(finalPrice.toFixed(2)));
    }
    state.priceRuntime = {
      loadedTier: tierKey,
      loadingTier: null,
      ...
      <div class="product-body">
        ...
        <div class="card-actions">
          <button class="primary-btn" data-action="toggle-product" data-product-id="${escapeHtml(product.product_id)}">
            ${inCart ? 'إزالة من السلة' : (resolveProductPrice(product, unit) ? 'شراء' : 'غير متاح')}
          </button>
        </div>
      </div>
    </article>
...
async function handleSelectTier(tier) {
  ...
  if (current === tier.tier_name) {
    ...
    await loadTierPrices({ tier_name: 'base' });
    resetCheckoutStage();
    syncCartPricesFromCurrentState();
    renderCart();
    renderApp();
...
function toggleProductFromCard(productId) {
  ...
  const price = resolveProductPrice(product, unit);
  if (!(Number.isFinite(price) && price > 0)) {
    smartToast('cart.unavailable', 'هذا المنتج غير متاح بالسعر الحالي', true);
    return;
  }
...
async function getLiveCheckoutSnapshot() {
  ...
  const tierKey = String(tierName() || 'base').trim() || 'base';
  const rows = ids.length ? await apiGet('api_products', {
    select: 'product_id,unit_code,tier_name,final_price',
    product_id: `in.(${ids.join(',')})`,
    tier_name: `eq.${tierKey}`,
    order: 'product_id.asc,unit_code.asc',
  }).catch(() => []) : [];
  ...
  for (const row of rows || []) {
    ...
    productsMap.get(productId).set(unitCode, Number(finalPrice.toFixed(2)));
  }

  const tierRow = tierKey ? (state.tiers || []).find((tier) => String(tier.tier_name || '').trim() === tierKey) || null : null;
...
async function validateCartAgainstDatabase() {
  ...
  for (const item of state.cart) {
    ...
    if (!productUnits) {
      return { ok: false, message: `المنتج ${item.title} غير متاح الآن` };
    }

    const livePrice = Number(productUnits.get(String(item.unit || '').trim()) || 0);
