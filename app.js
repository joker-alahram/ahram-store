...
function renderCompanyPage() {
  ...
  setSearchBarHtml(`
    <section class="section-card search-card">
      ...
      <div class="search-row">
        <button class="mini-btn icon-btn" type="button" aria-hidden="true" tabindex="-1">⌕</button>
...
function renderThemeSwitcherHost() {
  const host = els.themeSwitcherHost || document.getElementById('themeSwitcherHost');
  if (!host) return;

  const THEME_PRESETS = [
    { key: 'gold', label: 'gold', color: '#D4AF37' },
    { key: 'navy', label: 'navy', color: '#1B4D8C' },
    { key: 'emerald', label: 'emerald', color: '#00A86B' },
    { key: 'orange', label: 'orange', color: '#FF7A00' },
    { key: 'black', label: 'black', color: '#000000' },
    { key: 'white', label: 'white', color: '#F7F7F7' },
  ];

  const current = String(localStorage.getItem('b2b_theme') || document.body.dataset.theme || 'gold').trim().toLowerCase();
  host.innerHTML = `
    <div class="theme-switcher" role="group" aria-label="اختيار الثيم">
      ${THEME_PRESETS.map((theme) => `
        <button
          type="button"
          class="theme-dot ${theme.key === current ? 'active' : ''}"
...
function syncMobileCartRail() {
  ...
  const total = integer(cartTotal());
  if (els.mobileCartCount) els.mobileCartCount.textContent = integer(count);
  if (els.mobileCartTotal) els.mobileCartTotal.textContent = `${total} ج.م`;
  if (els.mobileCartRail) els.mobileCartRail.setAttribute('aria-label', `السلة، ${integer(count)} منتج، الإجمالي ${total} جنيه`);
}
...
function scheduleHeaderCompactState() {
  if (window.__b2bHeaderScrollRaf) return;
  window.__b2bHeaderScrollRaf = requestAnimationFrame(() => {
    setHeaderCompactState();
    window.__b2bHeaderScrollRaf = null;
  });
}
...
function updateHeader() {
  ...
  if (els.tierBtn) {
    els.tierBtn.textContent = 'اختار شريحتك';
    els.tierBtn.title = state.selectedTier ? `${tierLabel} · اختار شريحتك` : 'اختار شريحتك';
    els.tierBtn.setAttribute('aria-label', state.selectedTier ? `اختار شريحتك، الشريحة الحالية ${tierLabel}` : 'اختار شريحتك');
  }

  if (els.tierCurrentLabel) {
    els.tierCurrentLabel.textContent = state.selectedTier ? tierLabel : 'لم تُختر شريحة بعد';
  }

  if (els.userBtn) els.userBtn.textContent = getSessionLabel();
  if (els.cartLabel) els.cartLabel.textContent = `${integer(cartCount)} منتج`;
  if (els.cartValue) els.cartValue.textContent = `${integer(cartAmount)} ج.م`;
  if (els.cartBtn) els.cartBtn.setAttribute('aria-label', `${integer(cartCount)} منتج، الإجمالي ${integer(cartAmount)} جنيه`);

  renderThemeSwitcherHost();
  syncMobileCartRail();
  updateFlashHeader();
  syncCheckoutButton();
  setHeaderCompactState();
...
function contactStripHtml() {
  return `
    <div class="contact-strip" aria-label="وسائل التواصل السريع">
      <a class="contact-icon-btn" href="https://wa.me/${escapeHtml(CONFIG.supportWhatsapp)}" target="_blank" rel="noopener noreferrer" aria-label="واتساب" title="واتساب">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 0 0-8.6 14.8L2 22l5.4-1.4A10 10 0 1 0 12 2zm5.7 14.5c-.2.6-1 1-1.5 1.1-.4.1-.9.1-1.5 0-.3-.1-.7-.2-1.2-.4-2-.9-3.3-2.2-4.2-4.2-.2-.5-.3-.9-.4-1.2-.1-.6-.1-1.1 0-1.5.1-.5.5-1.3 1.1-1.5l.8-.2c.3-.1.6 0 .8.2l1 1c.2.2.3.6.2.9l-.4 1c-.1.2-.1.4 0 .6.4.8.9 1.4 1.7 1.7.2.1.4.1.6 0l1-.4c.3-.1.7 0 .9.2l1 1c.2.2.2.5.2.8l-.1.8z"/></svg>
      </a>
      <a class="contact-icon-btn" href="https://www.facebook.com/" target="_blank" rel="noopener noreferrer" aria-label="فيسبوك" title="فيسبوك">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M14 8.5V7c0-.8.5-1 1-1h1V3h-2c-2.7 0-4 2-4 4.5V8H8v3h2v10h4V11h2.6l.4-3H14z"/></svg>
      </a>
      <a class="contact-icon-btn" href="tel:01040880002" aria-label="اتصال" title="اتصال">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.6 10.8c1.4 2.8 3.4 4.8 6.2 6.2l2.1-2.1c.3-.3.7-.4 1.1-.3 1.2.4 2.6.6 3.9.6.6 0 1 .4 1 1v3.3c0 .6-.4 1-1 1C10.8 20.5 3.5 13.2 3.5 4c0-.6.4-1 1-1h3.3c.6 0 1 .4 1 1 0 1.4.2 2.7.6 3.9.1.4 0 .8-.3 1.1l-1.5 1.8z"/></svg>
      </a>
    </div>
  `;
...
function renderFeatureStrip() {
  const features = [
    { kind: 'shield', title: 'منتجات أصلية', sub: 'جودة مضمونة 100%' },
    { kind: 'truck', title: 'شحن سريع', sub: 'لكافة المحافظات' },
    { kind: 'refresh', title: 'ضمان استرجاع', sub: 'خلال 14 يوم' },
    { kind: 'support', title: 'دعم العملاء', sub: 'متاح 24/7' },
  ];

  const icon = (kind) => {
    const icons = {
      shield: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2 5 5v6c0 4.9 3.2 9.3 7 11 3.8-1.7 7-6.1 7-11V5z"></path><path d="m9 12 2 2 4-5"></path></svg>`,
      truck: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7h11v8H3z"></path><path d="M14 10h3l3 3v2h-6z"></path><circle cx="7" cy="17" r="2"></circle><circle cx="17" cy="17" r="2"></circle></svg>`,
      refresh: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 12a8 8 0 0 0-14-5"></path><path d="M6 3v4h4"></path><path d="M4 12a8 8 0 0 0 14 5"></path><path d="M18 21v-4h-4"></path></svg>`,
      support: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12a8 8 0 1 1 16 0v4H4z"></path><path d="M8 16v2a4 4 0 0 0 8 0v-2"></path></svg>`,
    };
    return icons[kind] || icons.support;
  };
    ...
    <section class="feature-strip">
      ${features.map((item) => `
        <article class="feature-card">
          <span class="feature-ico" aria-hidden="true">${icon(item.kind)}</span>
          <div class="feature-copy">
            <div class="feature-title">${escapeHtml(item.title)}</div>
            <div class="feature-sub">${escapeHtml(item.sub)}</div>
          </div>
        </article>
      `).join('')}
    </section>
...
function renderPremiumFooter() {
  const whatsapp = `https://wa.me/${escapeHtml(CONFIG.supportWhatsapp)}`;
  return `
      ...
      <div class="footer-grid">
        <div class="footer-col">
          ...
          <div class="footer-links">
            <span class="helper-text">منتجات أصلية</span>
            <span class="helper-text">شحن سريع</span>
            <span class="helper-text">استرجاع مرن</span>
          </div>
        </div>
        <div class="footer-col">
          <h3>حسابك</h3>
          <div class="footer-links">
            <button type="button" data-action="my-data">الملف الشخصي</button>
        ...
        <div class="footer-col">
          <h3>روابط مهمة</h3>
          <div class="footer-links">
            <button type="button" data-action="home">الرئيسية</button>
            <button type="button" data-action="cart">السلة</button>
            <button type="button" data-action="shelf-more" data-hash="#deals">العروض</button>
          </div>
        </div>
        <div class="footer-col">
          <h3>تواصل معنا</h3>
...
function renderOfferShelf(title, subLabel, items, type = 'deal', options = {}) {
  const list = (items || []).filter(Boolean);
  const moreHash = options.moreHash || '#home';
  const moreLabel = options.moreLabel || 'عرض الكل';
  return `
    <section class="shelf-card">
      <div class="shelf-head">
        <div>
        ...
        <button class="ghost-btn shelf-more" type="button" data-action="shelf-more" data-hash="${escapeHtml(moreHash)}">${escapeHtml(moreLabel)}</button>
      </div>
      <div class="deal-list home-offer-grid">
        ${list.length ? list.map((item) => dealCardHtml(item, type)).join('') : `<div class="empty-state">لا توجد عناصر الآن</div>`}
      </div>
    </section>
  `;
}
...
function renderHomePage() {
  const banner = appBannerImage();
  const q = normalizeText(state.search);
  const companyRows = q ? filteredCompanies() : bestCompanyCards(8);
  const recommended = topRecommendedProducts(8);
  const viewed = topViewedProducts(8);
  const bestSelling = topBestSellingProducts(8);
  const companion = cartCompanionProducts(8);
  const specialOffers = filteredDailyDeals().slice(0, 4);
  const todayDeal = filteredDailyDeals().slice(0, 1);
  const flashShelf = filteredFlashOffers().slice(0, 2);
  ...
  if (els.banner) {
    els.banner.innerHTML = `
      <section class="banner-card home-hero">
        <div class="home-hero-copy">
          <div class="hero-kicker">B2B PREMIUM COMMERCE</div>
          <h1>الأهرام للتجارة والتوزيع</h1>
          <p>حلول توزيع احترافية للتجار والشركات مع رحلة شراء أسرع، أوضح، وأكثر قابلية للتحويل.</p>
          <div class="hero-actions">
            <button class="primary-btn hero-cta" type="button" data-action="home-cta">تسوق الآن</button>
            <button class="ghost-btn" type="button" data-action="shelf-more" data-hash="#deals">العروض القوية</button>
          </div>
          <div class="hero-pills">
            <span class="badge">شحن سريع</span>
            <span class="badge">منتجات أصلية</span>
            <span class="badge">دعم 24/7</span>
          </div>
        </div>
        <div class="home-hero-art">
          ${bannerHtml}
    ...
    <div class="page-stack">
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            <h2>الشركات المتاحة</h2>
            <div class="helper-text">تصفح الشركات أولًا ثم انتقل مباشرة إلى المنتجات</div>
          </div>
        </div>
        <div class="company-shelf">
          ${companyRows.length ? companyRows.map((company) => `
            <article class="company-card company-card-home" data-action="open-company" data-company-id="${escapeHtml(company.company_id)}">
              <img class="company-logo" src="${escapeHtml(company.company_logo || placeholderImage(company.company_name))}" alt="${escapeHtml(company.company_name)}" loading="lazy" />
              <div class="company-name">${escapeHtml(company.company_name)}</div>
              <button class="ghost-btn company-btn" type="button">عرض المنتجات</button>
            </article>
          `).join('') : `<div class="empty-state">لا توجد شركات ظاهرة الآن</div>`}
        </div>
      </section>
      ...
      <section>
        ${renderProductShelf('الأكثر مشاهدة','المنتجات الأكثر تصفحًا', q ? viewed.filter((p) => matchesHomeSearch(p, q)) : viewed, { kind: 'viewed', moreHash: '#home', moreLabel: 'عرض المزيد', badge: 'الأكثر مشاهدة', actionLabel: 'شراء' })}
      </section>

      <section>
        ${renderProductShelf('الأكثر مبيعًا','الأعلى طلبًا من العملاء', q ? bestSelling.filter((p) => matchesHomeSearch(p, q)) : bestSelling, { kind: 'bestseller', moreHash: '#home', moreLabel: 'عرض المزيد', badge: 'الأكثر مبيعًا', actionLabel: 'شراء' })}
      </section>

      <section>
        ${renderProductShelf('منتجات موصى بها لك','مختارة بناءً على نشاطك', q ? recommended.filter((p) => matchesHomeSearch(p, q)) : recommended, { kind: 'recommended', moreHash: '#home', moreLabel: 'عرض الكل', badge: 'موصى به', actionLabel: 'شراء' })}
      </section>

      <section>
        ${renderProductShelf('مقترحات تناسب سلتك','منتجات تكمل طلبك الحالي', q ? companion.filter((p) => matchesHomeSearch(p, q)) : companion, { kind: 'cart-match', moreHash: '#cart', moreLabel: 'عرض السلة', badge: 'مكمّل للسلة', actionLabel: 'أضف مع الطلب' })}
      </section>

      ${renderOfferShelf('عروض خاصة','مختارات ساخنة من العروض', specialOffers, 'deal', { moreHash: '#deals', moreLabel: 'عرض كل العروض' })}
      ${renderOfferShelf('صفقة اليوم','أوضح فرصة بيع الآن', todayDeal, 'deal', { moreHash: '#deals', moreLabel: 'تفاصيل الصفقة' })}
      ${renderOfferShelf('عرض الساعة','المؤقت المباشر', flashShelf, 'flash', { moreHash: '#flash', moreLabel: 'افتح العرض' })}
...
(function installPremiumLayoutNow() {
  renderThemeSwitcherHost();
  setHeaderCompactState();
  syncMobileCartRail();
  window.addEventListener('scroll', scheduleHeaderCompactState, { passive: true });
  window.addEventListener('resize', scheduleHeaderCompactState, { passive: true });
})();
