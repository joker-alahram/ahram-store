...
function companyCardHtml(company) {
  ...
  const badge = companyId ? escapeHtml(companyId) : '';
  ...
  return `
    <button class="company-card company-card-home" type="button" data-action="open-company" data-company-id="${escapeHtml(companyId)}" aria-label="فتح شركة ${title}">
      ...
      ${badge ? `<div class="badge company-badge">${badge}</div>` : ''}
    </button>
  `;
}
...
function renderHomePage() {
  ...
  const recommended = state.dataReady ? topRecommendedProducts(8) : [];
  const viewed = state.dataReady ? topViewedProducts(8) : [];
  const bestSelling = state.dataReady ? topBestSellingProducts(8) : [];
  const companion = state.dataReady ? cartCompanionProducts(8) : [];
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      ...
      ` : `
      <section class="shelf-card" id="home-recommended">
        <div class="shelf-head">
          <div>
            <h2>المقترحات المميزة</h2>
            <div class="helper-text">منتجات مرتفعة التفاعل ومناسبة للتسعير الحالي</div>
          </div>
          <button class="ghost-btn shelf-more" type="button" data-action="shelf-more" data-hash="#home">عرض الكل</button>
        </div>
        <div class="shelf-grid shelf-grid-recommended">
          ${recommended.length ? recommended.map((product) => compactProductCardHtml(product)).join('') : `<div class="empty-state">لا توجد منتجات بعد</div>`}
        </div>
      </section>
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            <h2>الشركات</h2>
            <div class="helper-text">تصفح مباشرة حسب الشركة</div>
          </div>
        </div>
        <div class="company-grid">
          ${companyRows.length ? companyRows.map(companyCardHtml).join('') : `<div class="empty-state">لا توجد شركات مطابقة</div>`}
        </div>
      </section>
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            <h2>الأكثر مشاهدة</h2>
            <div class="helper-text">ترتيب مباشر بناء على التفاعل</div>
          </div>
        </div>
        <div class="shelf-grid shelf-grid-viewed">
          ${viewed.length ? viewed.map((product) => compactProductCardHtml(product)).join('') : `<div class="empty-state">لا توجد بيانات مشاهدة بعد</div>`}
        </div>
      </section>
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            <h2>الأكثر مبيعًا</h2>
            <div class="helper-text">يعرض المنتجات ذات النشاط الأقوى</div>
          </div>
        </div>
        <div class="shelf-grid shelf-grid-selling">
          ${bestSelling.length ? bestSelling.map((product) => compactProductCardHtml(product)).join('') : `<div class="empty-state">لا توجد بيانات مبيعات بعد</div>`}
        </div>
      </section>
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            <h2>تكملة السلة</h2>
            <div class="helper-text">اقتراحات قريبة من سلة العميل الحالية</div>
          </div>
        </div>
        <div class="shelf-grid shelf-grid-companion">
          ${companion.length ? companion.map((product) => compactProductCardHtml(product)).join('') : `<div class="empty-state">أضف منتجات أولًا</div>`}
        </div>
      </section>
      `}
    </div>
