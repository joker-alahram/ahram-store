...
function bindClick(el, label, handler) {
  if (!el) {
    console.log(`[bind] skipped ${label}: missing element`);
    return false;
  }
  console.log(`[bind] ${label}`, el);
  el.addEventListener('click', handler);
  return true;
}

function bindSafeEvent(el, type, label, handler, options) {
  if (!el) {
    console.log(`[bind] skipped ${label}: missing element`);
    return false;
  }
...
function placeholderImage(seed = 'item') {
  const label = encodeURIComponent(String(seed).slice(0, 24) || 'item');
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="640" height="640" viewBox="0 0 640 640">
      ...
      <circle cx="320" cy="238" r="86" fill="#d4af37" opacity="0.16"/>
      <path d="M140 440h360l-52-124-70 78-84-96-72 84-44-42-38 100z" fill="#d4af37" opacity="0.18"/>
      <text x="50%" y="52%" text-anchor="middle" fill="#f3d88b" font-family="Arial, sans-serif" font-size="34" font-weight="700">${label}</text>
    </svg>`;
...
function toggleUserMenu(force) {
  ...
  document.body.classList.toggle('menu-open', Boolean(next));
}
...
function updateHeader() {
  ...
  if (els.tierBtn) {
    els.tierBtn.textContent = tier ? 'تغيير' : 'اختيار';
...
function syncLiveCountdowns() {
  const flashState = getFlashState();
  const liveText = flashState.status === 'active'
    ? (flashState.remaining || '--:--:--')
    : flashState.status === 'expired'
      ? (flashState.endedAt || '')
      : (flashState.remaining || '--:--:--');

  document.querySelectorAll('[data-live-countdown="flash"]').forEach((node) => {
    if (!node) return;
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="flash-hero">
        ...
        <div class="flash-countdown mono" data-live-countdown="flash">${escapeHtml(heroCountdown || '--:--:--')}</div>
    ...
    <div class="cart-summary">
      <div class="cart-summary-grid">
        <div class="cart-summary-card">
          <span class="cart-summary-label">إجمالي السلة الكامل</span>
          <strong class="cart-summary-value mono">${num(fullTotal)} ج.م</strong>
        </div>
        <div class="cart-summary-card">
          <span class="cart-summary-label">إجمالي مؤهل للشريحة</span>
          <strong class="cart-summary-value mono">${num(eligibleTotal)} ج.م</strong>
        </div>
        <div class="cart-summary-card ${tier ? '' : 'is-muted'}">
          <span class="cart-summary-label">${tier ? 'المتبقي للوصول للحد الأدنى' : 'اختر شريحة أولاً'}</span>
          <strong class="cart-summary-value mono">${tier ? `${num(remaining)} ج.م` : '—'}</strong>
        </div>
      </div>
    </div>
  `;
...
async function createOrder() {
  ...
  console.log('[checkout] createOrder response', res);
...
async function insertOrderItems(orderId) {
  ...
  console.log('[checkout] insertOrderItems payload', items);
  const res = await apiPost('order_items', items);
  console.log('[checkout] insertOrderItems response', res);
}
...
function buildWhatsAppUrl(order, items) {
  const phone = String(CONFIG.supportWhatsapp || '').trim();
  const text = buildWhatsAppInvoice(order, items);
  return `https://wa.me/${phone}?text=${text}`;
}

function sendToWhatsApp(order, items, popup = null) {
  ...
  if (popup && !popup.closed) {
    try {
      ...
      console.warn('[checkout] whatsapp popup navigation failed', error);
    }
  }
...
async function handleCheckout() {
  ...
  try {
    console.log('[checkout] click received', {
      sessionId: state.session?.id || null,
      userType: state.session?.userType || null,
      cartCount: state.cart.length,
      cartTotal: cartTotal(),
      eligibleTotal: eligibleTierTotal(),
    });

    whatsappPopup = window.open('about:blank', '_blank');
    if (whatsappPopup) {
      whatsappPopup.document.write('<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>جارٍ إرسال الطلب...</title></head><body style="font-family:sans-serif;padding:24px">جارٍ إرسال الطلب...</body></html>');
      whatsappPopup.document.close();
    }

    const liveCheck = await validateCartAgainstDatabase();
    console.log('[checkout] live validation result', liveCheck);
    if (!liveCheck.ok) {
      if (whatsappPopup && !whatsappPopup.closed) whatsappPopup.close();
      smartToast('checkout.blocked', liveCheck.message || 'تعذر التحقق من الطلب', true);
      return;
    }
    ...
    console.log('[checkout] ORDER ITEMS INSERTED', { orderId: order.id, items: state.cart.length });

    const whatsappUrl = sendToWhatsApp(order, state.cart, whatsappPopup);
    console.log('[checkout] WhatsApp URL opened', whatsappUrl);

    recordUiEvent('checkout.submit', {
      orderId: order.id,
...
function wireGlobalEvents() {
  console.log('[wireGlobalEvents] document click');
  document.addEventListener('click', async (event) => {
    const origin = event.target instanceof Element ? event.target : null;
    const target = origin ? origin.closest('[data-action], [data-close], [data-company-id]') : null;
    if (!target) return;
    ...
    if (closeTarget === 'userMenu') return toggleUserMenu(false);
  ...
  console.log('[wireGlobalEvents] document input');
  ...
  console.log('[wireGlobalEvents] document keydown');
  ...
  console.log('[wireGlobalEvents] image error capture');
  document.addEventListener('error', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLImageElement)) return;
    const seed = target.getAttribute('alt') || target.getAttribute('data-fallback-seed') || 'item';
    const fallback = placeholderImage(seed);
    if (target.src !== fallback) {
      target.src = fallback;
    }
  }, true);

  console.log('[wireGlobalEvents] document click outside');
  document.addEventListener('click', (event) => {
    const target = event.target instanceof Element ? event.target : null;
    const clickedInsideUser = target ? target.closest('.user-wrap') : null;
    if (!clickedInsideUser) toggleUserMenu(false);
    const clickedInsideModal = target ? target.closest('.modal-card') : null;
  ...
  bindClick(els.homeBtn, 'homeBtn', () => { state.search = ''; navigate('#home'); });
  bindClick(els.tierBtn, 'tierBtn', () => openTierModal());
  bindClick(els.dealsBtn, 'dealsBtn', () => navigate('#deals'));
  bindClick(els.flashBtn, 'flashBtn', () => navigate('#flash'));
  bindClick(els.cartBtn, 'cartBtn', openCart);
  bindClick(els.userBtn, 'userBtn', (event) => {
  ...
  bindClick(els.submitLogin, 'submitLogin', handleLogin);
  const openRegisterBtn = document.getElementById('openRegister');
  bindClick(openRegisterBtn, 'openRegisterBtn', openRegisterPage);
  if (els.checkoutBtn) {
    console.log('[bind] checkoutBtn', els.checkoutBtn);
    els.checkoutBtn.onclick = async () => {
      await handleCheckout();
    };
  } else {
    console.log('[bind] skipped checkoutBtn: missing element');
  }
  bindClick(els.saveCartBtn, 'saveCartBtn', () => {
    persistCart();
    toast('تم حفظ السلة');
  });
}
...
async function init() {
  ...
  console.log('[init] wireGlobalEvents');
  wireGlobalEvents();
  renderCart();
  updateHeader();
  await loadData();
...
if (document.readyState === 'loading') {
  ...
  console.log('[bootstrap] DOM already ready');
  init().catch((error) => console.error('[init] failed', error));
}
...
/* === EXACT HOME UI OVERRIDES === */
(function () {
  const originalRenderHomePage = renderHomePage;
  const originalUpdateHeader = updateHeader;
  const originalRenderCart = renderCart;
  const originalRenderApp = renderApp;
  const WA_NUMBER = String(CONFIG.supportWhatsapp || '201040880002').replace(/\D/g, '') || '201040880002';

  function cartBadgeText() {
    return `${state.cart.length || 0} منتجات في السلة`;
  }

  function syncFloatingElements() {
    const bottomCount = document.getElementById('bottomCartCount');
    const bottomLabel = document.getElementById('bottomCartLabel');
    const bottomTotal = document.getElementById('bottomCartTotal');
    const whatsapp = document.getElementById('floatingWhatsapp');
    const totals = typeof cartTotals === 'function' ? cartTotals() : { total: 0 };
    const total = Number(totals?.total || 0);

    if (bottomCount) bottomCount.textContent = String(state.cart.length || 0);
    if (bottomLabel) bottomLabel.textContent = cartBadgeText();
    if (bottomTotal) bottomTotal.textContent = `${integer(total)} ج.م`;
    if (whatsapp) whatsapp.href = `https://wa.me/${WA_NUMBER}`;
  }

  function setHomeSearchBar() {
    setSearchBarHtml(`
      <section class="section-card search-card">
        <div class="exact-search-row">
          <svg class="exact-search-icon" viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M10 4a6 6 0 1 0 3.7 10.7l4.8 4.8 1.4-1.4-4.8-4.8A6 6 0 0 0 10 4Zm0 2a4 4 0 1 1 0 8a4 4 0 0 1 0-8Z"/></svg>
          <input id="searchInput" class="exact-search-input" type="search" placeholder="ابحث باسم المنتج أو الشركة..." value="${escapeHtml(state.search)}" />
          <button class="mini-btn icon-btn" id="clearSearchBtn" type="button">×</button>
        </div>
      </section>
    `);
  ...
  function heroImage() {
    return appBannerImage() || placeholderImage('warehouse');
  }
  ...
  renderHomePage = function () {
    ...
    const q = normalizeText(state.search);
    const companyRows = q ? filteredCompanies() : sortCompanies(state.companies);
    const bestSelling = q ? topBestSellingProducts(8).filter((p) => matchesHomeSearch(p, q)) : topBestSellingProducts(8);
    const dailyDeal = filteredDailyDeals()[0] || state.dailyDeals[0] || null;
    const flashState = getFlashState();
    const flashOffer = flashState.offer || filteredFlashOffers()[0] || state.flashOffers[0] || null;
    const tier = getSelectedTierObject();
    const minOrder = selectedTierMinimum();
    const eligibleTotal = Number(eligibleTierTotal() || 0);
    const remaining = tier ? Math.max(0, minOrder - eligibleTotal) : 0;
    const progress = tier && minOrder > 0 ? Math.min(100, Math.max(0, Math.round((eligibleTotal / minOrder) * 100))) : 0;

    if (els.banner) {
      els.banner.innerHTML = `
        <section class="banner-card home-hero">
          <div class="exact-hero-wrap">
            <div class="exact-hero-media">
              <img src="${escapeHtml(banner)}" alt="الأهرام للتجارة والتوزيع" loading="eager" />
            </div>
            <h1 class="exact-hero-title"><span class="accent">الأهرام</span> للتجارة والتوزيع</h1>
            <p class="exact-hero-subtitle">حلول توزيع احترافية للتجار والشركات</p>
            <button class="primary-btn exact-hero-cta" type="button" data-action="home-cta">تسوق الآن</button>
          </div>
        </section>
      `;
    }
    ...
    const flashText = flashState.status === 'active' ? (flashState.remaining || '--:--:--') : '--:--:--';

    els.pageContent.innerHTML = `
      <div class="page-stack">
        <section class="exact-offers-grid">
          <article class="exact-offer-card exact-offer-deal" data-action="open-offer" data-offer-type="deal" data-id="${escapeHtml(dailyDeal?.id || state.dailyDeals[0]?.id || '')}">
            <div class="exact-offer-icon">🎁</div>
            <div>
              <h3 class="exact-offer-title">صفقة اليوم</h3>
              <p class="exact-offer-note">فرصة يومية بسعر قوي</p>
            </div>
            <button class="primary-btn exact-offer-btn" type="button">تسوق الصفقة</button>
          </article>

          <article class="exact-offer-card exact-offer-flash" data-action="open-offer" data-offer-type="flash" data-id="${escapeHtml(flashOffer?.id || state.flashOffers[0]?.id || '')}">
            ${flashState.status === 'active' ? '<span class="exact-offer-live">LIVE</span>' : ''}
            <div class="exact-offer-icon">⚡</div>
            <div>
              <h3 class="exact-offer-title">عرض الساعة</h3>
              <div class="exact-flash-countdown mono" data-live-countdown="flash">${escapeHtml(flashText)}</div>
              <p class="exact-offer-note">${escapeHtml(flashOffer?.description || 'عرض محدود لمدة قصيرة')}</p>
            </div>
            <button class="primary-btn exact-offer-btn" type="button">تسوق العرض</button>
          </article>
        </section>

        <section class="exact-tier-card">
          <div class="exact-tier-head">
            <div>
              <h2>فئة الأسعار الحالية</h2>
              <div class="helper-text">الشريحة التجارية الحالية</div>
            </div>
            <button class="ghost-btn exact-tier-chip" type="button" data-action="open-tier-modal">تغيير الشريحة</button>
          </div>
          <div class="exact-tier-name">${escapeHtml(tierDisplayLabel(tier || { tier_name: 'base' }))}</div>
          <div class="exact-tier-label">بدون خصم</div>
          <div class="exact-tier-stats">
            <div class="exact-stat">
              <span>الحد الأدنى</span>
              <strong>${num(minOrder)} ج.م</strong>
            </div>
            <div class="exact-stat">
              <span>المؤهل الحالي</span>
              <strong class="mono">${num(eligibleTotal)} ج.م</strong>
            </div>
            <div class="exact-stat">
              <span>المتبقي</span>
              <strong>${tier ? `${num(remaining)} ج.م` : '—'}</strong>
            </div>
          </div>
          <div class="exact-progress" style="--progress:${progress}%"><span></span></div>
          <div class="exact-progress-meta">${progress}% من الحد الأدنى</div>
        </section>

        <section class="exact-nav-grid" aria-label="التنقل السريع">
          <button class="exact-nav-item" type="button" data-action="home-cta">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M4 12.5L12 5l8 7.5V20a1 1 0 0 1-1 1h-4v-6H9v6H5a1 1 0 0 1-1-1v-7.5Z"/></svg>
            <span>الرئيسية</span>
          </button>
          <button class="exact-nav-item" type="button" data-action="open-tiers-page">
          ...
          <button class="exact-nav-item" type="button" data-action="open-deals-page">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M12 2 2 7l10 5 8-4v5h2V7L12 2Zm-7 8v3l7 4 7-4v-3l-7 4-7-4Z"/></svg>
            <span>العروض</span>
          </button>
          <button class="exact-nav-item" type="button" data-action="scroll-companies">
            <svg viewBox="0 0 24 24" aria-hidden="true"><path fill="currentColor" d="M4 20V4h16v16H4Zm2-2h12V6H6v12Zm2-8h8v2H8v-2Zm0 4h8v2H8v-2Z"/></svg>
            <span>الشركات</span>
          </button>
        </section>

        <section class="exact-banner">
          <img src="${escapeHtml(banner)}" alt="توزيع احترافي" loading="lazy" />
          <div class="exact-banner-overlay">
            <div class="exact-banner-copy">
              <h3>توزيع احترافي</h3>
              <p>جودة · سرعة · ثقة</p>
            </div>
          </div>
          <div class="exact-dots" aria-hidden="true">
            <span class="active"></span><span></span><span></span><span></span>
          </div>
        </section>

        <section id="home-best-selling">
          <div class="exact-products-head">
            <h2>الأكثر مبيعًا</h2>
            <button type="button" data-action="shelf-more" data-hash="#home">عرض الكل</button>
          </div>
          <div class="grid product-grid">
            ${bestSelling.length ? bestSelling.map(productCardHtml).join('') : `<div class="empty-state">لا توجد عناصر بعد</div>`}
          </div>
        </section>
        ...
        <section>
          <div class="exact-products-head">
            <h2>الشركات</h2>
            <button type="button" data-action="scroll-companies">عرض الكل</button>
          </div>
          <div class="company-shelf">
  ...
  console.log('[wireGlobalEvents] document quick actions');
  document.addEventListener('click', (event) => {
    const origin = event.target instanceof Element ? event.target : null;
    const target = origin ? origin.closest('[data-action="scroll-companies"], [data-action="open-deals-page"]') : null;
    if (!target) return;
    const action = target.getAttribute('data-action');
    if (action === 'scroll-companies') {
      document.querySelector('.company-shelf')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'open-deals-page') {
      navigate('#deals');
      return;
    }
  });
  ...
  console.log('[wireGlobalEvents] bottomCheckoutBtn', bottomCheckoutBtn);
  bindSafeEvent(bottomCheckoutBtn, 'click', 'bottomCheckoutBtn', async () => {
    await handleCheckout();
  });

  const menuBtn = document.getElementById('menuBtn');
  console.log('[wireGlobalEvents] menuBtn', menuBtn);
  bindSafeEvent(menuBtn, 'click', 'menuBtn', (event) => {
    event.stopPropagation();
    if (!state.session) {
      openLogin();
      return;
  ...
  console.log('[wireGlobalEvents] brandHomeLink', brandHomeLink);
  bindSafeEvent(brandHomeLink, 'click', 'brandHomeLink', (event) => {
    event.preventDefault();
    navigate('#home');
  });
