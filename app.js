...
const MESSAGE_ALIASES = [
  [/^سجّل الدخول أولًا$/, 'يرجى تسجيل الدخول أولاً قبل المتابعة.'],
  [/^سجل الدخول أولًا$/, 'يرجى تسجيل الدخول أولاً قبل المتابعة.'],
  [/^أدخل البيانات كاملة$/, 'يرجى استكمال جميع الحقول المطلوبة قبل المتابعة.'],
  [/^المستخدم غير موجود$/, 'بيانات تسجيل الدخول غير صحيحة، يرجى التحقق والمحاولة مرة أخرى.'],
  [/^كلمة المرور غير صحيحة$/, 'بيانات تسجيل الدخول غير صحيحة، يرجى التحقق والمحاولة مرة أخرى.'],
  [/^هذه الصفحة للمندوب فقط$/, 'هذه الصفحة متاحة للمندوب فقط.'],
  [/^إضافة العملاء متاحة للمندوب فقط$/, 'هذه العملية متاحة للمندوب فقط.'],
  [/^اسم العميل مطلوب$/, 'يرجى إدخال اسم العميل.'],
  [/^تعذر إضافة العميل$/, 'تعذر إضافة العميل، يرجى المحاولة مرة أخرى.'],
  [/^تعذر تحميل البيانات من قاعدة البيانات$/, 'تعذر تحميل البيانات من قاعدة البيانات حالياً.'],
  [/^الموقع غير مدعوم على الجهاز$/, 'ميزة تحديد الموقع غير مدعومة على هذا الجهاز.'],
  [/^تعذر تحديد الموقع$/, 'تعذر تحديد الموقع، يرجى المحاولة مرة أخرى.'],
  [/^صفقة اليوم غير متاحة$/, 'صفقة اليوم غير متاحة حالياً.'],
  [/^العرض غير متاح$/, 'عرض الساعة غير متاح حالياً.'],
  [/^فشل إرسال الطلب$/, 'تعذر إرسال الطلب، يرجى المحاولة مرة أخرى.'],
  [/^تم تسجيل الخروج$/, 'تم تسجيل الخروج بنجاح.'],
  [/^تم حفظ السلة$/, 'تم حفظ السلة بنجاح.'],
  [/^تمت الإزالة من السلة$/, 'تمت إزالة الصنف من السلة.'],
  [/^تمت الإضافة للسلة$/, 'تمت إضافة الصنف إلى السلة بنجاح.'],
  [/^تم تحديد الموقع$/, 'تم تحديد الموقع بنجاح.'],
  [/^تم إرسال الطلب بنجاح$/, 'تم إرسال الطلب بنجاح.'],
];

function normalizeSystemMessage(message) {
  let value = String(message ?? '').trim();
  for (const [pattern, replacement] of MESSAGE_ALIASES) {
...
function toast(message) {
  els.toast.textContent = sanitizeToastMessage(normalizeSystemMessage(message));
...
const SYSTEM_MESSAGES = {
  ...
  'login.error': 'بيانات تسجيل الدخول غير صحيحة، يرجى التحقق والمحاولة مرة أخرى.',
  'login.missing': 'يرجى استكمال بيانات تسجيل الدخول قبل المتابعة.',
  'register.success': 'تم إنشاء الحساب بنجاح. يمكنك الآن متابعة الطلب.',
  'register.error': 'يرجى استكمال جميع الحقول المطلوبة قبل المتابعة.',
  'register.duplicate': 'هذا الرقم مسجل مسبقاً، يرجى استخدام بيانات الدخول.',
  'cart.empty': 'لا يمكن إتمام الطلب، السلة فارغة. يرجى إضافة منتجات أولاً.',
  'cart.add': 'تمت إضافة الصنف إلى السلة بنجاح.',
  'cart.remove': 'تمت إزالة الصنف من السلة.',
  'cart.idle': 'السلة بحاجة إلى إضافة أصناف قبل الإرسال.',
  'cart.filled': 'تم تجهيز السلة بنجاح.',
  'tier.missing': 'يرجى اختيار الشريحة المناسبة لعرض الأسعار بشكل صحيح.',
  'tier.incomplete': 'الطلب لم يصل إلى الحد الأدنى المطلوب للشريحة.',
  'tier.almost': 'أصبح الطلب قريباً من حد الشريحة المطلوب.',
  'tier.selected': 'تم اختيار الشريحة بنجاح.',
  'deals.suggest': 'يمكنك مراجعة صفقة اليوم قبل إتمام الطلب.',
  'deals.active': 'عرض الساعة متاح حالياً.',
  'checkout.validate': 'تم التحقق من الطلب بنجاح.',
  'checkout.missing': 'يرجى اختيار العميل قبل إرسال الفاتورة.',
  'checkout.blocked': 'تعذر إتمام الطلب بسبب وجود شرط غير مستوفى.',
  'success.order': 'تم إرسال الطلب بنجاح.',
  'success.whatsapp': 'تم إرسال نسخة الفاتورة إلى واتساب الشركة.',
  ...
  'errors.network': 'تعذر الاتصال بالخدمة حالياً، يرجى المحاولة لاحقاً.'
};
...
function getSmartMessage(type) {
  return SYSTEM_MESSAGES[String(type || '')] || null;
}

function smartToast(typeOrMessage, fallback, force = false) {
  const key = String(typeOrMessage ?? '').trim();
  const message = getSmartMessage(key) || normalizeSystemMessage(fallback ?? typeOrMessage);
...
function cartTotal() {
  return state.cart.reduce((sum, item) => sum + roundMoney(item.price || 0) * Number(item.qty || 0), 0);
}
...
function syncCheckoutButton() {
  ...
  els.checkoutBtn.textContent = 'التحقق من الطلب';
}

function resolveProductPrice(product, unit) {
  const tierPrice = tierUnitPrice(product, unit);
  if (tierPrice !== null && tierPrice > 0) return roundMoney(tierPrice);
  return roundMoney(baseUnitPrice(product, unit));
}
...
function syncCartPricesFromCurrentState() {
  ...
  state.cart = state.cart.map((item) => {
    ...
    const price = roundMoney(resolveProductPrice(product, item.unit));
...
function getSelectedTierObject() {
  if (!state.selectedTier) {
    const baseTier = state.tiers.find((tier) => Number(tier.discount_percent || 0) === 0)
      || state.tiers.find((tier) => /base|رئيسية|أساسي/i.test(String(tier.visible_label || tier.tier_name || '')))
      || state.tiers[0]
      || null;
    return baseTier;
  }
...
function displayPriceBlock(product, unit) {
  const base = roundMoney(baseUnitPrice(product, unit));
...
async function loadData() {
  try {
    ...
    if (state.selectedTier) {
      const matched = state.tiers.find((tier) => tier.tier_name === tierName()) || baseTier || state.selectedTier;
      state.selectedTier = matched;
      saveJSON(STORAGE.tier, matched);
      await loadTierPrices(matched);
    } else if (baseTier) {
      state.selectedTier = baseTier;
      saveJSON(STORAGE.tier, baseTier);
      await loadTierPrices(baseTier);
    }
  ...
  return `
    <article class="invoice-card">
      <div class="invoice-top">
        ...
        <span class="invoice-amount mono">${num(roundMoney(order.total_amount))} </span>
      </div>
      <div class="bad-line">
...
function dealCardHtml(item, type, activeFlash = null) {
  ...
  const price = num(roundMoney(item.price));
  ...
  els.cartItems.innerHTML = state.cart.map((item) => `
    <div class="cart-item">
      ...
      <div>
        ...
        <div class="cart-price">${escapeHtml(item.unitLabel || item.unit || '')} · ${num(roundMoney(item.price))} ج.م</div>
        <div class="qty-row">
...
async function validateCartAgainstDatabase() {
  if (!state.cart.length) {
    return { ok: false, message: 'لا يمكن إتمام الطلب، السلة فارغة. يرجى إضافة منتجات أولاً.', redirect: '#home' };
  }
  ...
  for (const item of state.cart) {
    ...
    if (!product) {
      return { ok: false, message: `المنتج ${item.title} غير متوفر حالياً.`, redirect: '#home' };
    }
    ...
    const livePrice = liveDiscount > 0 && liveBase > 0
      ? roundMoney(liveBase * (1 - liveDiscount / 100))
      : roundMoney(liveBase);

    if (roundMoney(item.price || 0) !== livePrice) {
      return {
        ok: false,
        message: `تم تعديل سعر الصنف ${item.title}، يرجى تحديث السلة من جديد.`,
        redirect: '#home',
      };
    }
    ...
    if (!Number.isFinite(qty) || qty <= 0) {
      return {
        ok: false,
        message: `الكمية الخاصة بالصنف ${item.title} غير صحيحة، يرجى تعديلها.`,
        redirect: '#home',
      };
    }
  }

  if (snapshot.tierRow && cartTotal() < Number(snapshot.tierRow.min_order || 0)) {
    const remaining = Number(snapshot.tierRow.min_order || 0) - cartTotal();
    return {
      ok: false,
      message: `الطلب لم يصل إلى الحد الأدنى المطلوب، متبقي ${num(roundMoney(remaining))} للوصول إلى الشريحة.`,
      redirect: '#home'
    };
  }
...
async function validateBeforeOrder() {
  if (!state.session) {
    closeCart();
    openRegisterPage();
    toast('يجب التسجيل أولاً لإرسال الفاتورة.');
    return { ok: false, redirect: '#register' };
  }

  if (!state.cart.length) {
    toast('لا يمكن إتمام الطلب، السلة فارغة. يرجى إضافة منتجات أولاً.');
  ...
  if (state.session.userType === 'rep' && !state.selectedCustomer) {
    closeCart();
    navigate('#my-customers');
    toast('يرجى اختيار العميل قبل إرسال الفاتورة.');
    return { ok: false, redirect: '#my-customers' };
  }
  ...
  if (!tier) {
    closeCart();
    navigate('#tiers');
    toast('يرجى اختيار الشريحة المناسبة لعرض الأسعار بشكل صحيح.');
    return { ok: false, redirect: '#tiers' };
  }

  const live = await validateCartAgainstDatabase();
  if (!live.ok) {
    if (live.redirect) {
      closeCart();
      navigate(live.redirect);
    }
    toast(live.message);
    return { ok: false, redirect: live.redirect || '#home' };
  }

  return { ok: true, snapshot: live.snapshot };
}

function buildFinalCartItems(snapshot) {
  return state.cart.map((item) => {
    ...
    const product = snapshot.productsMap.get(String(item.id));
    const base = item.unit === 'pack'
      ? Number(product?.pack_price || 0)
      : Number(product?.carton_price || 0);
    const discount = snapshot.tierRow && product && product.allow_discount !== false
      ? Number(snapshot.tierRow.discount_percent || 0)
      : 0;
    const livePrice = discount > 0 && base > 0
      ? roundMoney(base * (1 - discount / 100))
      : roundMoney(base);

    return {
      ...item,
      title: product?.product_name || item.title,
      image: product?.product_image || item.image || placeholderImage(item.title),
      company: companyName(product?.company_id) || item.company || '',
      price: livePrice,
      unitLabel: item.unit === 'carton' ? 'كرتونة' : 'دستة',
    };
  });
}

function buildOrderPayload(finalItems, orderNumber) {
  const isRep = state.session.userType === 'rep';
  const totals = finalItems.reduce((acc, item) => {
    const line = roundMoney(item.price || 0) * Number(item.qty || 0);
    if (item.type === 'deal') acc.deals_total += line;
    else if (item.type === 'flash') acc.flash_total += line;
    else acc.products_total += line;
    acc.total_amount += line;
    return acc;
  }, { total_amount: 0, products_total: 0, deals_total: 0, flash_total: 0 });
  ...
  return {
    ...
    total_amount: roundMoney(totals.total_amount),
    products_total: roundMoney(totals.products_total),
    deals_total: roundMoney(totals.deals_total),
    flash_total: roundMoney(totals.flash_total),
    status: 'pending',
  };
}

async function getNextInvoiceNumber() {
  const cacheKey = STORAGE.invoiceCounter;
  ...
  const extracted = (rows || [])
    .map((row) => String(row.order_number || '').match(/(\d+)/)?.[1])
    .map((v) => Number(v || 0))
    .filter((n) => Number.isFinite(n) && n >= 20000);

  const latest = extracted.length ? Math.max(...extracted) : 19999;
  const next = Math.max(latest + 1, 20000);
  localStorage.setItem(cacheKey, String(next));
  return `INV-${next}`;
}

async function createOrder(finalItems, orderNumber) {
  const payload = buildOrderPayload(finalItems, orderNumber);
...
async function insertOrderItems(orderId, finalItems) {
...
function buildWhatsAppInvoice(order, items) {
  ...
  const rep = isRep ? session : null;
  const customer = isRep ? state.selectedCustomer : session;

  const typeLine = isRep ? 'مندوب' : 'عميل مباشر';
  const repSection = isRep ? `المندوب:
${rep?.name || ''}
${rep?.phone || ''}
${rep?.location || rep?.address || ''}
━━━━━━━━━━━━━━
` : '';

  const customerSection = `العميل:
${customer?.name || ''}
${customer?.phone || ''}
...
كود الصنف: ${item.id}
الوحدة: ${unitLabel}
سعر الوحدة: ${num(roundMoney(item.price))} جنيه
الكمية: ${item.qty}
الإجمالي: ${num(roundMoney(item.price) * Number(item.qty || 0))} جنيه
...
${num(roundMoney(order.total_amount))} جنيه
━━━━━━━━━━━━━━`;
...
async function handleCheckout() {
  ...
  try {
    const finalItems = buildFinalCartItems(validation.snapshot);
    const orderNumber = await getNextInvoiceNumber();
    const order = await createOrder(finalItems, orderNumber);
    console.log('ORDER CREATED:', order.id);
    await insertOrderItems(order.id, finalItems);

    sendToWhatsApp(order, finalItems);

    toast('تم إرسال الطلب بنجاح.');
    toast('تم إرسال نسخة الفاتورة إلى واتساب الشركة.');
    ...
    resetCheckoutStage();
    syncCheckoutButton();
    toast('تعذر إرسال الطلب، يرجى المحاولة مرة أخرى.');
  }
}
...
async function handleLogin() {
  ...
  if (!identifier || !password) {
    toast('يرجى استكمال جميع الحقول المطلوبة قبل المتابعة.');
    return;
  }
...
function openFacebook() {
  window.open('https://www.facebook.com/alahram2014/', '_blank', 'noopener,noreferrer');
}
...
function callSupport() {
  const phone = CONFIG.supportWhatsapp.startsWith('20')
    ? `+${CONFIG.supportWhatsapp}`
    : `+20${String(CONFIG.supportWhatsapp || '').replace(/^0+/, '')}`;
...
function wireGlobalEvents() {
  ...
  if (els.homeBtn) els.homeBtn.addEventListener('click', openHome);
  if (els.facebookBtn) els.facebookBtn.addEventListener('click', openFacebook);
  if (els.whatsappBtn) els.whatsappBtn.addEventListener('click', openWhatsAppSupport);
  if (els.callBtn) els.callBtn.addEventListener('click', callSupport);
