...
const STORAGE = {
  ...
  lastOrderSignature: 'b2b_last_order_signature',
  lastOrderAt: 'b2b_last_order_at',
};
...
const SYSTEM_MESSAGES = {
  login: {
    ...
    missing: 'يرجى إدخال بيانات تسجيل الدخول كاملة قبل المتابعة.',
  },
  register: {
    success: 'تم إنشاء الحساب بنجاح. يمكنك الآن متابعة الطلب.',
    error: 'يرجى استكمال جميع الحقول المطلوبة قبل المتابعة.',
    duplicate: 'هذا الرقم مسجل مسبقًا، يرجى استخدام بيانات تسجيل الدخول.',
  },
  cart: {
    empty: 'لا يمكن إتمام الطلب، السلة فارغة. يرجى إضافة منتجات أولاً.',
    add: 'تمت إضافة الصنف إلى السلة بنجاح.',
    remove: 'تمت إزالة الصنف من السلة.',
    idle: 'يرجى مراجعة السلة قبل المتابعة إلى الإرسال.',
    filled: 'السلة جاهزة للمراجعة والإرسال.',
  },
  tier: {
    missing: 'تم اعتماد الشريحة الافتراضية لعرض الأسعار بشكل صحيح.',
    incomplete: 'إجمالي الطلب أقل من الحد الأدنى المطلوب لهذه الشريحة.',
    almost: 'أنت قريب من الحد الأدنى المطلوب لهذه الشريحة.',
    selected: 'تم اختيار الشريحة بنجاح.',
  },
  deals: {
    suggest: 'يمكنك مراجعة العروض المتاحة قبل إكمال الطلب.',
    active: 'عرض الساعة متاح حاليًا.',
  },
  checkout: {
    validate: 'تم التحقق من الطلب بنجاح. يرجى تأكيد الإرسال للمتابعة.',
    missing: 'يرجى اختيار العميل المرتبط بالطلب قبل المتابعة.',
    blocked: 'تعذر إتمام الطلب بسبب وجود شرط غير مكتمل.',
    duplicate: 'تم استلام طلب مطابق مؤخرًا. يرجى مراجعة السلة قبل إعادة الإرسال.',
  },
  success: {
    order: 'تم إرسال الطلب بنجاح.',
    whatsapp: 'تم إنشاء نسخة الفاتورة وإرسالها إلى واتساب الشركة.',
    login: 'تم تسجيل الدخول بنجاح.',
    register: 'تم إنشاء الحساب بنجاح.',
  },
  errors: {
    generic: 'حدث خطأ غير متوقع، يرجى المحاولة مرة أخرى.',
    network: 'تعذر الاتصال بالخدمة في الوقت الحالي، يرجى المحاولة لاحقًا.',
  },
};
...
function getSmartPool(type) {
  ...
  return typeof cursor === 'string' ? [cursor] : Array.isArray(cursor) ? cursor : [];
}
...
function smartToast(typeOrMessage, fallback, force = false) {
  ...
  if (!force && !canShowMessage()) return false;
  toast(message);
  return true;
}
...
async function registerCustomer() {
  ...
  try {
    ...
    if (exists?.length) {
      smartToast('register.duplicate', undefined, true);
      return;
    }
    ...
    console.error(error);
    smartToast('errors.generic', undefined, true);
  }
}
...
async function saveCustomer() {
  ...
  try {
    ...
    console.error(error);
    toast('تعذر إضافة العميل، يرجى المحاولة مرة أخرى.');
  }
}
...
function cartTotal() {
  return state.cart.reduce((sum, item) => sum + roundMoney(item.price) * Number(item.qty || 0), 0);
}
...
function updateFlashHeader() {
  ...
  if (!flashState.offer) {
    ...
    els.flashBtn.classList.remove('status-active', 'status-danger', 'has-meta');
    return;
  }
  ...
  if (status === 'active') {
    ...
    els.flashBtn.classList.add('status-active', 'has-meta');
...
function getSelectedTierObject() {
  if (!state.selectedTier) return getBaseTierObject();
...
function displayPriceBlock(product, unit) {
  ...
  const discounted = tierUnitPrice(product, unit) === null ? null : roundMoney(tierUnitPrice(product, unit));
  const unitLabel = unit === 'carton' ? 'كرتونة' : 'دستة';

  if (discounted !== null && discounted < base) {
    return `
      <div class="price-wrap">
        <span class="price-old">${num(base)} جنيه</span>
        <span class="price-new">${num(discounted)} جنيه</span>
  ...
  return `
    <div class="price-wrap">
      <span class="price-main">${num(base)} جنيه</span>
      <span class="product-sub">${unitLabel}</span>
    </div>
  `;
}
...
async function loadInvoices() {
  ...
  state.invoices = rows || [];
  state.invoicesLoaded = true;
  syncInvoiceCounterFromInvoices();
}
...
function contactStripHtml() {
  const rawPhone = String(CONFIG.supportWhatsapp || '').replace(/\D/g, '');
  const phone = rawPhone.startsWith('20') ? rawPhone : `20${rawPhone.replace(/^0+/, '')}`;
  const waLink = phone ? `https://wa.me/${phone}` : '#';
  const callLink = rawPhone ? `tel:${rawPhone}` : '#';
  const facebookLink = 'https://www.facebook.com/alahram2014/';
  return `
    <div class="contact-strip" aria-label="وسائل التواصل">
      <a class="contact-link" href="${escapeHtml(callLink)}" aria-label="الاتصال">☎</a>
      <a class="contact-link" href="${escapeHtml(waLink)}" target="_blank" rel="noopener noreferrer" aria-label="واتساب">واتساب</a>
      <a class="contact-link" href="${escapeHtml(facebookLink)}" target="_blank" rel="noopener noreferrer" aria-label="فيسبوك">f</a>
    </div>
  `;
}
  ...
  return `
    <article class="deal-card">
      ...
      <div class="deal-body">
        ...
        <div class="price-wrap">
          <span class="price-main">${price} جنيه</span>
        </div>
        <div class="card-actions">
  ...
  if (registerLocateBtn) {
    registerLocateBtn.addEventListener('click', () => {
      ...
      if (!navigator.geolocation) {
        toast('ميزة تحديد الموقع غير مدعومة على هذا الجهاز.');
        return;
      }
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          toast('تعذر تحديد الموقع، يرجى المحاولة مرة أخرى.');
        },
...
async function handleSelectTier(tier) {
  ...
  renderApp();
  navigate('#home');
  smartToast('tier.selected', undefined, true);
}
  ...
  els.cartItems.innerHTML = state.cart.map((item) => `
    <div class="cart-item">
      ...
      <div>
        ...
        <div class="cart-price">${escapeHtml(item.unitLabel || item.unit || '')} · ${num(roundMoney(item.price))} جنيه</div>
        <div class="qty-row">
...
async function validateCartAgainstDatabase() {
  if (!state.cart.length) {
    return { ok: false, message: SYSTEM_MESSAGES.cart.empty };
  }

  const snapshot = await getLiveCheckoutSnapshot();
  const liveTier = snapshot.tierRow || getBaseTierObject();
  const validatedItems = [];

  for (const item of state.cart) {
    const normalizedQty = Math.max(1, Math.floor(Number(item.qty || 1)));
    if (!Number.isFinite(normalizedQty) || normalizedQty <= 0) {
      return { ok: false, message: 'يرجى التأكد من أن الكمية أكبر من صفر.' };
    }

    if (item.type !== 'product') {
      validatedItems.push({ ...item, qty: normalizedQty, price: roundMoney(item.price) });
      continue;
    }

    const product = snapshot.productsMap.get(String(item.id));
    if (!product) {
      return { ok: false, message: `المنتج ${item.title} غير موجود الآن، يرجى تحديث السلة.` };
    }

    const liveBase = item.unit === 'pack'
      ? Number(product.pack_price || 0)
      : Number(product.carton_price || 0);
    const liveDiscount = liveTier && product.allow_discount !== false
      ? Number(liveTier.discount_percent || 0)
      : 0;
    const livePrice = liveDiscount > 0 && liveBase > 0
      ? roundMoney(liveBase * (1 - liveDiscount / 100))
      : roundMoney(liveBase);

    if (roundMoney(item.price) !== livePrice) {
      return {
        ok: false,
        message: `تم تعديل سعر ${item.title}. يرجى مراجعة السلة ثم إعادة المحاولة.`,
      };
    }
  ...
  const liveTotal = validatedItems.reduce((sum, item) => sum + roundMoney(item.price) * Number(item.qty || 0), 0);
  const minOrder = roundMoney(liveTier?.min_order || 0);
  if (minOrder > 0 && liveTotal < minOrder) {
    const remaining = minOrder - liveTotal;
    return {
      ok: false,
      message: `الحد الأدنى لهذه الشريحة لم يكتمل بعد. المتبقي ${num(remaining)} جنيه.`,
    };
  }

  return { ok: true, snapshot, validatedItems, liveTier, liveTotal };
}
...
function syncInvoiceCounterFromInvoices() {
  const existing = Number(localStorage.getItem(STORAGE.invoiceCounter) || 0);
  const highestFromInvoices = (state.invoices || []).reduce((max, order) => {
    const match = String(order.order_number || '').match(/(\d+)/);
    if (!match) return max;
    return Math.max(max, Number(match[1]) || 0);
  }, 19999);
  const next = Math.max(20000, existing || 20000, highestFromInvoices + 1);
  localStorage.setItem(STORAGE.invoiceCounter, String(next));
  return next;
}

function allocateInvoiceNumber() {
  const next = syncInvoiceCounterFromInvoices();
  localStorage.setItem(STORAGE.invoiceCounter, String(next + 1));
  return `INV-${next}`;
}

function normalizeOrderItemsForSignature(items) {
  return items
    .map((item) => ({
      id: String(item.id),
      type: String(item.type || 'product'),
      unit: String(item.unit || 'single'),
      qty: Math.max(1, Math.floor(Number(item.qty || 1))),
      price: roundMoney(item.price),
    }))
    .sort((a, b) => `${a.type}:${a.id}:${a.unit}`.localeCompare(`${b.type}:${b.id}:${b.unit}`));
}

function buildOrderSignature(items = state.cart) {
  const customerKey = state.session?.userType === 'rep'
    ? `rep:${state.session?.id || ''}:customer:${state.selectedCustomer?.id || ''}`
    : `customer:${state.session?.id || ''}`;
  const tierKey = getSelectedTierObject()?.tier_name || 'base';
  const normalized = normalizeOrderItemsForSignature(items);
  const total = normalized.reduce((sum, item) => sum + item.price * item.qty, 0);
  return JSON.stringify({ customerKey, tierKey, items: normalized, total });
}

function isDuplicateOrderAttempt(signature) {
  const lastSignature = localStorage.getItem(STORAGE.lastOrderSignature) || '';
  const lastAt = Number(localStorage.getItem(STORAGE.lastOrderAt) || 0);
  return Boolean(lastSignature && signature && lastSignature === signature && Date.now() - lastAt < 180000);
}

function rememberOrderSignature(signature) {
  localStorage.setItem(STORAGE.lastOrderSignature, signature);
  localStorage.setItem(STORAGE.lastOrderAt, String(Date.now()));
}

function buildOrderPayload() {
  const isRep = state.session.userType === 'rep';
  const productsTotal = roundMoney(state.cart.filter((item) => item.type === 'product').reduce((sum, item) => sum + roundMoney(item.price) * Number(item.qty || 0), 0));
  const dealsTotal = roundMoney(state.cart.filter((item) => item.type === 'deal').reduce((sum, item) => sum + roundMoney(item.price) * Number(item.qty || 0), 0));
  const flashTotal = roundMoney(state.cart.filter((item) => item.type === 'flash').reduce((sum, item) => sum + roundMoney(item.price) * Number(item.qty || 0), 0));
  const totalAmount = roundMoney(productsTotal + dealsTotal + flashTotal);

  return {
    order_number: allocateInvoiceNumber(),
...
كود: ${i.id}
الوحدة: ${unitLabel}
سعر الوحدة: ${num(roundMoney(i.price))} جنيه
الكمية: ${i.qty}
الإجمالي: ${num(roundMoney(Number(i.qty || 0) * Number(i.price || 0)))} جنيه
...
async function handleCheckout() {
  ...
  try {
    if (state.checkoutStage === 'validate') {
      ...
      state.checkoutSnapshot = validation.snapshot;
      state.cart = validation.validatedItems;
      persistCart();
      renderCart();
      renderApp();
      smartToast('checkout.validate', undefined, true);
    ...
    if (state.checkoutStage === 'submit') {
      ...
      if (isDuplicateOrderAttempt(signature)) {
        smartToast('checkout.duplicate', undefined, true);
        return;
      }

      const confirmed = window.confirm('سيتم حفظ الطلب في قاعدة البيانات وإرسال نسخة الفاتورة إلى واتساب الشركة. هل تريد المتابعة؟');
      if (!confirmed) return;
      ...
      rememberOrderSignature(signature);
      smartToast('success.order', undefined, true);
      smartToast('success.whatsapp', undefined, true);
...
async function handleLogin() {
  ...
  try {
    ...
    if (!user) {
      smartToast('login.error', undefined, true);
      return;
    }
