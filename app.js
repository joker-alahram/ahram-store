...
async function registerCustomer() {
  ...
  try {
    ...
    smartToast('register.success', 'نورتنا ❤️ الحساب اتعمل', true);
    navigate('#home');
  } catch (error) {
    console.error(error);
...
function renderRegisterPage() {
  if (!els.registerPage) return;

  els.registerPage.innerHTML = `
      ...
      <div class="section-card register-card">
        ...
        <form class="register-form" id="registerForm" autocomplete="on">
          ...
          <div class="register-actions">
            <button class="primary-btn" id="registerSubmitBtn" type="button">تحقق وسجل</button>
...
async function handleSelectTier(tier) {
  ...
  renderApp();
  navigate('#home');
  smartToast('tier.selected', `تم اختيار ${tier.visible_label || tier.tier_name}`, true);
}
...
function validateBeforeOrder() {
  if (!state.session) {
    smartToast('login.missing', 'يجب تسجيل الدخول', true);
    return false;
  }
  ...
  if (!tier) {
    smartToast('tier.missing', 'اختار شريحة الأول عشان تاخد خصم 🔥', true);
    return false;
  }
  ...
  if (total < tier.min_order) {
    ...
    smartToast('tier.incomplete', `فاضل ${diff.toFixed(0)} جنيه وتكمل الشريحة 💰`, true);
    return false;
  }
