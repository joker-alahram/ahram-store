...
function renderMyDataContent() {
  ...
  const rows = [
    ['نوع الحساب', s.userType === 'rep' ? 'مندوب' : s.userType === 'customer' ? 'عميل مباشر' : 'غير معروف'],
    ['الاسم', s.name || s.username || 'غير متاح'],
    ['رقم الهاتف', s.phone || 'غير متاح'],
    ['العنوان', s.address || s.location || 'غير متاح'],
    ['الشريحة', tier ? tierDisplayLabel(tier) : 'لم تُحدد'],
    ['حالة السلة', `${integer(cartCount)} منتج`],
    ['الطلبات', `${integer(orderCount)} طلب`],
  ];
  if (!els.myDataContent) return;
  els.myDataContent.innerHTML = `
    <div class="account-summary-grid">
      <div class="account-summary-card">
        <span class="account-summary-label">الحساب</span>
        <strong>${escapeHtml(getSessionLabel())}</strong>
        <small>${escapeHtml(s.userType === 'rep' ? 'مندوب تشغيل' : 'عميل تشغيل مباشر')}</small>
      </div>
      <div class="account-summary-card">
        <span class="account-summary-label">الحالة</span>
        <strong>${escapeHtml(tier ? tierDisplayLabel(tier) : 'بدون شريحة')}</strong>
        <small>${escapeHtml(tier ? tierStatusLine() : 'لم يتم اختيار شريحة بعد')}</small>
      </div>
    </div>
    <div class="profile-grid">${rows.map(([label, value]) => `
      <div class="profile-item">
        <span class="profile-label">${escapeHtml(label)}</span>
        <span class="profile-value">${escapeHtml(value)}</span>
      </div>
    `).join('')}</div>
    <div class="account-actions-rail">
      <button class="primary-btn" type="button" data-action="invoices">مراجعة الحساب</button>
      <button class="ghost-btn" type="button" data-action="${state.session ? 'logout' : 'open-login'}">${state.session ? 'تسجيل الخروج' : 'تسجيل الدخول'}</button>
    </div>
  `;
}
...
function renderFlashPage() {
  ...
  const heroNote = flashState.status === 'active'
    ? `متبقي ${activeOffer?.stock ? integer(activeOffer.stock) : 'X'} عروض`
    : flashState.status === 'expired'
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack">
      <section class="flash-hero">
        <div class="badge-row">
          <span class="badge">${escapeHtml(flashDiscount > 0 ? `خصم ${num(flashDiscount)}%` : 'عرض مباشر')}</span>
...
function renderCart() {
  ...
  const itemCount = state.cart.reduce((sum, item) => sum + Number(item.qty || 1), 0);
