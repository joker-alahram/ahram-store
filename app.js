...
function createRequestTimeoutSignal(timeoutMs = 15000) {
...
async function apiGet(path, params = {}, options = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    const { signal, cancel } = createRequestTimeoutSignal(timeoutMs);
    try {
...
async function fetchCollection(path, params = {}, fallback = []) {
  try {
    const rows = await apiGet(path, params, { timeoutMs: 15000, retries: 1 });
...
async function loadData() {
  ...
  try {
    ...
    void (async () => {
      try {
        const [topProductsView, topCompaniesView] = await Promise.all([
        ...
        if (!(topProductsView || []).length || !(topCompaniesView || []).length) {
          const [salesOrders, salesItems] = await Promise.all([
