...
function commercialStatusHtml() {
  const tier = getSelectedTierObject();
  const tierLabel = tier ? getSelectedTierLabel() : 'الشريحة الرئيسية';
  const discount = Number(tier?.discount_percent || 0);
  const minimum = Number(tier?.min_order || 0);
  const eligible = Number(eligibleTierTotal() || 0);
  const remaining = tier ? Math.max(0, minimum - eligible) : null;
  const qualificationText = tier
    ? (remaining > 0 ? `متبقي ${num(remaining)} ج.م` : 'مؤهل بالكامل')
    : 'اختيار الشريحة مطلوب';

  return `
    <section class="commercial-status-strip" aria-label="الحالة التجارية الحالية">
      <div class="status-card status-card-main">
        <div class="status-card-topline">
          <div>
            <div class="status-label">الشريحة الحالية</div>
            <div class="status-value">${escapeHtml(tierLabel)}</div>
          </div>
          <button class="ghost-btn status-action" type="button" data-action="open-tier-modal">${tier ? 'تغيير' : 'اختيار'}</button>
        </div>
        <div class="status-subline mono">${escapeHtml(tierStatusLine())}</div>
      </div>
      <div class="status-grid">
        <div class="status-card">
          <div class="status-label">نسبة الخصم الحالية</div>
          <div class="status-value mono">${tier ? `${num(discount)}%` : '—'}</div>
        </div>
        <div class="status-card">
          <div class="status-label">المتبقي للتأهل</div>
          <div class="status-value mono">${tier ? `${num(remaining)} ج.م` : '—'}</div>
        </div>
        <div class="status-card">
          <div class="status-label">إجمالي المؤهل</div>
          <div class="status-value mono">${num(eligible)} ج.م</div>
        </div>
        <div class="status-card">
          <div class="status-label">الحد الأدنى للشريحة</div>
          <div class="status-value mono">${tier ? `${num(minimum)} ج.م` : '—'}</div>
        </div>
      </div>
      <button class="ghost-btn status-wide-cta" type="button" data-action="open-tier-modal">إدارة الشريحة التجارية</button>
    </section>
  `;
}
...
function homeQuickActionsHtml() {
  const actions = [
    { label: 'إعادة طلب', target: '#home-most-ordered' },
    { label: 'الأكثر طلبًا', target: '#home-most-ordered' },
    { label: 'الطلبات السابقة', target: '#invoices' },
    { label: 'العروض النشطة', target: '#flash' },
  ];

  return `
    <section class="section-card home-quick-actions" aria-label="إجراءات تجارية سريعة">
      <div class="shelf-head">
        <div>
          <h2>إجراءات سريعة</h2>
          <div class="helper-text">اختصارات تشغيلية للتاجر والموزع</div>
        </div>
      </div>
      <div class="quick-actions-grid">
        ${actions.map((action) => `
          <button class="quick-action-btn" type="button" data-action="home-jump" data-target="${escapeHtml(action.target)}">${escapeHtml(action.label)}</button>
        `).join('')}
      </div>
    </section>
  `;
...
function homeCategoriesHtml() {
  const categories = [
    { label: 'صفقة اليوم', target: '#deals' },
    { label: 'عرض الساعة', target: '#flash' },
    { label: 'الأكثر طلبًا', target: '#home-most-ordered' },
    { label: 'الشركات', target: '#home-companies' },
    { label: 'المنتجات', target: '#home-products' },
    { label: 'الفواتير', target: '#invoices' },
  ];

  return `
    <section class="section-card home-category-shell" aria-label="الأقسام الأساسية">
      <div class="shelf-head">
        <div>
          <h2>الأقسام الأساسية</h2>
          <div class="helper-text">اختصارات سريعة للوصول إلى مناطق القرار التجاري</div>
        </div>
      </div>
      <div class="category-shelf">
        ${categories.map((category) => `
          <button class="category-chip" type="button" data-action="home-jump" data-target="${escapeHtml(category.target)}">${escapeHtml(category.label)}</button>
        `).join('')}
      </div>
    </section>
  `;
...
function homeFeaturedOfferHtml(type, item, options = {}) {
  if (!item) {
    return `
      <section class="section-card home-feature home-feature-${escapeHtml(type)}">
        <div class="shelf-head">
          <div>
            <h2>${escapeHtml(options.title || '')}</h2>
            <div class="helper-text">${escapeHtml(options.subtitle || '')}</div>
          </div>
        </div>
        <div class="empty-state">لا توجد عناصر متاحة الآن</div>
      </section>
    `;
  }

  const title = escapeHtml(options.title || (type === 'flash' ? 'عرض الساعة' : 'صفقة اليوم'));
  const subtitle = escapeHtml(options.subtitle || (type === 'flash' ? 'العرض النشط في أعلى الأولويات التجارية' : 'فرصة مبكرة عالية التحويل'));
  const image = escapeHtml(item.image || placeholderImage(item.title));
  const price = num(item.price);
  const canBuy = Boolean(item.can_buy);
  const statusText = type === 'flash'
    ? (item.status === 'active' ? 'نشط الآن' : item.status === 'expired' ? 'منتهي' : 'قريبًا')
    : (canBuy ? 'متاح الآن' : 'غير متاح');
  ...
  const openAction = type === 'flash' ? 'open-offer' : 'open-offer';
  const toggleAction = type === 'flash' ? 'toggle-flash' : 'toggle-deal';
  const buttonText = !canBuy ? 'انتهى العرض' : (findCartItem(`${type}:${item.id}:single`) ? 'إزالة من السلة' : (options.cta || 'شراء الآن'));
  ...
  return `
    <section class="section-card home-feature home-feature-${escapeHtml(type)}">
      <div class="shelf-head">
        ...
        <button class="ghost-btn shelf-more" type="button" data-action="home-jump" data-target="${escapeHtml(options.moreTarget || (type === 'flash' ? '#flash' : '#deals'))}">${escapeHtml(options.moreLabel || 'عرض الكل')}</button>
      </div>
      <div class="home-feature-body">
        <button class="home-feature-visual" type="button" data-action="${openAction}" data-offer-type="${escapeHtml(type)}" data-id="${escapeHtml(item.id)}">
          <img class="home-feature-image" src="${image}" alt="${escapeHtml(item.title)}" loading="lazy" />
        </button>
        <div class="home-feature-copy">
          ...
          <h3 class="home-feature-title">${escapeHtml(item.title)}</h3>
          <p class="home-feature-desc">${escapeHtml(item.description || '')}</p>
          <div class="badge-row product-badge-row"><span class="badge ${type === 'flash' ? 'badge-flash' : 'badge-deal'}">${type === 'flash' ? 'عرض محدود · غير محتسب ضمن هدف الشريحة' : 'سعر خاص · غير محتسب ضمن هدف الشريحة'}</span></div>
          <div class="home-feature-price mono">${price} ج.م</div>
          <div class="card-actions home-feature-actions">
            <button class="primary-btn" data-action="${toggleAction}" data-id="${escapeHtml(item.id)}" ${!canBuy ? 'disabled' : ''}>${escapeHtml(buttonText)}</button>
            <button class="ghost-btn" type="button" data-action="${openAction}" data-offer-type="${escapeHtml(type)}" data-id="${escapeHtml(item.id)}">تفاصيل</button>
          </div>
        </div>
      </div>
    </section>
...
function homeCompanyShelfHtml(companyRows) {
  return `
    <section class="section-card home-companies-shell" id="home-companies">
      <div class="shelf-head">
        <div>
          <h2>الشركات</h2>
          <div class="helper-text">شريط أفقي مضغوط لتقليل الهدر في مساحة الموبايل</div>
        </div>
      </div>
      <div class="company-shelf company-shelf-home">
        ${(companyRows || []).length ? companyRows.map((company) => `
          <article class="company-card company-card-home company-card-compact" data-action="open-company" data-company-id="${escapeHtml(company.company_id)}">
            ...
            <button class="ghost-btn company-btn" type="button">تصفح</button>
          </article>
...
function renderHomePage() {
  ...
  const flashState = getFlashState();
  const flashOffer = flashState.offer;
  ...
  setSearchBarHtml(`
    <section class="section-card search-card home-status-host">
      ${commercialStatusHtml()}
  ...
  els.pageContent.innerHTML = `
    <div class="page-stack home-stack">
      <section id="home-flash">
        ${homeFeaturedOfferHtml('flash', flashOffer, {
          title: 'عرض الساعة',
          subtitle: 'قسم واضح وقوي بصريًا للعروض الزمنية',
          cta: 'شراء الآن',
          moreTarget: '#flash',
          moreLabel: 'كل العروض',
        })}
      ...
      <section id="home-deal">
        ${homeFeaturedOfferHtml('deal', dailyDeal, {
          title: 'صفقة اليوم',
          subtitle: 'تظهر مبكرًا جدًا لأنها فرصة تحويل مباشرة',
          cta: 'شراء الآن',
          moreTarget: '#deals',
          moreLabel: 'تفاصيل الصفقة',
        })}
      </section>

      ${homeQuickActionsHtml()}

      <section id="home-most-ordered">
        ${renderProductShelf(
          'الأكثر طلبًا من التجار',
          'يظهر قبل الشركات لأنه يمثّل الطلب المتكرر الأكثر تأثيرًا على القرار التجاري',
          q ? bestSelling.filter((p) => matchesHomeSearch(p, q)) : bestSelling,
          { kind: 'bestseller', moreHash: '#home', moreLabel: 'عرض المزيد', badge: 'الأكثر طلبًا', actionLabel: 'شراء' }
        )}
      </section>

      ${homeCompanyShelfHtml(companyRows)}

      ${homeCategoriesHtml()}

      <section id="home-products" class="home-product-sections">
        ${renderProductShelf(
          'منتجات موصى بها لك',
          'مختارة بناءً على النشاط والسياق التجاري',
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'home-jump') {
      const hash = target.getAttribute('data-target') || '#home';
      if (state.view.type === 'home' && hash.startsWith('#home')) {
