...
function updateHeader() {
  ...
  if (els.tierBtn) {
    if (state.selectedTier) {
      const label = `شريحتك الحالية: ${tierLabel}`;
      els.tierBtn.innerHTML = `<span class="tier-chip-copy">شريحتك الحالية:</span><span class="tier-chip-name">${escapeHtml(tierLabel)}</span>`;
      els.tierBtn.title = label;
      els.tierBtn.setAttribute('aria-label', label);
    } else {
      const label = 'ادخل لاختيار شريحتك';
      els.tierBtn.innerHTML = `<span class="tier-chip-copy">${label}</span>`;
      els.tierBtn.title = label;
      els.tierBtn.setAttribute('aria-label', label);
    }
  }
  ...
  if (els.cartValue) {
    els.cartValue.textContent = `${integer(cartTotal())} ج.م`;
  }
  updateFlashHeader();
  syncCheckoutButton();
}
...
function updateFlashHeader() {
  ...
  if (!flashState.offer) {
    ...
    if (els.flashMeta) {
      ...
      els.flashMeta.classList.add('hidden');
    }
  ...
  if (status === 'active') {
    els.flashBtnText.textContent = 'عرض ساعة';
    if (els.flashMeta) {
      els.flashMeta.textContent = `متبقي ${remaining}`;
      els.flashMeta.classList.remove('hidden');
    }
