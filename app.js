...
function validateBeforeOrder() {
  if (!state.session) {
    ...
    state.pendingOpenCart = !els.cartDrawer?.classList.contains('hidden');
    closeCart();
    toast('يجب تسجيل الدخول أو إنشاء حساب لإتمام الطلب.');
    openRegisterPage();
    return false;
  }
  ...
  if (!tier) {
    smartToast('tier.missing', 'يجب اختيار شريحة قبل إتمام الطلب.', true);
    return false;
  }
  ...
  if (total < tier.min_order) {
    ...
    smartToast('tier.incomplete', `المتبقي ${num(diff)} جنيه للوصول إلى الشريحة.`, true);
    return false;
  }
...
async function handleCheckout() {
  ...
  try {
    if (state.checkoutStage === 'validate') {
      toast('تم التحقق من الطلب بنجاح، اضغط إرسال الطلب لإكمال العملية.');
...
async function handleLogin() {
  ...
  if (!identifier || !password) {
    toast('اكتب رقم الهاتف أو اسم المستخدم وكلمة المرور.');
    return;
  }
  ...
  try {
    ...
    if (!user) {
      toast('بيانات الدخول غير صحيحة أو الحساب غير موجود.');
      return;
    }
    if (String(user.password || '').trim() !== password) {
      toast('كلمة المرور غير صحيحة، راجعها ثم حاول مرة أخرى.');
      return;
    }
