...
function openProductModal(productId) {
  ...
  els.productModalAction.textContent = inCart ? 'إزالة من السلة' : 'إضافة للسلة';
...
function openOfferModal(type, id) {
  ...
  const actionLabel = type === 'deal' ? 'أضف الآن' : 'أضف الآن';
...
function compactProductCardHtml(product, options = {}) {
  ...
  const actionLabel = options.actionLabel || 'إضافة للسلة';
  const unit = currentUnitForProduct(product);
    ...
    <article class="product-tile" data-product-id="${escapeHtml(product.product_id)}">
      ...
      <div class="product-body compact">
        ...
        <div class="card-actions compact-actions">
          ...
          <button class="ghost-btn" data-action="open-product" data-product-id="${escapeHtml(product.product_id)}">تفاصيل</button>
        </div>
      </div>
    </article>
  `;
...
function updateHeader() {
  ...
  if (els.tierBtn) {
    els.tierBtn.textContent = 'عرض الشرائح';
    els.tierBtn.title = `${tierLabel} · عرض الشرائح`;
    els.tierBtn.setAttribute('aria-label', `عرض الشرائح · ${tierLabel}`);
  }
      ...
      <div class="product-body">
        ...
        <div class="card-actions">
          <button class="primary-btn" data-action="toggle-product" data-product-id="${escapeHtml(product.product_id)}">
            ${inCart ? 'إزالة من السلة' : 'إضافة للسلة'}
          </button>
          <button class="ghost-btn" data-action="open-product" data-product-id="${escapeHtml(product.product_id)}" type="button">تفاصيل</button>
        </div>
      </div>
    </article>
  `;
    ...
    els.banner.innerHTML = `
      <section class="banner-card home-hero">
        <div class="home-hero-copy">
          <div class="hero-kicker">Modern Clean · Amazon Style</div>
          <h1>اكتشف أفضل العروض الآن</h1>
          <p>واجهة سريعة، توصيات مبنية على السلوك، وعروض فعلية لمدة ساعة. كل قسم هنا يتحرك حسب ما يفعله المستخدمون داخل المتجر.</p>
          ...
          <div class="hero-pills">
            ...
            <span class="badge">تتبع سلوكي</span>
          </div>
        </div>
    ...
    <div class="page-stack">
      <section id="home-recommended">
        ${renderProductShelf(
          ...
          { kind: 'recommended', moreHash: '#home', moreLabel: 'عرض الكل', badge: 'موصى به', actionLabel: 'إضافة للسلة' }
        )}
      </section>
      ...
      <section>
        ${renderProductShelf(
          ...
          { kind: 'viewed', moreHash: '#home', moreLabel: 'عرض المزيد', badge: 'الأكثر مشاهدة', actionLabel: 'إضافة للسلة' }
        )}
      </section>
      ...
      <section>
        ${renderProductShelf(
          ...
          { kind: 'bestseller', moreHash: '#home', moreLabel: 'عرض المزيد', badge: 'الأكثر مبيعًا', actionLabel: 'أضف الآن' }
        )}
      </section>
      ...
      <section class="shelf-card">
        <div class="shelf-head">
          <div>
            ...
            <div class="helper-text">مصادر موثوقة ومنظمة للوصول السريع</div>
          </div>
        </div>
        <div class="company-shelf">
          ${companyRows.length ? companyRows.map((company) => `
            <article class="company-card company-card-home" data-action="open-company" data-company-id="${escapeHtml(company.company_id)}">
              ...
              <button class="ghost-btn company-btn" type="button">استعرض الشركة</button>
            </article>
