...
const els = {
  ...
  supportFabBtn: document.getElementById('supportFabBtn'),
  supportFabMenu: document.getElementById('supportFabMenu'),
...
async function syncInvoiceSequence() {
  ...
  try {
    const latest = await apiGet('orders', { select: 'order_number,created_at', order: 'created_at.desc', limit: '1' }).catch(() => []);
  ...
  els.tierModalBody.innerHTML = `
    <div class="tier-modal-grid">
      ${state.tiers.length ? state.tiers.map((tier) => {
        ...
        const minOrder = num(tier.min_order || 0);
        const discount = num(tier.discount_percent || 0);
        const label = tierDisplayLabel(tier);
        return `
          <article class="tier-modal-card ${isCurrent ? 'active' : ''}">
            <div class="tier-modal-top">
              <div>
                <div class="tier-modal-name">${escapeHtml(label)}</div>
                <div class="tier-modal-desc">${escapeHtml(Number(tier.discount_percent || 0) > 0 ? `خصم ${discount}%` : 'بدون خصم')}</div>
              </div>
              ${isCurrent ? '<span class="badge">الحالية</span>' : `<span class="badge">${discount}%</span>`}
            </div>
            <div class="tier-modal-min">الحد الأدنى: ${minOrder} ج.م</div>
            <button class="primary-btn" data-action="select-tier" data-tier-name="${escapeHtml(tier.tier_name)}" data-visible-label="${escapeHtml(label)}">${isCurrent ? 'تفعيل الحالية' : 'اختيار الشريحة'}</button>
          </article>
        `;
...
function closeTierModal() {
  if (els.tierModal) setOverlay(els.tierModal, false);
}
...
function updateHeader() {
  ...
  if (els.cartValue) els.cartValue.textContent = integer(count);
  if (els.mobileCartCount) els.mobileCartCount.textContent = integer(count);
  if (els.mobileCartTotal) els.mobileCartTotal.textContent = `${num(total)} ج.م`;
  updateFlashHeader();
  syncCheckoutButton();
}
...
function renderHomePage() {
  ...
  const flashState = getFlashState();
  const dailyDeal = filteredDailyDeals()[0] || null;
  ...
  const bannerHtml = banner
    ? `<img class="hero-banner-image" src="${escapeHtml(banner)}" alt="بانر الصفحة الرئيسية" loading="eager" />`
    : `<div class="banner-fallback hero-banner-fallback">اكتشف أفضل العروض الآن</div>`;

  els.pageContent.innerHTML = `
    <div class="page-stack home-mobile-stack">
      <section class="hero-cta-card">
        <div class="hero-cta-copy">
          <div class="hero-kicker">B2B Distribution Platform</div>
          <h1>الأهرام للتجارة والتوزيع</h1>
          <p>حلول توزيع احترافية للتجار والشركات</p>
      ...
      <section class="live-offers-grid">
        <article class="offer-card offer-flash">
          <div class="offer-card-head">
            <div class="offer-icon">⚡</div>
            <div>
              <div class="offer-title">عرض الساعة</div>
              <div class="offer-sub">فرصة محدودة بزمن مباشر</div>
            </div>
          </div>
          <div class="offer-countdown mono">${escapeHtml(flashState.status === 'active' ? flashState.remaining : '--:--:--')}</div>
          <button class="ghost-btn offer-link" type="button" data-action="shelf-more" data-hash="#flash">تسوق العرض</button>
        </article>
        <article class="offer-card offer-deal">
          <div class="offer-card-head">
            <div class="offer-icon">🔥</div>
            <div>
              <div class="offer-title">صفقة اليوم</div>
              <div class="offer-sub">${escapeHtml(dailyDeal ? dailyDeal.title : 'فرصة يومية بسعر خاص')}</div>
            </div>
          </div>
          <div class="offer-price mono">${escapeHtml(dailyDeal ? `${num(dailyDeal.price)} ج.م` : 'تسعير خاص')}</div>
          <button class="ghost-btn offer-link" type="button" data-action="shelf-more" data-hash="#deals">تسوق الصفقة</button>
        </article>
      </section>

      <section class="tier-context-card">
        <div class="shelf-head">
          <div>
            <h2>فئة الأسعار الحالية</h2>
            <div class="helper-text">تابع تأهلك للشريحة والخصم</div>
          </div>
          <button class="ghost-btn shelf-more" type="button" data-action="open-tier-modal">تغيير الشريحة</button>
        </div>
        <div class="tier-context-main">
          <div>
            <div class="tier-context-big" id="homeTierLabel">${escapeHtml(getSelectedTierLabel())}</div>
            <div class="tier-context-small">${escapeHtml(state.selectedTier ? 'تسعير الشريحة فعال' : 'بدون خصم')}</div>
          </div>
          <span class="badge">${escapeHtml(state.selectedTier ? 'نشطة' : 'أساسية')}</span>
        </div>
        <div class="tier-context-stats">
          ${(() => {
            const ctx = getTierCommercialContext();
            const tier = ctx.tier;
            const minOrder = tier ? num(ctx.minOrder) : '0';
            const eligible = num(ctx.eligible);
            const remaining = tier ? num(ctx.remaining) : '0';
            return `
              <div class="tier-stat"><span>الحد الأدنى</span><strong class="mono">${escapeHtml(minOrder)} ج.م</strong></div>
              <div class="tier-stat"><span>المؤهل الحالي</span><strong class="mono">${escapeHtml(eligible)} ج.م</strong></div>
              <div class="tier-stat"><span>المتبقي</span><strong class="mono">${escapeHtml(remaining)} ج.م</strong></div>
            `;
          })()}
        </div>
      </section>

      <section class="theme-switcher-shell">
        <div class="theme-switcher-slot" id="themeSwitcherHost"></div>
      </section>

      <section class="section-card search-card search-card-home">
      ...
      <section class="quick-nav-card">
        <button class="quick-pill" type="button" data-action="home">
          <span>⌂</span><strong>الرئيسية</strong>
        </button>
        <button class="quick-pill" type="button" data-action="navigate-tier">
          <span>☷</span><strong>التصنيفات</strong>
        </button>
        <button class="quick-pill" type="button" data-action="shelf-more" data-hash="#deals">
          <span>⚡</span><strong>العروض</strong>
        </button>
        <button class="quick-pill" type="button" data-action="shelf-more" data-hash="#home-companies">
          <span>▣</span><strong>الشركات</strong>
        </button>
      </section>

      <section class="banner-card hero-banner-section">
        ${bannerHtml}
      </section>
        ...
        <section class="shelf-card">
          <div class="shelf-head">
            <div>
              ...
              <div class="helper-text">تشكيل متنوع من الشركات</div>
            </div>
          </div>
...
function renderApp() {
  ...
  if (els.banner) els.banner.classList.toggle('hidden', isRegister || state.view.type === 'home');
  if (els.searchBar) els.searchBar.classList.toggle('hidden', isRegister || state.view.type === 'home');
...
function renderCart() {
  ...
  const eligibleTotal = eligibleTierTotal();
  const tier = getSelectedTierObject();
...
async function createOrder() {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const orderNumber = attempt < maxAttempts - 1 ? await reserveInvoiceNumber() : buildFallbackOrderNumber();
    ...
    try {
      ...
      const isConflict = /\b409\b|duplicate key|unique constraint|conflict/i.test(message);
      if (!isConflict || attempt === maxAttempts - 1) break;
      await syncInvoiceSequence();
    }
  }
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'mobile-checkout') {
      openCart();
      return;
    }
    if (action === 'toggle-support') {
      const next = els.supportFabMenu?.classList.contains('hidden');
      if (els.supportFabMenu) {
        els.supportFabMenu.classList.toggle('hidden', !next);
        els.supportFabMenu.setAttribute('aria-hidden', String(!next));
      }
      return;
    }
  });
  ...
  document.addEventListener('click', (event) => {
    ...
    if (!event.target.closest('.support-fab')) {
      if (els.supportFabMenu) {
        els.supportFabMenu.classList.add('hidden');
        els.supportFabMenu.setAttribute('aria-hidden', 'true');
      }
    }
  });
  ...
  if (els.mobileCheckoutBtn) els.mobileCheckoutBtn.addEventListener('click', () => openCart());
  if (els.supportFabBtn) els.supportFabBtn.addEventListener('click', () => {
    if (!els.supportFabMenu) return;
    const next = els.supportFabMenu.classList.contains('hidden');
    els.supportFabMenu.classList.toggle('hidden', !next);
    els.supportFabMenu.setAttribute('aria-hidden', String(!next));
  });
...
(function () {
  ...
  function decorateHero() {
    ...
    host.innerHTML = '';
    host.appendChild(createThemeSwitcher());
  }
