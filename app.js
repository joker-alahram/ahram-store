...
function toast(message) {
  ...
  if (els.toast.dataset.message === text && els.toast.classList.contains('show')) {
  ...
  els.toast.dataset.message = text;
...
const SALES_SEEDS = {
  login: {
    ...
    missing: ["يرجى استكمال بيانات الدخول."],
  },
  register: {
    success: ["تم إنشاء الحساب بنجاح."],
    error: ["تعذر إنشاء الحساب. راجع البيانات."],
    duplicate: ["هذا الرقم مسجل مسبقًا. استخدم تسجيل الدخول."],
  },
  cart: {
    empty: ["السلة فارغة الآن."],
    add: ["تمت إضافة المنتج إلى السلة."],
    remove: ["تمت إزالة المنتج من السلة."],
    idle: ["أضف منتجًا لبدء الطلب."],
    filled: ["السلة جاهزة للإرسال."],
  },
  tier: {
    missing: ["يلزم اختيار الشريحة أولًا."],
    incomplete: ["المتبقي لتحقيق الشريحة: {amount}. أضف منتجات لإكمال الخصم."],
    almost: ["اقتربت من تحقيق الشريحة."],
    selected: ["تم اختيار الشريحة بنجاح."],
  },
  deals: {
    suggest: ["اطلع على صفقة اليوم."],
    active: ["عرض الساعة متاح الآن."],
  },
  checkout: {
    validate: ["يرجى مراجعة الطلب قبل الإرسال."],
    missing: ["يوجد نقص في بيانات الإرسال."],
    blocked: ["لا يمكن الإرسال قبل استيفاء الشروط."],
  },
  success: {
    order: ["تم إنشاء الطلب بنجاح."],
    whatsapp: ["تم إرسال الفاتورة إلى واتساب."],
    login: ["تم تسجيل الدخول بنجاح."],
    register: ["تم إنشاء الحساب بنجاح."],
  },
  errors: {
    generic: ["تعذر تنفيذ العملية الآن."],
    network: ["تعذر الاتصال بالخادم الآن."],
  },
};
...
function buildSalesEngine(seeds) {
  ...
  Object.entries(seeds).forEach(([section, groups]) => {
    ...
    Object.entries(groups).forEach(([kind, templates]) => {
      out[section][kind] = templates.map((template) => String(template || '').trim()).filter(Boolean);
    });
  });
  return out;
}
...
function getSmartMessage(type) {
  ...
  const msg = pool.find((m) => m !== last) || pool[0];
...
function smartToast(typeOrMessage, fallback, force = false) {
  ...
  const message = fallback ?? (isEngineType ? (getSmartMessage(typeOrMessage) || '') : typeOrMessage);
      ...
      <div class="rail company-rail">
        ${companies.length ? companies.map((company) => `
          <article class="mini-company-card" data-action="open-company" data-company-id="${escapeHtml(company.company_id)}">
            <img class="mini-company-logo" src="${escapeHtml(company.company_logo || placeholderImage(company.company_name))}" alt="${escapeHtml(company.company_name)}" loading="lazy" decoding="async" />
      ...
      <div class="rail product-rail">
        ${products.length ? products.map((product) => `
          <article class="rail-product-card" data-product-id="${escapeHtml(product.product_id)}">
            <img class="rail-product-image" src="${escapeHtml(product.product_image || placeholderImage(product.product_name))}" alt="${escapeHtml(product.product_name)}" loading="lazy" decoding="async" />
...
function invoiceViewHtml(model) {
  ...
  const rep = model.rep || {};
  const customerName = customer.name || customer.username || 'غير متاح';
  const customerPhone = customer.phone || 'غير متاح';
  const customerAddress = customer.address || customer.location || 'غير متاح';
  const repName = rep.name || 'غير متاح';
  const repPhone = rep.phone || 'غير متاح';
  const tierName = model.tier_name || 'بدون خصم';
  return `
    <article class="invoice-card invoice-view">
      <div class="invoice-surface">
        <div class="invoice-header">
          <div class="invoice-heading">🧾 فاتورة طلب شراء</div>
          <div class="invoice-row invoice-row-tight">
            <span class="invoice-label">رقم الفاتورة</span>
            <span class="invoice-value">${escapeHtml(model.invoice_number || model.order_number || '')}</span>
          </div>
        </div>

        <div class="invoice-section">
          <div class="invoice-section-title">👨‍💼 المندوب</div>
          <div class="invoice-row"><span class="invoice-label">الاسم</span><span class="invoice-value">${escapeHtml(repName)}</span></div>
          <div class="invoice-row"><span class="invoice-label">الهاتف</span><span class="invoice-value">${escapeHtml(repPhone)}</span></div>
        </div>

        <div class="invoice-section">
          <div class="invoice-section-title">👤 العميل</div>
          <div class="invoice-row"><span class="invoice-label">الاسم</span><span class="invoice-value">${escapeHtml(customerName)}</span></div>
          <div class="invoice-row"><span class="invoice-label">الهاتف</span><span class="invoice-value">${escapeHtml(customerPhone)}</span></div>
          <div class="invoice-row"><span class="invoice-label">العنوان</span><span class="invoice-value">${escapeHtml(customerAddress)}</span></div>
        </div>

        <div class="invoice-section">
          <div class="invoice-section-title">🏷️ الشريحة</div>
          <div class="invoice-row"><span class="invoice-label">اسم الشريحة</span><span class="invoice-value">${escapeHtml(tierName)}</span></div>
        </div>
      </div>

      <div class="invoice-surface">
        <div class="invoice-section-title">📦 تفاصيل الطلب</div>
        <div class="invoice-items">
          ${model.items.length ? model.items.map((item) => `
            <div class="invoice-item">
              <div class="invoice-item-main">
                <strong>${escapeHtml(item.title || '')}</strong>
                <span>كود: ${escapeHtml(item.id || '')}</span>
              </div>
              <div class="invoice-item-meta">
                <span>الوحدة: ${escapeHtml(item.unit === 'carton' ? 'كرتونة' : item.unit === 'pack' ? 'دستة' : 'قطعة')}</span>
                <span>الكمية: ${integer(item.qty || 0)}</span>
                <span>سعر الوحدة: ${num(item.price)} ج.م</span>
                <span>الإجمالي: ${num((item.qty || 0) * (item.price || 0))} ج.م</span>
              </div>
            </div>
      ...
      <div class="invoice-surface">
        <div class="invoice-section-title">💰 الإجمالي</div>
        ...
        <div class="invoice-row invoice-total-row"><span class="invoice-label">الإجمالي النهائي</span><span class="invoice-value">${num(model.totals.total_amount)} ج.م</span></div>
      </div>
    </article>`;
}
...
function invoiceViewText(model) {
  const customer = model.customer || {};
  const rep = model.rep || {};
  const customerName = customer.name || customer.username || '-';
  const customerPhone = customer.phone || '-';
  const customerAddress = customer.address || customer.location || '-';
  ...
  const lines = [
    '🧾 فاتورة طلب شراء',
    `رقم الفاتورة: ${model.invoice_number || model.order_number || '-'}`,
    '',
    '━━━━━━━━━━━━━━',
    '',
    '👨‍💼 المندوب',
  ...
  for (const item of model.items) {
    ...
    lines.push(`كود: ${item.id || '-'}`);
    lines.push(`الوحدة: ${item.unit === 'carton' ? 'كرتونة' : item.unit === 'pack' ? 'دستة' : 'قطعة'}`);
    lines.push(`سعر الوحدة: ${num(item.price)} جنيه`);
    lines.push(`الكمية: ${integer(item.qty || 0)}`);
    lines.push(`الإجمالي: ${num((item.qty || 0) * (item.price || 0))} جنيه`);
    lines.push('');
  }
  ...
  lines.push('');
  lines.push('💰 الإجمالي');
  lines.push(`إجمالي المنتجات: ${num(model.totals.products_total)} جنيه`);
  lines.push(`إجمالي العروض: ${num(model.totals.deals_total)} جنيه`);
  lines.push(`إجمالي الساعة: ${num(model.totals.flash_total)} جنيه`);
  lines.push(`الإجمالي النهائي: ${num(model.totals.total_amount)} جنيه`);
...
function productCardHtml(product) {
  ...
  return `
    <article class="product-card" data-product-id="${escapeHtml(product.product_id)}">
      <button type="button" class="product-media-btn" data-action="toggle-product" data-product-id="${escapeHtml(product.product_id)}" aria-label="${escapeHtml(product.product_name)}">
        <img class="product-image" src="${escapeHtml(image)}" alt="${escapeHtml(product.product_name)}" loading="lazy" decoding="async" />
      </button>
      <div class="product-body">
        <div>
          <button type="button" class="product-title-btn" data-action="toggle-product" data-product-id="${escapeHtml(product.product_id)}">${escapeHtml(product.product_name)}</button>
        </div>
...
function renderHomePage() {
  ...
  const bannerHtml = banner
    ? `<img class="banner-image" src="${escapeHtml(banner)}" alt="بانر الصفحة الرئيسية" loading="eager" fetchpriority="high" decoding="async" />`
    : `<div class="banner-fallback">لا توجد صورة بانر</div>`;

  if (els.banner) {
    els.banner.innerHTML = `
      <section class="banner-card hero-banner hero-banner-image-only">
        <div class="hero-art hero-art-full">
          ${bannerHtml}
        </div>
      </section>
    `;
...
function dealCardHtml(item, type, activeFlash = null) {
  ...
  return `
    <article class="deal-card">
      <button type="button" class="deal-media-btn" data-action="toggle-${type}" data-id="${escapeHtml(item.id)}" aria-label="${escapeHtml(title)}" ${buttonDisabled ? 'disabled' : ''}>
        <img class="deal-image" src="${image}" alt="${title}" loading="lazy" decoding="async" />
      </button>
      <div class="deal-body">
        <div class="badge-row">
        ...
        <button type="button" class="product-title-btn deal-title-btn" data-action="toggle-${type}" data-id="${escapeHtml(item.id)}" ${buttonDisabled ? 'disabled' : ''}>${title}</button>
