...
function scheduleHeaderCollapse() {
  ...
  headerCollapseRaf = window.requestAnimationFrame(() => {
    headerCollapseRaf = 0;
    const collapsed = window.scrollY > 40;
    document.body.classList.toggle('is-header-collapsed', collapsed);
  });
}
...
function syncDynamicRefs() {
  ...
  els.registerPage = document.getElementById('registerPage');
  els.quickFooter = document.getElementById('quickFooter');
  els.mainHeader = document.getElementById('mainHeader');
  els.userMenu = document.getElementById('userMenu');
  els.tierGateModal = document.getElementById('tierGateModal');
  els.tierGateContent = document.getElementById('tierGateContent');
}
...
const SMART_CATEGORY_RULES = [
  { key: 'shampoo', label: 'شامبوهات', aliases: ['شامبو', 'شامبوهات', 'shampoo'] },
  { key: 'cream', label: 'كريمات', aliases: ['كريم', 'كريمات', 'cream', 'lotion'] },
  { key: 'rollon', label: 'رول أون', aliases: ['رول اون', 'رول أون', 'roll on', 'rollon'] },
  { key: 'spray', label: 'اسبراي', aliases: ['اسبراي', 'سبراي', 'spray', 'spray mist'] },
  { key: 'hair-dye', label: 'صبغات', aliases: ['صبغه', 'صبغات', 'صبغه شعر', 'hair dye', 'dye'] },
  { key: 'wash', label: 'غسول', aliases: ['غسول', 'wash', 'face wash', 'body wash'] },
  { key: 'soap', label: 'صابون', aliases: ['صابون', 'soap', 'bar soap'] },
  { key: 'conditioner', label: 'بلسم', aliases: ['بلسم', 'conditioner'] },
  { key: 'deodorant', label: 'مزيلات', aliases: ['مزيل', 'مزيل عرق', 'deodorant', 'roll-on'] },
  { key: 'skin-care', label: 'العناية بالبشرة', aliases: ['بشرة', 'skin', 'serum', 'toner', 'mask'] },
  { key: 'hair-care', label: 'العناية بالشعر', aliases: ['شعر', 'hair', 'serum شعر'] },
  { key: 'toothcare', label: 'العناية بالفم', aliases: ['اسنان', 'معجون', 'mouthwash', 'tooth', 'oral'] },
];

function smartCategoryScore(product, rule) {
  const name = normalizeArabicText(product?.product_name || '');
  if (!name) return 0;
  let score = 0;
  for (const alias of rule.aliases) {
    const aliasNorm = normalizeArabicText(alias);
    if (!aliasNorm) continue;
    if (name === aliasNorm) score += 6;
    if (name.includes(aliasNorm)) score += 4;
    const tokens = tokenizeArabicText(aliasNorm);
    if (tokens.length && tokens.every((token) => name.includes(token))) score += 3;
  }
  if (!score) {
    const ruleParts = tokenizeArabicText(rule.label);
    if (ruleParts.some((part) => name.includes(part))) score += 2;
  }
  return score;
}

function buildSmartCategoryBuckets(products = state.products) {
  const buckets = SMART_CATEGORY_RULES.map((rule) => ({ ...rule, products: [], count: 0 }));
  for (const product of products || []) {
    const matches = buckets
      .map((bucket, index) => ({ bucket, index, score: smartCategoryScore(product, bucket) }))
      .filter((item) => item.score > 0)
      .sort((a, b) => b.score - a.score || a.index - b.index);
    if (!matches.length) continue;
    const best = matches[0].bucket;
    const key = String(product.product_id || '');
    if (!best.products.some((item) => String(item.product_id) === key)) {
      best.products.push(product);
      best.count += 1;
    }
  }
  return buckets.filter((bucket) => bucket.count > 0).sort((a, b) => b.count - a.count || compareNatural(a.label, b.label)).slice(0, 6);
}

function getTierProgressData() {
  const tier = getSelectedTierObject();
  const tiers = [...state.tiers].sort((a, b) => Number(a.min_order || 0) - Number(b.min_order || 0));
  const total = Number(cartTotal() || 0);

  if (!tier) {
    const nextTier = tiers[0] || null;
    const nextTarget = Number(nextTier?.min_order || 0);
    const remaining = Math.max(0, nextTarget - total);
    const achieved = nextTarget > 0 ? Math.min(total, nextTarget) : total;
    return {
      tier: null,
      currentLabel: 'لم يتم اختيار شريحة',
      nextTier,
      total,
      achieved,
      remaining,
      percent: nextTarget > 0 ? Math.min(100, (total / nextTarget) * 100) : 0,
      nextLabel: nextTier ? tierDisplayLabel(nextTier) : 'الشريحة التالية',
      target: nextTarget,
    };
  }

  const currentIndex = tiers.findIndex((row) => String(row.tier_name) === String(tier.tier_name));
  const nextTier = currentIndex >= 0 ? tiers[currentIndex + 1] || null : tiers.find((row) => Number(row.min_order || 0) > Number(tier.min_order || 0)) || null;
  const currentTarget = Number(tier.min_order || 0);
  const nextTarget = Number(nextTier?.min_order || currentTarget);
  const achieved = Math.max(total, currentTarget);
  const remaining = nextTier ? Math.max(0, nextTarget - total) : Math.max(0, currentTarget - total);
  const percent = nextTarget > 0 ? Math.min(100, (total / nextTarget) * 100) : 100;

  return {
    tier,
    currentLabel: tierDisplayLabel(tier),
    nextTier,
    total,
    achieved,
    remaining,
    percent,
    nextLabel: nextTier ? tierDisplayLabel(nextTier) : 'تثبيت الشريحة الحالية',
    target: nextTarget,
  };
}
...
function companyBannerHtml() {
  ...
  if (banner) {
    return `<img class="company-banner-image" src="${escapeHtml(banner)}" alt="بانر الشركة" loading="eager" />`;
  }
  return `<div class="company-banner-fallback">متجر الأهرام للتجارة والتوزيع</div>`;
}

function renderThemeSwitcherHtml() {
  const themeOrder = ['gold', 'dark-enterprise', 'light-professional', 'blue-distribution', 'emerald-sales'];
  return `
    <div class="theme-switcher" role="group" aria-label="اختيار الثيم">
      ${themeOrder.map((key) => `<button type="button" class="theme-dot" data-action="set-theme" data-theme-key="${key}" aria-label="${escapeHtml(key)}"></button>`).join('')}
    </div>
  `;
}

function contactActionsHtml() {
  const phone = '01040880002';
  const support = CONFIG.supportWhatsapp || '201040880002';
  return `
    <div class="contact-actions" aria-label="وسائل التواصل السريع">
      <a class="contact-icon-btn" href="tel:${phone}" aria-label="اتصال مباشر" title="اتصال مباشر">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6.6 10.8c1.4 2.8 3.4 4.8 6.2 6.2l2.1-2.1c.3-.3.7-.4 1.1-.3 1.2.4 2.6.6 3.9.6.6 0 1 .4 1 1v3.3c0 .6-.4 1-1 1C10.8 20.5 3.5 13.2 3.5 4c0-.6.4-1 1-1h3.3c.6 0 1 .4 1 1 0 1.4.2 2.7.6 3.9.1.4 0 .8-.3 1.1l-1.5 1.8z"/></svg>
      </a>
      <a class="contact-icon-btn" href="https://wa.me/${support}" target="_blank" rel="noopener noreferrer" aria-label="واتساب" title="واتساب">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a9.9 9.9 0 0 0-8.6 14.8L2 22l5.4-1.4A10 10 0 1 0 12 2zm5.7 14.5c-.2.6-1 1-1.5 1.1-.4.1-.9.1-1.5 0-.3-.1-.7-.2-1.2-.4-2-.9-3.3-2.2-4.2-4.2-.2-.5-.3-.9-.4-1.2-.1-.6-.1-1.1 0-1.5.1-.5.5-1.3 1.1-1.5l.8-.2c.3-.1.6 0 .8.2l1 1c.2.2.3.6.2.9l-.4 1c-.1.2-.1.4 0 .6.4.8.9 1.4 1.7 1.7.2.1.4.1.6 0l1-.4c.3-.1.7 0 .9.2l1 1c.2.2.2.5.2.8l-.1.8z"/></svg>
      </a>
      <a class="contact-icon-btn" href="https://www.facebook.com/alahram2014/" target="_blank" rel="noopener noreferrer" aria-label="فيسبوك" title="فيسبوك">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.2 22v-8.1h2.7l.4-3.1h-3.1V8.8c0-.9.3-1.5 1.6-1.5h1.6V4.5c-.8-.1-1.7-.2-2.6-.2-2.6 0-4.4 1.6-4.4 4.5v2H6.6v3.1h2.8V22h3.8z"/></svg>
      </a>
      <button class="contact-icon-btn" type="button" data-action="support" aria-label="الدعم" title="الدعم">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2a10 10 0 1 0 10 10A10.01 10.01 0 0 0 12 2Zm.5 15h-1v-1h1Zm1.4-5.9c-.7.5-1.2.9-1.4 1.4a2.6 2.6 0 0 0-.2 1v.5h-1.3v-.7a3.1 3.1 0 0 1 .4-1.6c.3-.6.8-1.1 1.6-1.7.7-.5 1.1-1 1.1-1.7 0-.9-.7-1.5-1.7-1.5a1.8 1.8 0 0 0-1.9 1.6H8.8a3.1 3.1 0 0 1 3.3-2.9c2 0 3.4 1.1 3.4 2.8 0 1.3-.6 2-1.6 2.8Z"/></svg>
      </button>
    </div>
  `;
}

function renderHeaderActionsRow() {
  const tierLabel = getSelectedTierLabel();
  const selectedTierLabel = state.selectedTier ? tierLabel : 'أدخل واختار شريحتك';
  return `
    <div class="action-row">
      <button class="quick-action-btn" id="tierBtn" data-action="open-tiers" type="button">${escapeHtml(selectedTierLabel)}</button>
      <button class="quick-action-btn" data-action="open-companies" type="button">تصفح الشركات</button>
      <button class="quick-action-btn" id="dealsBtn" data-action="open-deals" type="button">صفقة اليوم</button>
      <button class="quick-action-btn" id="flashBtn" data-action="open-flash" type="button">عرض الساعة</button>
      <button class="quick-action-btn" data-action="open-categories" type="button">الأقسام</button>
    </div>
  `;
}

function renderOperationalHeroHtml() {
  const flashState = getFlashState();
  if (flashState.status === 'active' && flashState.offer) {
    return `
      <section class="hero-card">
        <h2 class="hero-title">عرض الساعة</h2>
        <div class="hero-countdown mono" id="flashCountdownValue">${escapeHtml(flashState.remaining || '--:--:--')}</div>
        <div class="hero-note">متبقي على انتهاء العرض</div>
        <div class="hero-subtitle">${escapeHtml(flashState.offer.title || '')}</div>
      </section>
    `;
  }

  const progress = getTierProgressData();
  return `
    <section class="hero-card">
      <h2 class="hero-title">أنت الآن على شريحة</h2>
      <div class="hero-subtitle">${escapeHtml(progress.currentLabel || 'الشريحة الرئيسية')}</div>
      <div class="hero-progress">
        <div class="hero-progress-bar"><span style="width:${Number(progress.percent || 0).toFixed(2)}%"></span></div>
        <div class="hero-progress-meta">
          <div class="pill"><span class="value mono">${num(progress.achieved || 0)}</span><span class="label">المحقق</span></div>
          <div class="pill"><span class="value mono">${num(progress.remaining || 0)}</span><span class="label">المتبقي</span></div>
          <div class="pill"><span class="value">${escapeHtml(progress.nextLabel || 'المرحلة التالية')}</span><span class="label">الهدف</span></div>
        </div>
      </div>
      <div class="hero-note">إجمالي السلة الحالي: ${num(progress.total || 0)}</div>
    </section>
  `;
}
...
function renderQuickCardsHtml() {
  ...
  const topRequested = topBestSellingProducts(1)[0] || null;
  const smartBuckets = buildSmartCategoryBuckets();
  const topCategory = smartBuckets[0] || null;
  return `
    <section class="quick-card-grid" id="quickOverview">
      <article class="op-card">
        <h3 class="op-title">الشركات المتاحة</h3>
        <div class="op-metric"><strong class="mono">${integer(companiesCount)}</strong><span>شركة جاهزة للتصفح</span></div>
        <div class="op-copy">دخول مباشر إلى الموردين والماركات المتاحة على المنصة.</div>
        <button class="primary-btn" type="button" data-action="open-companies">عرض الشركات</button>
      </article>
      <article class="op-card">
        <h3 class="op-title">الأكثر طلبًا</h3>
        <div class="op-metric"><strong class="mono">${integer(state.topProducts?.length || 0) || integer(state.products.length)}</strong><span>منتج متابع</span></div>
        <div class="op-copy">${topRequested ? escapeHtml(topRequested.product_name) : 'اعتمادًا على نشاط البيع الحقيقي.'}</div>
        <button class="primary-btn" type="button" data-action="scroll-best-selling">عرض الأكثر مبيعًا</button>
      </article>
      <article class="op-card">
        <h3 class="op-title">تصفح حسب الأصناف</h3>
        <div class="op-metric"><strong class="mono">${integer(smartBuckets.length)}</strong><span>تصنيف ذكي</span></div>
        <div class="op-copy">${topCategory ? escapeHtml(topCategory.label) : 'تصنيف ديناميكي من أسماء المنتجات.'}</div>
        <button class="primary-btn" type="button" data-action="open-categories">استكشاف الأصناف</button>
      </article>
    </section>
  `;
}

function renderSmartCategoriesHtml() {
  const buckets = buildSmartCategoryBuckets();
  return `
    <section class="section-card smart-card" id="smartCategoriesSection">
      <div class="smart-card-head">
        <div>
          <h2>تصفح المنتجات حسب الأصناف</h2>
          <div class="helper-text">تصنيف ذكي يعتمد على أسماء المنتجات والمرادفات والتطابق الجزئي.</div>
        </div>
        <button class="ghost-btn shelf-more" type="button" data-action="open-categories">عرض الكل</button>
      </div>
      <div class="category-grid">
        ${buckets.length ? buckets.map((bucket) => {
          const samples = bucket.products.slice(0, 3).map((product) => escapeHtml(product.product_name)).join(' · ');
          return `
            <article class="tier-card category-tile" data-action="jump-category" data-category-key="${escapeHtml(bucket.key)}">
              <div class="bad-line">
                <div>
                  <div class="tier-name">${escapeHtml(bucket.label)}</div>
                  <div class="tier-visible">${integer(bucket.count)} منتج</div>
                </div>
                <span class="badge">ذكي</span>
              </div>
              <div class="helper-text">${samples || 'مطابقة تلقائية من القاموس الذكي.'}</div>
              <button class="ghost-btn" type="button" data-action="jump-category" data-category-key="${escapeHtml(bucket.key)}">عرض التصنيف</button>
            </article>
          `;
        }).join('') : `<div class="empty-state">لا توجد بيانات كافية لتكوين الأصناف الذكية</div>`}
      </div>
    </section>
  `;
}

function renderCompaniesSectionHtml(rows) {
  return `
    <section class="section-card" id="companiesSection">
      <div class="smart-card-head">
        <div>
          <h2>الشركات المتاحة</h2>
          <div class="helper-text">دخول سريع إلى العلامات والموردين.</div>
        </div>
      </div>
...
function renderCategoryProductsSectionHtml() {
  const buckets = buildSmartCategoryBuckets();
  return buckets.length ? buckets.map((bucket) => `
    <section class="section-card shelf-card" id="category-${escapeHtml(bucket.key)}">
      <div class="smart-card-head">
        <div>
          <h2>${escapeHtml(bucket.label)}</h2>
          <div class="helper-text">${integer(bucket.count)} منتج مطابق</div>
        </div>
        <button class="ghost-btn shelf-more" type="button" data-action="scroll-top">أعلى الصفحة</button>
      </div>
      <div class="shelf-grid shelf-grid-category">
        ${bucket.products.slice(0, 6).map((product) => compactProductCardHtml(product, { actionLabel: 'شراء' })).join('')}
      </div>
    </section>
  `).join('') : '';
}

function renderStickyBarHtml() {
  const placeholder = state.view.type === 'company' ? 'ابحث في منتجات الشركة أو الكود' : 'ابحث باسم المنتج أو الشركة';
  return `
    <div class="opbar-row">
      <div class="search-shell">
        <input id="searchInput" type="search" placeholder="${escapeHtml(placeholder)}" value="${escapeHtml(state.search)}" />
        <div class="search-hint">بحث سريع للموزعين والتجار</div>
      </div>
      <button class="cart-chip" id="cartBtn" type="button" data-action="open-cart" aria-label="إجمالي المشتريات">
        <span class="cart-value mono" id="cartValue">${integer(cartTotal())}</span>
        <span class="cart-label" id="cartLabel">إجمالي المشتريات — اضغط لإتمام الشراء</span>
      </button>
    </div>
  `;
}
...
function renderFooterHtml() {
  const accountLabel = state.session ? 'حسابي' : 'تسجيل الدخول';
  return `
    <div class="footer-card">
      <div class="footer-row">
        <button class="footer-btn" id="homeBtn" data-action="go-home" type="button"><span>الرئيسية</span><small>عودة سريعة</small></button>
        <button class="footer-btn" id="tierFooterBtn" data-action="open-tiers" type="button"><span>الشريحة</span><small>تسعير العميل</small></button>
        <button class="footer-btn" id="dealsFooterBtn" data-action="open-deals" type="button"><span>العروض</span><small>صفقات اليوم</small></button>
        <button class="footer-btn" id="userBtn" data-action="account" type="button"><span>${escapeHtml(accountLabel)}</span><small>${state.session && state.session.userType === 'rep' ? 'ملف المندوب' : 'ملف الحساب'}</small></button>
        <button class="footer-btn" data-action="quick-shopping" type="button"><span>التسوق السريع</span><small>دخول مباشر</small></button>
      </div>
    </div>
  `;
}

function renderTierGateContentHtml() {
  const tiers = [...state.tiers].sort((a, b) => Number(a.min_order || 0) - Number(b.min_order || 0));
  return tiers.length ? tiers.map((tier) => `
    <article class="tier-gate-card">
      <div class="tier-name">${escapeHtml(tierDisplayLabel(tier))}</div>
      <div class="tier-meta">الحد الأدنى: ${num(tier.min_order || 0)} · الخصم: ${num(tier.discount_percent || 0)}%</div>
      <button class="primary-btn" type="button" data-action="choose-tier" data-tier-name="${escapeHtml(tier.tier_name)}" data-visible-label="${escapeHtml(tierDisplayLabel(tier))}">اختيار</button>
    </article>
  `).join('') : `<div class="empty-state">لا توجد شرائح معرفة</div>`;
}

function openTierGate(pendingAction = null) {
  state.pendingPurchaseAction = pendingAction || state.pendingPurchaseAction || null;
  if (els.tierGateContent) els.tierGateContent.innerHTML = renderTierGateContentHtml();
  setOverlay(document.getElementById('tierGateModal'), true);
}

function closeTierGate() {
  setOverlay(document.getElementById('tierGateModal'), false);
}

function resumePendingPurchase() {
  const pending = state.pendingPurchaseAction;
  if (!pending || !state.selectedTier) return;
  state.pendingPurchaseAction = null;
  setTimeout(() => {
    if (pending.type === 'product') {
      toggleProductFromCard(pending.productId, pending.unit, true);
      return;
    }
    if (pending.type === 'deal' || pending.type === 'flash') {
      toggleDeal(pending.type, pending.id, true);
    }
  }, 0);
}

function scrollToSectionByAction(actionName) {
  const map = {
    'open-companies': 'companiesSection',
    'open-categories': 'smartCategoriesSection',
    'scroll-best-selling': 'bestSellingSection',
    'scroll-top': 'mainHeader',
  };
  const id = map[actionName];
  if (!id) return;
  ...
  if (els.banner) {
    const headerHtml = `
      <div class="header-stack">
        <section class="header-banner-card">
          <div class="company-banner-wrap">
            ${companyBannerHtml()}
          </div>
          <div class="identity-row">
            <div class="company-identity">
              <span class="eyebrow">منصة توزيع B2B</span>
              <strong class="name">متجر الأهرام للتجارة والتوزيع</strong>
              <span class="sub">واجهة تشغيل احترافية للتجار وأصحاب المحلات والمندوبين.</span>
            </div>
            <div class="user-wrap">
              <button class="quick-action-btn" id="loginTopBtn" type="button" data-action="account">${escapeHtml(state.session ? 'حسابي' : 'تسجيل الدخول')}</button>
              <div class="user-menu hidden" id="userMenu" aria-hidden="true">
                <button type="button" data-action="my-data">حسابي</button>
                <button type="button" data-action="my-customers">عملائي</button>
                <button type="button" data-action="invoices">فواتيري</button>
                <button type="button" data-action="logout">تسجيل الخروج</button>
              </div>
            </div>
          </div>
          <div class="theme-contact-row">
            ${renderThemeSwitcherHtml()}
            ${contactActionsHtml()}
          </div>
          ${renderOperationalHeroHtml()}
          ${renderHeaderActionsRow()}
          ${renderQuickCardsHtml()}
        </section>
      </div>
    `;
    els.banner.innerHTML = headerHtml;
  }

  if (els.searchBar) {
    els.searchBar.innerHTML = renderStickyBarHtml();
  }

  if (els.quickFooter) {
    els.quickFooter.innerHTML = renderFooterHtml();
  }
...
function refreshHeaderValues() {
  const tierLabel = getSelectedTierLabel();
  const currentTierText = state.selectedTier ? tierLabel : 'أدخل واختار شريحتك';
  const tierBtn = document.getElementById('tierBtn');
  if (tierBtn) tierBtn.textContent = currentTierText;

  const userBtn = document.getElementById('userBtn');
  if (userBtn) {
    const span = userBtn.querySelector('span');
    if (span) span.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
    else userBtn.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
  }

  const loginTopBtn = document.getElementById('loginTopBtn');
  if (loginTopBtn) loginTopBtn.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';

  const cartValue = document.getElementById('cartValue');
  if (cartValue) cartValue.textContent = integer(cartTotal());

  const flashCountdown = document.getElementById('flashCountdownValue');
  const flashState = getFlashState();
  if (flashCountdown) flashCountdown.textContent = flashState.status === 'active' ? flashState.remaining : (flashState.status === 'expired' ? 'منتهي' : 'قريبًا');
  ...
  if (hero) {
    ...
    const progressBar = hero.querySelector('.hero-progress-bar > span');
    const progress = getTierProgressData();

    if (flashState.status === 'active' && flashState.offer) {
      if (title) title.textContent = 'عرض الساعة';
      if (countdown) countdown.textContent = flashState.remaining || '--:--:--';
      if (note) note.textContent = 'متبقي على انتهاء العرض';
      if (subtitle) subtitle.textContent = flashState.offer.title || '';
      if (progressBar) progressBar.style.width = '100%';
    } else {
      if (title) title.textContent = 'أنت الآن على شريحة';
      if (subtitle) subtitle.textContent = progress.currentLabel || 'الشريحة الرئيسية';
      if (note) note.textContent = `إجمالي السلة الحالي: ${num(progress.total || 0)}`;
      if (countdown) countdown.textContent = progress.nextLabel || 'الشريحة التالية';
      if (progressBar) progressBar.style.width = `${Number(progress.percent || 0).toFixed(2)}%`;
    }
  }

  const footerUser = document.querySelector('#quickFooter [data-action="account"] span');
  if (footerUser) footerUser.textContent = state.session ? 'حسابي' : 'تسجيل الدخول';
...
function renderHomePage() {
  ...
  const recommendedList = q ? recommended.filter((p) => matchesHomeSearch(p, q)) : recommended;
  const viewedList = q ? viewed.filter((p) => matchesHomeSearch(p, q)) : viewed;
  const bestList = q ? bestSelling.filter((p) => matchesHomeSearch(p, q)) : bestSelling;
  const companionList = q ? companion.filter((p) => matchesHomeSearch(p, q)) : companion;

  els.pageContent.innerHTML = `
    <div class="section-stack">
      ${renderQuickCardsHtml()}
      ${renderSmartCategoriesHtml()}
      ${renderCompaniesSectionHtml(companyRows)}
      <section class="section-card shelf-card">
        <div class="smart-card-head">
          <div>
            <h2>منتجات موصى بها لك</h2>
            <div class="helper-text">مختارة بناءً على نشاطك.</div>
          </div>
        </div>
        <div class="shelf-grid">
          ${recommendedList.length ? recommendedList.map((product) => compactProductCardHtml(product, { actionLabel: 'شراء' })).join('') : `<div class="empty-state">لا توجد عناصر بعد</div>`}
        </div>
      </section>

      <section class="section-card shelf-card" id="bestSellingSection">
        <div class="smart-card-head">
          <div>
            <h2>الأكثر مشاهدة</h2>
            <div class="helper-text">المنتجات الأكثر تصفحًا خلال الفترة الأخيرة.</div>
          </div>
        </div>
        <div class="shelf-grid">
          ${viewedList.length ? viewedList.map((product) => compactProductCardHtml(product, { actionLabel: 'شراء' })).join('') : `<div class="empty-state">لا توجد عناصر بعد</div>`}
        </div>
      </section>
      ...
      <section class="section-card shelf-card" id="bestSellingSection">
        <div class="smart-card-head">
          <div>
            <h2>الأكثر مبيعًا</h2>
            <div class="helper-text">قوة البيع الحقيقية داخل المنصة.</div>
          </div>
        </div>
        <div class="shelf-grid">
          ${bestList.length ? bestList.map((product) => compactProductCardHtml(product, { actionLabel: 'شراء' })).join('') : `<div class="empty-state">لا توجد عناصر بعد</div>`}
        </div>
      </section>
      ...
      <section class="section-card shelf-card">
        <div class="smart-card-head">
          <div>
            <h2>مقترحات تناسب سلتك</h2>
            <div class="helper-text">منتجات تكمل الطلب وتساعد على التجميع السريع.</div>
          </div>
        </div>
        <div class="shelf-grid">
          ${companionList.length ? companionList.map((product) => compactProductCardHtml(product, { actionLabel: 'أضف مع الطلب' })).join('') : `<div class="empty-state">لا توجد عناصر بعد</div>`}
        </div>
      </section>

      ${renderCategoryProductsSectionHtml()}
    </div>
  `;
}
      ...
      <section class="tier-grid">
        ${tiers.length ? tiers.map((tier) => `
          <article class="tier-card">
            ...
            <button class="primary-btn" data-action="choose-tier" data-tier-name="${escapeHtml(tier.tier_name)}" data-visible-label="${escapeHtml(tierDisplayLabel(tier))}">
  ...
  els.pageContent.innerHTML = `
    <div class="section-stack">
      <section class="hero-card">
        ...
        <div class="hero-countdown mono">${escapeHtml(heroCountdown || '--:--:--')}</div>
        <div class="hero-note">${escapeHtml(heroNote)}</div>
      </section>
      <section class="deal-list">
...
function renderApp() {
  ...
  if (els.quickFooter) els.quickFooter.classList.toggle('hidden', isRegister);

  if (isRegister) {
    if (els.pageContent) els.pageContent.innerHTML = '';
    renderRegisterPage();
...
function toggleProductFromCard(productId, unitOverride = null, bypassTierGate = false) {
  const product = state.products.find((item) => item.product_id === productId);
  if (!product) return;

  if (!bypassTierGate && !state.selectedTier) {
    state.pendingPurchaseAction = { type: 'product', productId, unit: unitOverride || null };
    openTierGate(state.pendingPurchaseAction);
    return;
  }

  const card = document.querySelector(`.product-card[data-product-id="${CSS.escape(productId)}"]`);
  const unit = unitOverride || card?.querySelector('.unit-chip.active')?.getAttribute('data-unit') || currentUnitForProduct(product);
...
function toggleDeal(type, id, bypassTierGate = false) {
  const source = type === 'deal' ? state.dailyDeals : state.flashOffers;
  const item = source.find((row) => String(row.id) === String(id));
  if (!item) return;
  ...
  if (!bypassTierGate && !state.selectedTier) {
    state.pendingPurchaseAction = { type, id };
    openTierGate(state.pendingPurchaseAction);
    return;
  }
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (closeTarget === 'tierGateModal') return closeTierGate();
    ...
    if (action === 'home-cta' || action === 'go-home') {
      navigate('#home');
      setTimeout(() => document.getElementById('companiesSection')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      return;
    }
    if (action === 'open-tiers' || action === 'go-tiers') {
      navigate('#tiers');
      return;
    }
    if (action === 'open-deals') {
      navigate('#deals');
      return;
    }
    if (action === 'open-flash') {
      navigate('#flash');
      return;
    }
    if (action === 'open-categories') {
      document.getElementById('smartCategoriesSection')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'jump-category') {
      const key = target.getAttribute('data-category-key');
      document.getElementById(`category-${CSS.escape(key || '')}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'open-companies') {
      document.getElementById('companiesSection')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'scroll-best-selling') {
      document.getElementById('bestSellingSection')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'scroll-top') {
      document.getElementById('mainHeader')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      return;
    }
    if (action === 'quick-shopping') {
      navigate('#home');
      setTimeout(() => document.getElementById('searchInput')?.focus(), 100);
      return;
    }
    if (action === 'support') {
      window.open(`https://wa.me/${CONFIG.supportWhatsapp || '201040880002'}`, '_blank', 'noopener,noreferrer');
      return;
    }
    if (action === 'account') {
      if (!state.session) {
  ...
  document.addEventListener('click', (event) => {
    const clickedInsideUser = event.target.closest('.user-wrap') || event.target.closest('[data-action="account"]');
    ...
    if (!clickedInsideModal && event.target === document.getElementById('tierGateModal')) closeTierGate();
    if (!clickedInsideModal && event.target === els.myDataModal) closeMyData();
    if (event.target === els.cartDrawer) closeCart();
  });

  if (els.submitLogin) els.submitLogin.addEventListener('click', handleLogin);
  if (els.checkoutBtn) {
...
async function init() {
  ...
  window.addEventListener('scroll', scheduleHeaderCollapse, { passive: true });
  scheduleHeaderCollapse();
...
(function () {
  ...
  const THEMES = {
    ...
    'dark-enterprise': {
      ...
      vars: {
        ...
        '--hero-grad': 'radial-gradient(circle at top right, rgba(148,163,184,.15), transparent 34%), linear-gradient(145deg, rgba(17,22,29,.98), rgba(7,9,13,.98))',
        '--cta-grad': 'linear-gradient(135deg, #dbe4ef 0%, #94a3b8 48%, #5c6b7c 100%)',
        '--card-grad': 'linear-gradient(180deg, rgba(20,26,34,.98), rgba(10,14,19,.98))',
        '--glow': '0 0 0 1px rgba(148,163,184,.16), 0 0 32px rgba(148,163,184,.12)',
        '--focus': 'rgba(148,163,184,.14)',
        '--chip-bg': 'rgba(255,255,255,.04)',
      },
    },
    'light-professional': {
      label: 'Light Professional Theme',
      color: '#F4F6F8',
      vars: {
        '--bg': '#F4F6F8',
        '--bg-2': '#EBEEF2',
        ...
        '--hero-grad': 'radial-gradient(circle at top right, rgba(15,23,42,.05), transparent 34%), linear-gradient(145deg, rgba(255,255,255,.99), rgba(244,246,248,.99))',
        '--cta-grad': 'linear-gradient(135deg, #111827 0%, #334155 48%, #0f172a 100%)',
        '--card-grad': 'linear-gradient(180deg, rgba(255,255,255,.98), rgba(245,248,252,.98))',
        '--glow': '0 0 0 1px rgba(15,23,42,.08), 0 0 24px rgba(15,23,42,.08)',
        '--focus': 'rgba(15,23,42,.08)',
        '--chip-bg': 'rgba(0,0,0,.03)',
      },
    },
    'blue-distribution': {
      label: 'Blue Distribution Theme',
      color: '#1D4ED8',
      vars: {
        '--bg': '#06111f',
        '--bg-2': '#091527',
        ...
        '--shadow-soft': '0 10px 24px rgba(0,0,0,.28)',
        '--hero-grad': 'radial-gradient(circle at top right, rgba(59,130,246,.18), transparent 34%), linear-gradient(145deg, rgba(13,27,47,.98), rgba(6,17,31,.98))',
        '--cta-grad': 'linear-gradient(135deg, #93c5fd 0%, #3b82f6 48%, #1d4ed8 100%)',
        '--card-grad': 'linear-gradient(180deg, rgba(14,31,52,.98), rgba(9,18,30,.98))',
        '--glow': '0 0 0 1px rgba(59,130,246,.18), 0 0 32px rgba(59,130,246,.14)',
        '--focus': 'rgba(59,130,246,.14)',
        '--chip-bg': 'rgba(255,255,255,.04)',
      },
    },
    'emerald-sales': {
      label: 'Emerald Sales Theme',
      color: '#059669',
      vars: {
        '--bg': '#07130e',
        '--bg-2': '#0a1b14',
  ...
  function updateThemeDots() {
    ...
    document.querySelectorAll('.theme-dot').forEach((dot) => {
      dot.classList.toggle('active', dot.getAttribute('data-theme-key') === current);
      const key = dot.getAttribute('data-theme-key');
      const theme = THEMES[key];
      if (theme) dot.style.setProperty('--theme-color', theme.color);
    });
  }
