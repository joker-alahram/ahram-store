...
export class MockCommerceGateway implements CommerceGateway {
  ...
  async dashboardSummaries(): Promise<SummaryMetric[]> {
    await delay(75);
    ...
    return [
      { label: 'Orders', value: String(totalOrders), tone: 'default' as const },
      { label: 'Customers', value: String(totalCustomers), tone: 'success' as const },
      { label: 'Representatives', value: String(totalReps), tone: 'warning' as const },
      { label: 'Revenue', value: `${totalRevenue.toLocaleString('en-US')} EGP`, tone: 'success' as const },
    ];
  }

  async repPerformance(): Promise<SummaryMetric[]> {
    await delay(50);
    return [
      { label: 'Top rep', value: this.db.reps[0]?.name ?? '-', tone: 'success' as const },
      { label: 'Active portfolios', value: `${this.db.customers.filter((c) => c.sales_rep_id).length}`, tone: 'default' as const },
    ];
  }

  async customerTotals(): Promise<SummaryMetric[]> {
    await delay(50);
    return [
      { label: 'Direct customers', value: `${this.db.customers.filter((c) => c.customer_type === 'direct').length}`, tone: 'default' as const },
      { label: 'Rep customers', value: `${this.db.customers.filter((c) => c.customer_type === 'rep_customer').length}`, tone: 'warning' as const },
    ];
  }
