...
function setCheckoutFeedback(message = '', variant = 'idle') {
  state.checkoutStatus = String(message || '');
  state.checkoutStatusType = variant;
  if (els.checkoutStatus) {
    els.checkoutStatus.textContent = state.checkoutStatus || '';
    els.checkoutStatus.className = `status-message ${variant ? `is-${variant}` : ''}`.trim();
    els.checkoutStatus.hidden = !state.checkoutStatus;
  }
}
function resetCheckoutStage() {
  state.checkoutStage = 'validate';
  setCheckoutFeedback('', 'idle');
  syncCheckoutButton();
}
function setCheckoutStage(stage, message = '', variant = 'idle') {
  state.checkoutStage = stage;
  setCheckoutFeedback(message, variant);
  syncCheckoutButton();
}
function syncCheckoutButton() {
  if (!els.checkoutBtn) return;
  if (state.checkoutStage === 'loading') {
    els.checkoutBtn.textContent = 'جارى التحقق...';
    els.checkoutBtn.disabled = true;
  } else if (state.checkoutStage === 'submit') {
    els.checkoutBtn.textContent = 'إرسال الطلب الآن';
    els.checkoutBtn.disabled = false;
  } else {
    els.checkoutBtn.textContent = 'تحقق من الطلب';
    els.checkoutBtn.disabled = false;
  }
  if (els.checkoutStatus) {
    els.checkoutStatus.textContent = state.checkoutStatus || '';
    els.checkoutStatus.className = `status-message ${state.checkoutStatusType ? `is-${state.checkoutStatusType}` : ''}`.trim();
    els.checkoutStatus.hidden = !state.checkoutStatus;
  }
}
...
function updateHeader() {
  ...
  if (els.tierBtn) els.tierBtn.textContent = tierLabel;
  if (els.loginBtn) {
    els.loginBtn.querySelector('#loginBtnText')?.replaceChildren(document.createTextNode(getSessionLabel()));
  }
...
function evaluateCheckoutConstraints({ applySideEffects = false } = {}) {
  if (!state.session) {
    if (applySideEffects) {
    ...
    return { ok: false, message: 'يلزمك التسجيل أولًا لإرسال الطلب.' };
  }
  ...
  if (state.session.userType === 'rep' && !state.selectedCustomer) {
    ...
    return { ok: false, message: 'يلزمك اختيار عميل قبل إرسال الطلب.' };
  }

  if (!state.cart.length) {
    if (applySideEffects) toast('السلة فارغة.');
    return { ok: false, message: 'السلة فارغة.' };
  }

  const tier = getSelectedTierObject();
  if (!tier) {
    if (applySideEffects) toast('يلزمك اختيار الشريحة أولًا.');
    return { ok: false, message: 'يلزمك اختيار الشريحة أولًا.' };
  }
  ...
  if (total < minOrder) {
    ...
    const message = `المتبقي لتحقيق الشريحة: ${num(diff)}. أضف منتجات لإكمال الخصم.`;
    if (applySideEffects) {
      closeCart();
      navigate('#home');
      toast(message);
...
function validateBeforeOrder() {
  return evaluateCheckoutConstraints({ applySideEffects: true }).ok;
}
...
async function submitCheckoutOrder() {
  if (!evaluateCheckoutConstraints({ applySideEffects: true }).ok) {
    resetCheckoutStage();
    return;
  }

  setCheckoutStage('loading', 'جارى إرسال الطلب...', 'loading');
  try {
    ...
    console.error(e);
    setCheckoutStage('submit', 'تعذر إرسال الطلب الآن، حاول مرة أخرى.', 'error');
    syncCheckoutButton();
    toast('فشل إرسال الطلب');
  }
}
async function handleCheckout() {
  if (state.checkoutStage === 'loading') return;
  if (state.checkoutStage === 'submit') return submitCheckoutOrder();

  setCheckoutStage('loading', 'جارى التحقق من الطلب والاسعار والبيانات', 'loading');
  const prerequisite = evaluateCheckoutConstraints({ applySideEffects: false });
  if (!prerequisite.ok) {
    setCheckoutStage('validate', prerequisite.message, 'error');
    return;
  }
  ...
  try {
    ...
    if (!dbCheck.ok) {
      setCheckoutStage('validate', dbCheck.message, 'error');
      return;
    }

    setCheckoutStage('submit', 'تم التحقق بنجاح يمكنك الآن إرسال الطلب', 'success');
    syncCheckoutButton();
  } catch (e) {
    console.error(e);
    setCheckoutStage('validate', 'تعذر التحقق الآن، حاول مرة أخرى.', 'error');
  }
}
function checkout() {
...
function wireGlobalEvents() {
  document.addEventListener('click', async (event) => {
    ...
    if (interactive && !event.target.closest('[data-action]')) return;
    ...
    if (action === 'open-product') {
      ...
      const product = state.products.find((item) => String(item.product_id) === String(productId));
      if (product?.company_id) {
        navigate(`#company/${encodeURIComponent(product.company_id)}`);
      } else {
        toast('تعذر فتح المنتج الآن');
      }
      return;
    }
